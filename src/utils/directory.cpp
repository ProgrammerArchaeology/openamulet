...
Am_Directory::addEntry(WIN32_FIND_DATA *aEntryData, long aLevel)
{
  ...
  if ((aEntryData->dwFileAttributes & FILE_ATTRIBUTE_HIDDEN) ==
  ...
  if ((aEntryData->dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) ==
      FILE_ATTRIBUTE_DIRECTORY) {
    // check if the pattern equals . or ..
    size_t nameLength = strlen(aEntryData->cFileName);
    if ((nameLength >= 1 &&
         strcmp((aEntryData->cFileName) + nameLength - 1, ".") == 0) ||
        (nameLength >= 1 &&
         strcmp((aEntryData->cFileName) + nameLength - 2, "..") == 0)) {
...
std::string
Am_Directory::getLongPath(mEntry *aEntry)
{
  ...
  std::string longPath;
  for (std::list<char *>::iterator it = parts.begin(); it != parts.end();
       ++it) {
    longPath += *it;
  }
...
Am_Directory::readEntries(long aLevel)
{
  ...
  if (aLevel > 0) {
    ...
    for (long x = 0; x <= aLevel; ++x) {
      ...
      for (LEVEL_ENTRIES::iterator it = selectedRange.first;
           it != selectedRange.second; ++it) {
        directoriesToScan.push_back(it);
      }

      // for each entry read the entries in this directory
      for (ITERATOR_LIST::const_iterator itl = directoriesToScan.begin();
           itl != directoriesToScan.end(); ++itl) {
        ...
        if (pathToCheck.compare(pathToCheck.length() - 1, 1, ".") != 0 &&
            pathToCheck.compare(pathToCheck.length() - 2, 2, "..") != 0) {
          pathToCheck.append("/*");
...
void
Am_Directory::_readEntries(std::string aPattern, long aLevel)
{
  // check if the pattern equals . or ..
  if ((aPattern.length() >= 1 &&
       aPattern.compare(aPattern.length() - 1, 1, ".") == 0) ||
      (aPattern.length() >= 2 &&
       aPattern.compare(aPattern.length() - 2, 2, "..") == 0)) {
    return;
  }
  ...
  while (continueLoop == true) {
    ...
    if (FindNextFile(mDirectoryIterator, mEntryInformation) == false) {
      ...
      if (GetLastError() == ERROR_NO_MORE_FILES) {
        ...
        std::cout << "There happend an error while reading the directory."
                  << std::endl;
      }
...
void
Am_Directory::printDirectories(long aHowManyLevels, long aStartLevel)
{
  ...
  for (int x = -1; x < aHowManyLevels; ++x) {
    ...
    LEVEL_RANGE_ITERATORS selectedRange =
...
void
Am_Directory::printFiles(long aHowManyLevels, long aStartLevel)
{
  ...
  for (int x = -1; x < aHowManyLevels; ++x) {
    ...
    LEVEL_RANGE_ITERATORS selectedRange =
        mLevel2Directories.equal_range(aStartLevel + x);
    for (LEVEL_ENTRIES::const_iterator itDirectories = selectedRange.first;
         itDirectories != selectedRange.second; ++itDirectories) {
