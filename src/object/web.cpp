...
{
public:
  Am_Web_Data(Am_Web_Create_Proc *create, Am_Web_Initialize_Proc *initialize,
...
bool
Web_Constraint::Get(const Am_Slot &fetching_slot, Am_Value &out_value,
                    bool &out_changed)
{
...
void
Web_Constraint::Invalidated(const Am_Slot &slot_invalidated,
                            Am_Constraint *invalidating_constraint,
                            const Am_Value &)
{
...
void
Web_Constraint::Slot_Event(Am_Object_Context *oc, const Am_Slot &slot)
{
  Output_Port *curr;
...
Am_Constraint *
Web_Constraint::Constraint_Added(const Am_Slot &adding_slot)
{
...
Am_Constraint *
Web_Constraint::Dependency_Added(const Am_Slot &adding_slot)
{
...
bool
Web_Constraint::Is_Overridden_By(const Am_Slot & /*slot*/,
...
bool
Am_Web_Data::Validate(const Am_Slot &context, Am_Value &out_value,
                      bool &out_changed)
{
  validating = true;
  ...
  if (initialized) {
    ...
    if (changed_first) {
      if (validate_proc) {
        ...
        Am_PUSH_CC(&wc) validate_proc(events);
        Am_POP_CC()
      }
      Clear_Changed();
    }
    ...
    if (initialize_proc) {
      ...
      Am_PUSH_CC(&wc) initialize_proc(context, init);
      Am_POP_CC()
    }
    initialized = true;
  }
...
void
Web_Context::Set(const Am_Object_Advanced &object, Am_Slot_Key key,
                 const Am_Value &new_value, Am_Slot_Flags flags)
{
  if (flags & Am_NO_DEPENDENCY) {
...
void
Web_Context::Note_Unchanged(const Am_Object_Advanced &object, Am_Slot_Key key)
{
  Output_Port *curr = owner->out_list->Search(object, key);
  if (curr) {
    if (curr->context == validating_slot) {
...
const Am_Value &
Web_Context::Raise_Get_Exception(const Am_Value &value,
                                 const Am_Object_Advanced &, Am_Slot_Key,
...
void
Web_Context::Set_Data(Am_Wrapper * /* data*/)
{
  //// NIY
}
...
void
Am_Web_Init::Note_Input(Am_Object_Advanced object, Am_Slot_Key key)
{
  object.Get_Slot(key).Add_Dependency(*data);
}

void
Am_Web_Init::Note_Output(Am_Object_Advanced object, Am_Slot_Key key)
{
