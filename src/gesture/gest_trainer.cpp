...
bool
Am_Gesture_Trainer::Delete_Class(Am_String classname, bool unique)
{
  if (!data)
    data = new Am_Gesture_Trainer_Data();
  else if (unique)
...
bool
Am_Gesture_Trainer::Rename_Class(Am_String old_name, Am_String new_name,
                                 bool unique)
{
  if (!data)
    data = new Am_Gesture_Trainer_Data();
...
bool
Am_Gesture_Trainer::Delete_Example(Am_String classname, Am_Feature_Vector fv,
                                   bool unique)
{
  if (!data)
    data = new Am_Gesture_Trainer_Data();
...
Am_Value_List
Am_Gesture_Trainer::Get_Examples(Am_String classname)
{
  if (data) {
    Am_Gesture_Trainer_Data::Gesture_Class *c = data->Find_Class(classname);
    if (c)
...
std::ostream &
operator<<(std::ostream &s, Am_Gesture_Trainer &tr)
{
...
void
Am_Gesture_Trainer_Data::Add_Class(Am_String classname)
{
...
void
Am_Gesture_Trainer_Data::Delete_Class(Gesture_Class *c)
{
  if (c->next)
    c->next->prev = c->prev;
  else
...
Am_Gesture_Trainer_Data::Gesture_Class *
Am_Gesture_Trainer_Data::Find_Class(Am_String classname)
{
  Gesture_Class *c;
...
Am_Gesture_Trainer_Data::Train()
{
    ...
    if (nexamples == 0) {
      ...
      if (d->is_dot) {
        if (dotclass == nullptr) {
          ...
          std::cerr << "Cannot discriminate the dot-like gestures " << d->name
                    << " and " << dotclass->name << std::endl;
          goto LFail;
        }
      }
    }
  ...
  if (fabs(det) <= EPS && !fix_classifier(avgcov, cldata->invavgcov))
    goto LFail;
