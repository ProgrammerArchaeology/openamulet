...
static int
crosses_to_right_of(int x, int y, int x1, int y1, int x2, int y2)
{
  if ((y1 < y && y < y2) && (x2 - x) * (y1 - y) < (x1 - x) * (y2 - y))
    return 1;
  else if ((y2 < y && y < y1) && (x1 - x) * (y2 - y) < (x2 - x) * (y1 - y))
    return -1;
  else
    return 0;
}
                  ...
                  const Am_Object &ref_obj))
{
  if ((bool)in_obj.Get(Am_VISIBLE)) {
    ...
    // do simple bounding box test first
    if (x < -threshold || x > width + threshold || y < -threshold ||
        y > height + threshold) {
      return Am_No_Object;
    }
...
//web functions
static bool
polygon_web_create(const Am_Slot &slot)
{
...
static inline float
scale_factor(int new_dim, int old_dim)
{
  ...
  return (float)((new_dim > 1 ? new_dim - 1 : 0.25f) /
...
static void
general_polygon_validate(Am_Point_List &pl, int &left, int &top, int &width,
                         int &height, Am_Style ls, Am_Drawonable *draw)
{
  int pl_left, pl_top, pl_width, pl_height;
  draw->Get_Polygon_Bounding_Box(pl, ls, pl_left, pl_top, pl_width, pl_height);

  if (left == pl_left && top == pl_top && width == pl_width &&
      height == pl_height)
  ...
  draw->Get_Polygon_Bounding_Box(new_pl, ls, curr_left, curr_top, curr_width,
                                 curr_height);
...
init()
{
  ...
  temp.Get_Slot(Am_POINT_LIST)
      .Set_Demon_Bits(Am_MOVING_REDRAW | Am_EAGER_DEMON);
}

static Am_Initializer *initializer =
    new Am_Initializer(DSTR("Am_Polygon"), init, 2.1f);
