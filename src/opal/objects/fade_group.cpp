                  ...
                  int y_offset))
{
  ...
  local_drawonable->Draw_Rectangle(Am_No_Style, clear_halftone(value), left,
                                   top, width, height, Am_DRAW_COPY);
  mask_drawonable->Draw_Rectangle(Am_No_Style, clear_halftone(value), left, top,
                                  width, height, Am_DRAW_COPY);
  local_drawonable->Pop_Clip();
  drawonable->Push_Clip(left, top, width, height);
  drawonable->Bitblt(left, top, width, height, mask_drawonable, left, top,
                     Am_DRAW_GRAPHIC_NIMP);
  drawonable->Bitblt(left, top, width, height, local_drawonable, left, top,
                     Am_DRAW_GRAPHIC_OR);
  drawonable->Pop_Clip();
}
...
init()
{
  ...
  ((Am_Object_Advanced &)Am_Fade_Group)
      .Get_Slot(Am_VALUE)
