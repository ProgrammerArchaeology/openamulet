...
bool
Animation_Constraint::Get(const Am_Slot &, Am_Value &, bool &)
{
  return false;
}
...
void
Animation_Constraint::Slot_Event(Am_Object_Context *, const Am_Slot &)
{
}

// Just return the prototype.
Am_Constraint *
Animation_Constraint::Get_Prototype()
{
  return prototype;
}
...
bool
Animation_Constraint::Is_Instance_Of(Am_Constraint *proto)
{
...
static void
animator_initialize_demon(Am_Slot slot)
{
  Am_Constraint_Iterator iter = slot;
  Am_Constraint *curr;
  for (iter.Start(); !iter.Last(); iter.Next()) {
...
void
Animation_Constraint::Initialize()
{
  if (must_initialize_instance) {
...
Am_Constraint *
Animation_Constraint::Dependency_Added(const Am_Slot &)
{
  return this;
}

void
Animation_Constraint::Constraint_Removed(const Am_Slot &removing_slot)
{
...
void
Animation_Constraint::Dependency_Removed(const Am_Slot &)
{
}

bool
Animation_Constraint::Is_Overridden_By(const Am_Slot & /*slot*/,
...
Am_Constraint *
Animation_Constraint::Create(const Am_Slot &current_slot,
...
Am_Constraint *
Animation_Constraint::Copy(const Am_Slot &current_slot,
                           const Am_Slot & /*new_slot*/)
{
  if (current_slot == primary_slot) {
    Animation_Constraint *copy =
        new Animation_Constraint(prototype, interpolator.Copy());
...
// Just return the id.
Am_ID_Tag
Animation_Constraint::ID() const
{
  return id;
}

const char *
Animation_Constraint::Get_Name()
{
...
static void
remove_dependency(Am_Slot slot, Animation_Constraint *self)
{
...
static void
remove_constraint(Am_Slot slot, Animation_Constraint *self)
{
  Am_Constraint_Iterator iter(slot);
  for (iter.Start(); !iter.Last(); iter.Next()) {
    Am_Constraint *curr = iter.Get();
...
void
Animation_Constraint::Set_Target_Value(const Am_Slot &changed_slot,
                                       const Am_Value &new_value)
{
  if (must_initialize_instance)
    return;
  ...
  if (multi_slot) {
    ...
    for (slots.Start(), targets.Start(); !slots.Last() && !targets.Last();
    ...
    std::cerr << "** Animator " << interpolator
              << " received change notification for "
              << "unknown slot ";
...
Am_Constraint *
Am_Animate_With(const Am_Object &animator)
{
  Am_Value v;
  ...
  if (v.Valid())
    ...
    return new Animation_Constraint(animator);
}
...
Am_Object
Am_Get_Animator(Am_Constraint *constraint)
{
...
inline static void
set_animated_slot(Am_Slot_Key key, Am_Object_Advanced &obj,
                  Am_Constraint *constraint, const Am_Value &value)
{
  if (constraint) {
