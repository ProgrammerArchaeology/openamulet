...
Am_Point_List_Data::~Am_Point_List_Data() { Make_Empty(); }
...
void
Am_Point_List_Data::Insert(float x, float y, Am_Point_Item *current,
                           Am_Insert_Position position)
{
  ++number;
  Am_Point_Item *item = new Am_Point_Item(x, y);
...
void
Am_Point_List_Data::Set(float x, float y, Am_Point_Item *current)
{
  if (!current)
...
std::ostream &
operator<<(std::ostream &os, Am_Point_List &list)
{
  list.Print(os);
  return os;
}
...
void
Am_Point_List::Print(std::ostream &os) const
{
  if (data)
    data->Print(os);
  else
...
{
  return (in_data &&
          (in_data->ID() == Am_Point_List_Data::Am_Point_List_Data_ID()));
}
bool
Am_Point_List::Test(const Am_Value &in_value)
{
  return (in_value.value.wrapper_value &&
          (in_value.type == Am_Point_List_Data::Am_Point_List_Data_ID()));
}
Am_Value_Type
Am_Point_List::Type_ID()
{
...
bool
Am_Point_List::operator==(const Am_Point_List &test_value) const
{
...
void
Am_Point_List::Get(float &x, float &y) const
{
  if (!item)
    Am_Error("** Am_Point_List::Get: no item is current\n");
  x = item->x;
...
void
Am_Point_List::Delete(bool unique)
{
  if (item && data) {
    if (unique)
      data->Make_Unique(data, item);
...
void
Am_Point_List::Get_Extents(int &min_x, int &min_y, int &max_x, int &max_y) const
{
  if (!data || !data->head)
    return;
...
// origin_y = 0
void
Am_Point_List::Scale(float scale_x, float scale_y, int origin_x, int origin_y,
                     bool unique)
{
  if (!data)
    return;
...
void
Am_Point_List::Translate(int offset_x, int offset_y, bool unique)
{
  if (!data)
    return;
