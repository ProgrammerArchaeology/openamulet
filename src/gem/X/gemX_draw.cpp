...
static short
get_graphical_line_thickness(const Am_Style &ls)
{
  if (ls == Am_No_Style)
    return 0;
  else {
...
inline void
adjust_gcvalues_color(XGCValues &values, unsigned long &mask,
...
void
Am_Drawonable_Impl::adjust_gcvalues_stipple(XGCValues &values,
                                            unsigned long &mask,
...
inline void
adjust_gcvalues_thickness(XGCValues &values, unsigned long &mask,
                          short thickness)
{
...
inline void
set_gcvalues_fill_poly_rule(XGCValues &values, unsigned long &mask,
...
inline void
adjust_gcvalues_capstyle(XGCValues &values, unsigned long &mask,
                         Am_Line_Cap_Style_Flag cap)
{
  switch (cap) {
  case Am_CAP_BUTT:
...
void
Am_Drawonable_Impl::Draw_Arc(const Am_Style &ls, const Am_Style &fs, int left,
                             ...
                             Am_Arc_Style_Flag)
{
  ...
  if (ls.Valid()) { //if there is a line style
    ...
    int half_thickness = thickness / 2;
    if ((height <= ((unsigned)thickness) * 2) ||
        (width <=
         ((unsigned)thickness) * 2)) { // line-style too thick to fit in bbox,
      //so fill bbox with line-style color
      set_gc_using_fill(ls, f);
      XFillArc(disp, xlib_drawable, gc, left, top, width, height, angle1,
               angle2);
    } else {            // The usual case -- bbox can accommodate the line-style
      if (fs.Valid()) { //if there is a filling style
        ...
        int fill_height =
            height > (unsigned)thickness2 ? height - thickness2 : 0;
        if (fill_width && fill_height) {
          set_gc_using_fill(fs, f);
          XFillArc(disp, xlib_drawable, gc, (left + even_thickness),
                   (top + even_thickness), fill_width, fill_height, angle1,
                   angle2);
        }
      }
      set_gc_using_line(ls, f);
      XDrawArc(disp, xlib_drawable, gc, (left + half_thickness),
               (top + half_thickness), (width - thickness),
               (height - thickness), angle1, angle2);
    }
  } else {            // No line style
    if (fs.Valid()) { //if there is a filling style but no line style
      set_gc_using_fill(fs, f);
...
//   f = Am_DRAW_COPY
void
Am_Drawonable_Impl::Draw_Line(const Am_Style &ls, int x1, int y1, int x2,
                              int y2, Am_Draw_Function f)
{
  GC gc = screen->gc;
  Display *disp = screen->display;
...
void
draw_roundtangle_outline(Am_Drawonable_Impl *d, int left, int top, int right,
                         int bottom, unsigned short x_radius,
                         unsigned short y_radius, int thickness,
                         int half_thickness,
                         int, // th2 not used
                         int corner_width, int corner_height)
{
  ...
  if (vert_line_y1 < vert_line_y2) {
    ...
    seg[seg_count].x1 = left_line_x;
    seg[seg_count].y1 = vert_line_y1;
    seg[seg_count].x2 = left_line_x;
    seg[seg_count].y2 = vert_line_y2;
    seg_count += 1;
    // Bottom right to top right
    seg[seg_count].x1 = right_line_x;
    seg[seg_count].y1 = vert_line_y2;
    seg[seg_count].x2 = right_line_x;
    seg[seg_count].y2 = vert_line_y1;
    seg_count += 1;
  }

  if (horiz_line_x1 < horiz_line_x2) {
    // Bottom left to bottom right
    seg[seg_count].x1 = horiz_line_x1;
    seg[seg_count].y1 = bot_line_y;
    seg[seg_count].x2 = horiz_line_x2;
    seg[seg_count].y2 = bot_line_y;
    seg_count += 1;
    // Top right to top left
    seg[seg_count].x1 = horiz_line_x2;
    seg[seg_count].y1 = top_line_y;
    seg[seg_count].x2 = horiz_line_x1;
    seg[seg_count].y2 = top_line_y;
    seg_count += 1;
  }
  XDrawSegments(d->screen->display, d->xlib_drawable, d->screen->gc, seg,
                seg_count);
...
//   f = Am_DRAW_COPY
void
Am_Drawonable_Impl::Draw_Roundtangle(const Am_Style &ls, const Am_Style &fs,
                                     int left, int top, int width, int height,
                                     ...
                                     Am_Draw_Function f)
{
  ...
  if (ls != Am_No_Style) { //if there is a line style
    if ((height < th2) ||
      ...
      if (fs != Am_No_Style) { //if there is a filling style
        ...
        draw_roundtangle_fill(this, left, top, width, height, right, bottom,
                              x_radius, y_radius, thickness, half_thickness,
                              th2, corner_width, corner_height);
      }
      set_gc_using_line(ls, f);
      draw_roundtangle_outline(this, left, top, right, bottom, x_radius,
                               y_radius, thickness, half_thickness, th2,
    ...
    if (fs != Am_No_Style) { //if there is a filling style but no line style
      set_gc_using_fill(fs, f);
      draw_roundtangle_fill(this, left, top, width, height, right, bottom,
                            x_radius, y_radius, thickness, half_thickness, th2,
                            corner_width, corner_height);
    }
  }
}
...
//
void
Am_Drawonable_Impl::Set_Clip(Am_Region *region)
{
...
void
Am_Drawonable_Impl::Set_Cursor(Am_Cursor new_cursor)
{
  cursor = new_cursor;
...
inline float
color_error(int r1, int r2, int g1, int g2, int b1, int b2)
{
...
void
Paint_It_Black(XColor &c, Display *d, Colormap cm)
{
  c.red = 0;
  c.blue = 0;
  c.green = 0;
