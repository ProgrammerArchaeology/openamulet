...
{
  transform(data->points.begin(), data->points.end(), data->points.begin(),
...
// Forward reference
static void compute_mitered_join_extents(Am_Point_List pl, int thick,
...
// to it
static bool
next_vertex(Am_Point_List &pl, int x, int y, int &nextx, int &nexty, double &ux,
            double &uy)
{
...
static void
compute_mitered_join_extents(Am_Point_List pl, int thick,
                             Am_Line_Cap_Style_Flag cap, int &out_left,
                             int &out_top, int &out_right, int &out_bottom)
{
...
void
Am_Drawonable_Impl::poly_draw_it(const Am_Style &ls, const Am_Style &fs,
                                 Am_Draw_Function f, XPoint *ar, int num_points)
{
  if (fs != Am_No_Style) { // if there is a fill style, fill it.
    set_gc_using_fill(fs, f);
    XFillPolygon(screen->display, xlib_drawable, screen->gc, ar, num_points,
                 Complex, CoordModeOrigin);
  }
  if (ls != Am_No_Style) { // if there is a line style, outline it.
    set_gc_using_line(ls, f);
    XDrawLines(screen->display, xlib_drawable, screen->gc, ar, num_points,
               CoordModeOrigin);
  }
}
...
void
Am_Drawonable_Impl::Draw_2_Lines(const Am_Style &ls, const Am_Style &fs, int x1,
...
// such, so provide a function that doesn't need to allocate an Am_Point_Array
void
Am_Drawonable_Impl::Draw_3_Lines(const Am_Style &ls, const Am_Style &fs, int x1,
