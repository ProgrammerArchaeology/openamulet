...
Am_Image_Array::Am_Image_Array(unsigned int width, unsigned int height,
                               int depth, Am_Style initial_color)
{
...
int
Am_Image_Array::Write_To_File(const char *file_name, Am_Image_File_Format form)
{
  if (data)
...
void
Am_Image_Array::Get_Size(int &width, int &height) const
{
  if (data)
...
{
  Am_WINCLEAN_CONSTR(Am_Image_Array_Data) m_nGrayFactor = -1;
}
...
#if _MSC_VER >= 1200
  HBITMAP hbmpOld =
      reinterpret_cast<HBITMAP__ *>(SelectObject(hdcMem, hbmpNew));
#else
...
#if _MSC_VER >= 1200
  HBRUSH hbrushOld = reinterpret_cast<HBRUSH__ *>(SelectObject(
      hdcMem, clrd ? clrd->WinBrush() : GetStockObject(WHITE_BRUSH)));
#else
  HBRUSH hbrushOld = SelectObject(hdcMem, clrd ? clrd->WinBrush()
    ...
    : m_form(XBM_FILE_FORMAT), m_image((0L)), m_ptHotSpot(0, 0)
{
  Am_WINCLEAN_CONSTR(Am_Image_Array_Data)
      ...
      m_nGrayFactor =
          (percent >= 0) ? ((percent < 100) ? percent * 16 / 100 : 16) : -1;

  HBITMAP hbmpNew =
      CreateBitmap(8, 8, 1, 1, (m_nGrayFactor >= 0)
                                   ? (LPBYTE)stipple_bits[16 - m_nGrayFactor]
                                   : (LPBYTE)diamond_bits);
  //CreateCompatibleBitmap(hdc, width, height);
    ...
    : m_form(XBM_FILE_FORMAT), m_image((0L)), m_ptHotSpot(0, 0)
{
  Am_WINCLEAN_CONSTR(Am_Image_Array_Data)
      ...
      HBITMAP hbmpNew = CreateBitmap(height, width, 1, 1, (LPBYTE)bit_data);
  m_image = new Am_WinBMPImage(hbmpNew, height, width);
  m_nGrayFactor = -1;
}

// create by stretching proto to be width X height
Am_Image_Array_Data::Am_Image_Array_Data(Am_Image_Array_Data *proto, int width,
                                         int height)
    : m_form(XBM_FILE_FORMAT), m_image((0L)), m_ptHotSpot(0, 0)
{
  Am_WINCLEAN_CONSTR(Am_Image_Array_Data)
  ...
  if (pw && ph) {
    HDC hdc = GetDC(0);
    m_image =
        new Am_WinBMPImage(hdc, proto->m_image, 0, 0, 0, 0, width, height);
    ReleaseDC(0, hdc);
...
int
Am_Image_Array_Data::Get_Bit(int x, int y)
{
  return 0;
}

void
Am_Image_Array_Data::Set_Bit(int x, int y, int val)
{
}

void
Am_Image_Array_Data::Set_Hot_Spot(int x, int y)
{
  m_ptHotSpot.x = x;
  m_ptHotSpot.y = y;
}
...
void
Am_Image_Array_Data::Get_Hot_Spot(int &x, int &y) const
{
  x = m_ptHotSpot.x;
  y = m_ptHotSpot.y;
}
...
void
Am_Image_Array_Data::Get_Size(int &width, int &height)
{
  if (m_image) {
    width = m_image->width();
...
int Am_Image_Array_Data::Write_To_File(
...
void
Am_Image_Array_Data::WinShow(HDC hdc, short xfrom, short yfrom, short xto,
                             short yto, short width, short height, bool masked,
...
#endif
  if (m_image)
    m_image->show(hdc, xfrom, yfrom, xto, yto, width, height, masked,
                  monochrome, invert, ropcode);
}

HPALETTE
Am_Image_Array_Data::WinPalette()
{
#ifdef USE_WINCLEANER
  WinSetUsedRes();
#endif
...
HBITMAP
Am_Image_Array_Data::WinBitmap(HDC hdc)
{
#ifdef USE_WINCLEANER
  WinSetUsedRes();
#endif
...
BOOL
Am_Image_Array_Data::WinHasRes() const
{
  return (BOOL)(m_image && m_image->get_ddb());
}

BOOL
Am_Image_Array_Data::WinFreeRes()
{
