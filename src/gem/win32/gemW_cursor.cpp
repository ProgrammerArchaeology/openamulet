...
Am_Cursor::Am_Cursor() { data = (0L); }

Am_Cursor::Am_Cursor(Am_Image_Array image, Am_Image_Array mask,
                     Am_Style /*fg_color*/,
                     Am_Style /*bg_color*/) //cannot have color cursors
{
    ...
    : m_img((0L)), m_msk(NULL), m_hcurs(0)
{
  Am_WINCLEAN_CONSTR(Am_Cursor_Data)
      ...
      PREPARE_DATA(Am_Image_Array, imgd, image)
          PREPARE_DATA(Am_Image_Array, mskd, mask) if (!imgd || !mskd ||
                                                       !imgd->Valid() ||
                                                       !mskd->Valid())
  {
    DISCARD_DATA(imgd)
...
void
Am_Cursor_Data::Set_Hot_Spot(int x, int y)
{
  m_ptHotSpot.x = x;
  m_ptHotSpot.y = y;
}
...
void
Am_Cursor_Data::Get_Hot_Spot(int &x, int &y) const
{
  x = m_ptHotSpot.x;
  y = m_ptHotSpot.y;
}
...
void
Am_Cursor_Data::Get_Size(int &width, int &height)
{
  if (m_msk) {
    width = m_msk->width();
    height = m_msk->height();
...
HCURSOR
Am_Cursor_Data::WinCursor(HINSTANCE hinst)
{
  ...
  if (!m_hcurs && m_img && m_msk) {
    ...
    m_hcurs = ::CreateCursor(hinst, m_ptHotSpot.x, m_ptHotSpot.y, cw, ch,
...
BOOL
Am_Cursor_Data::WinHasRes() const
{
  return (BOOL)(m_msk->get_ddb() || m_img->get_ddb());
}

BOOL
Am_Cursor_Data::WinFreeRes()
{
  BOOL bRes = FALSE;
