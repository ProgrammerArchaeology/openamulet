...
Am_WinImageData::~Am_WinImageData()
{
  ...
  if (hbm_ddb)
    DeleteBitmap(hbm_ddb);

  // If a mask exists, destroy it.
  if (hbm_mask)
    DeleteBitmap(hbm_mask);
}
//---------------------------------------------------------------
//  I m a g e : : I m a g e
//  Copy constructor
...
void
Am_WinImage::image_data(const Am_WinImage *img)
{
  img->imdata->count++;
  if (--imdata->count <= 0)
    delete imdata;
...
Am_WinImage::numcolors()
{
  ...
  case 8:
    return 256;
  default:
    return 0; // Must be 24-bit/pixel image
  }
}
//---------------------------------------------------------------
// I m a g e : : bitpixel
...
Am_WinImage::bitpixel()
{
  ...
  case 256:
    return 8;
  default:
    return 24; // Must be 24-bit/pixel image
  }
}
//---------------------------------------------------------------
...
Am_WinImage::make_palette()
{
  ...
  if (imdata->hpal != 0)
    DeletePalette(imdata->hpal);

  // Set up the palette, if needed
  if (numcolors() > 0) {
    LPLOGPALETTE p_pal = (LPLOGPALETTE)GlobalAllocPtr(
        GHND, sizeof(LOGPALETTE) + numcolors() * sizeof(PALETTEENTRY));
    if (p_pal) {
...
BOOL
Am_WinImage::delete_ddb()
{
  // If a DDB exists, destroy it & return true
  if (imdata->hbm_ddb) {
    if (!(LPBITMAPINFOHEADER)(imdata->p_dib))
      DDBtoDIB();
...
Am_WinImage::DIBtoDDB(HDC hdc)
{
  ...
  if (numcolors() > 2)
    ...
    if (memdcn != 0 && memdco != 0) {
      HBITMAP ohbm = SelectBitmap(
          memdco, CreateDIBitmap(hdc, p_bminfo, CBM_INIT, p_image,
                                 (LPBITMAPINFO)p_bminfo, DIB_RGB_COLORS));
      HBITMAP nhbm = SelectBitmap(memdcn, imdata->hbm_ddb);
      BitBlt(memdcn, 0, 0, width(), height(), memdco, 0, 0,
             SRCCOPY /*NOTSRCCOPY*/);
...
Am_WinImage::DDBtoDIB()
{
  ...
  imdata->bytes_per_line =
      (unsigned short)(((long)bh.biWidth * (long)bh.biBitCount + 31L) / 32 * 4);
  ...
  unsigned long dibsize =
      sizeof(BITMAPINFOHEADER) + ncolors * sizeof(RGBQUAD) + bh.biSizeImage;
  ...
  LPSTR p_image =
      (LPSTR)imdata->p_dib + (WORD)bh.biSize + ncolors * sizeof(RGBQUAD);
...
void
Am_WinImage::show(HDC hdc, short xfrom, short yfrom, short xto, short yto,
                  short wdth, short hght, BOOL masked, BOOL monochrome,
                  BOOL invert, DWORD ropcode)
{
  // Set up a pointer to the DIB
  LPBITMAPINFOHEADER p_bminfo = (LPBITMAPINFOHEADER)(imdata->p_dib);
  if (p_bminfo != (0L)) {
                         ...
                         short newh)
{
  ...
  if (imdata->hbm_ddb != 0) {
    ...
    if (memdcn != 0 && memdco != 0) {
      HBITMAP ohbm = SelectBitmap(memdco, img->GetDDB(hdc));
      HBITMAP nhbm = SelectBitmap(memdcn, imdata->hbm_ddb);
      StretchBlt(memdcn, 0, 0, nw, nh, memdco, (neww >= 0) ? x : w - 1,
                 (newh >= 0) ? y : h - 1, (neww >= 0) ? w : -(int)w,
                 (newh >= 0) ? h : -(int)h, SRCCOPY);
      SelectBitmap(memdco, ohbm);
      SelectBitmap(memdcn, nhbm);
      DeleteDC(memdco);
      DeleteDC(memdcn);
...
int
Am_WinImage::write_dib(std::ofstream &ofs)
{
  ...
  bfhdr.bfOffBits = sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER) +
                    numcolors() * sizeof(RGBQUAD);
  bfhdr.bfSize =
      (long)height() * (long)imdata->bytes_per_line + bfhdr.bfOffBits;
  ...
  if (imdata->hbm_ddb != 0) {
    ...
    if (srcdc != 0 && maskdc != 0) {
      ...
      if (is_transparent() && (numcolors() > 0)) {
        ...
        COLORREF oldBkColor = SetBkColor(
            srcdc, RGB(color0.rgbRed, color0.rgbGreen, color0.rgbBlue));
      }
