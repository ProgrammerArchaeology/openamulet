...
void
Am_GIF_Image::add_to_image(GIF_Load_Info &gli, unsigned short pixval)
{
  // force index of transparent color to be 0
  unsigned char pixchar = (unsigned char)pixval;
  if (iminfo.raw_transparent_color_index != 0) {
    if (pixval == 0)
      pixchar = (unsigned char)iminfo.raw_transparent_color_index;
    else if (pixval == iminfo.raw_transparent_color_index)
      pixchar = 0;
  }
