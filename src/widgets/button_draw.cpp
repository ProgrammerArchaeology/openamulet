...
void
Am_Draw_Button_Widget(int left, int top, int width, int height, // dimensions
                      const char *string, Am_Object obj, bool interim_selected,
                      bool selected, bool active, bool key_selected,
                      bool is_default, bool fringe, Am_Font font,
                      const Computed_Colors_Record &color_rec,
                      Am_Widget_Look look, Am_Button_Type type,
                      Am_Drawonable *draw, int box_width, int box_height,
                      bool box_on_left, Am_Alignment align, int offset_left,
                      bool mask)
{
  ...
  case Am_PUSH_BUTTON: {
    ...
    case Am_MOTIF_LOOK_val:
      if (key_selected)
        draw->Draw_Rectangle(Am_Key_Border_Line, Am_No_Style, r.left, r.top,
                             r.width, r.height);
      if (fringe || key_selected)
        Inset_Rect(r, kMotKeySel);

      if (is_default)
        Am_Draw_Motif_Box(r.left, r.top, r.width, r.height, true, color_rec,
                          draw);
    ...
    case Am_WINDOWS_LOOK_val:
      if (is_default && !depressed) {
        draw->Draw_Rectangle(Am_Style("black", 1), Am_No_Style, r.left, r.top,
                             r.width, r.height);
        Inset_Rect(r, 1);
      }
      break;
    ...
    case Am_MACINTOSH_LOOK_val: {
      ...
      if (is_default)
        draw->Draw_Roundtangle(active ? black : ltgray, Am_No_Style, r.left,
                               r.top, r.width, r.height, 8, 8);
      if (fringe || is_default)
    ...
    int inset = Am_Draw_Push_Button_Box(r, draw, look, depressed, key_selected,
                                        color_rec);
    Inset_Rect(r, inset);
    break;
  }
  ...
  case Am_RADIO_BUTTON:
    ...
    break;
  default:
    Am_Error("Switch statement is not complete");
    break;
  }
  ...
  if (string) // draw the string if it exists
  {
    ...
    Am_Style text_style =
        compute_text_style(active, depressed, color_rec, look, type);
    ...
    draw->Draw_Text(text_style, string, strlen(string), font, str_left, str_top,
  ...
  if (key_selected && look == Am_WINDOWS_LOOK) {
    ...
    if (type == Am_PUSH_BUTTON) {
      ...
      draw->Draw_Rectangle(dash, Am_No_Style, r.left, r.top, r.width, r.height);
      Inset_Rect(r, 1);
...
int
Am_Draw_Push_Button_Box(am_rect r, // coords in terms of draw
                        Am_Drawonable *draw, Am_Widget_Look look, bool selected,
                        bool /* key_selected */,
                        const Computed_Colors_Record &color_rec)
{
  draw->Push_Clip(r.left, r.top, r.width, r.height);
  ...
  case Am_MOTIF_LOOK_val: // just use the every useful Am_Draw_Motif_Bo_valx
    Am_Draw_Motif_Box(r.left, r.top, r.width, r.height, selected, color_rec,
                      draw);
    inset = kMotBox;
    break;

  case Am_WINDOWS_LOOK_val: {
    Am_Draw_Windows_Box(r.left, r.top, r.width, r.height, selected, color_rec,
                        draw);
...
void
Am_Draw_Radio_Button_Box(am_rect r, // coords in terms of draw
                         Am_Drawonable *draw, Am_Widget_Look look,
                         bool selected, bool interim_selected,
                         const Computed_Colors_Record &color_rec,
                         bool mask = false)
{
  ...
  case Am_MOTIF_LOOK_val: {
    ...
    // top edges
    draw->Draw_2_Lines(top_fill, Am_No_Style, r.left, center_y, center_x, r.top,
                       right, center_y,
                       mask ? Am_DRAW_MASK_COPY : Am_DRAW_COPY);
    draw->Draw_2_Lines(top_fill, inside_fill, r.left + 1, center_y, center_x,
                       r.top + 1, right - 1, center_y,
                       mask ? Am_DRAW_MASK_COPY : Am_DRAW_COPY);

    // bottom edges
    draw->Draw_2_Lines(bot_fill, Am_No_Style, r.left, center_y, center_x,
                       bottom, right, center_y,
                       mask ? Am_DRAW_MASK_COPY : Am_DRAW_COPY);
    draw->Draw_2_Lines(bot_fill, inside_fill, r.left + 1, center_y, center_x,
                       bottom - 1, right - 1, center_y,
                       mask ? Am_DRAW_MASK_COPY : Am_DRAW_COPY);
    break;
  }
  ...
  case Am_WINDOWS_LOOK_val: {
    // draw outer arcs
    draw->Draw_Arc(color_rec.data->background_style, Am_No_Style, r.left, r.top,
                   r.width, r.height, 45, 180,
                   mask ? Am_DRAW_MASK_COPY : Am_DRAW_COPY);
    draw->Draw_Arc(color_rec.data->highlight_style, Am_No_Style, r.left, r.top,
                   r.width, r.height, 225, 180,
                   mask ? Am_DRAW_MASK_COPY : Am_DRAW_COPY);
    Inset_Rect(r, 1);
    ...
    Am_Style fill_style =
        interim_selected ? color_rec.data->foreground_style : Am_White;
    draw->Draw_Arc(color_rec.data->shadow_style, fill_style, r.left, r.top,
                   r.width, r.height, 45, 180,
                   mask ? Am_DRAW_MASK_COPY : Am_DRAW_COPY);
    draw->Draw_Arc(color_rec.data->foreground_style, fill_style, r.left, r.top,
                   r.width, r.height, 225, 180,
                   mask ? Am_DRAW_MASK_COPY : Am_DRAW_COPY);
    ...
    if (selected) {
      ...
      draw->Draw_Arc(Am_No_Style, mask ? Am_On_Bits : Am_Black, r.left, r.top,
                     r.width, r.height);
    }
    break;
  }
  ...
  case Am_MACINTOSH_LOOK_val: {
    ...
    draw->Draw_Arc(line_style, Am_White, r.left, r.top, r.width, r.height, 0,
                   360, mask ? Am_DRAW_MASK_COPY : Am_DRAW_COPY);
