...
Am_Value_List
Am_Copy_Object_List(Am_Value_List orig, const Am_Object &ref_obj, int offset)
{
...
Am_Sort_Obs_In_Group(Am_Value_List unsorted_sel_objs, Am_Object group)
{
  ...
  if (!unsorted_sel_objs.Empty()) {
    Am_ERRORO("Selected parts " << unsorted_sel_objs << " not in group "
                                << group,
              group, Am_GRAPHICAL_PARTS);
  }
...
void
get_all_sub_parts(Am_Value_List &objs)
{
  Am_Object obj;
  Am_Value is_group;
...
void
update_objs_from_val(Am_Object cmd, Am_Value objs_value,
...
void
change_property_general_undo_redo(Am_Object command_obj, bool undo,
                                  ...
                                  Am_Value new_value = Am_No_Value)
{
  ...
  if (new_sel.Valid()) { //then have something to operate on
    ...
    if (selective) {
      ...
      old_data_value = command_obj.Peek(Am_VALUE);
    if (undo)
      new_data_value = command_obj.Peek(Am_OLD_VALUE);
...
Am_Define_Method(Am_Object_Method, void, property_undo, (Am_Object command_obj))
{
                 ...
                 (Am_Object command_obj, Am_Value &new_value))
{
  ...
  if (val.type == Am_OBJECT) {
    ...
    Am_Error("Widget's value is not an object");
}
...
void
copy_objects_into_clipboard(Am_Object cmd, Am_Value_List selected_objs)
{
  Am_Object clipboard;
...
void
delete_objects_internal(Am_Object &cmd, Am_Value new_sel)
{
...
void
Am_Put_Objects_Into_Group(Am_Object group, Am_Value_List objects)
{
  Am_INTER_TRACE_PRINT(Am_INTER_TRACE_SETTING,
                       "++ Adding parts " << objects << " to " << group);
  Am_Object object;
...
void
restore_old_clipboard_objs(Am_Object command_obj)
{
  Am_Object clipboard;
...
void
delete_general_undo_redo(Am_Object command_obj, bool selective,
                         bool undo_clipboard)
{
  ...
  if (currently_undone) { // then redo = re-delete the
    ...
    if (!selective) {
      ...
      Am_Put_Objects_Into_Group(group, deleted_objs);
    ...
    if (old_sel_val.Valid())
      old_selection = old_sel_val;
    else
      old_selection = deleted_objs;
...
Am_Define_Formula(bool, active_if_any)
{
  ...
  if (selection_widget.Valid()) {
    Am_Value_List all_parts =
        selection_widget.Get_Object(Am_OPERATES_ON).Get(Am_GRAPHICAL_PARTS);
...
void
paste_duplicate_undo_branch(Am_Object command_obj, bool selective)
{
  Am_Object selection_widget, group;
...
Am_Define_Method(Am_Selective_New_Allowed_Method, bool, new_sel_valid,
...
void
adjust_objects_positions(Am_Value_List objs, int left_offset, int top_offset)
{
  Am_Object obj;
  for (objs.Start(); !objs.Last(); objs.Next()) {
    obj = objs.Get();
...
void
remove_and_add_objects(Am_Value_List objs, Am_Object old_group,
                       Am_Object new_group, Am_Object place = Am_No_Object)
{
  Am_INTER_TRACE_PRINT(Am_INTER_TRACE_SETTING,
                       "++ Removing parts " << objs << " from " << old_group
                                            << " and adding them to "
                                            << new_group);
  Am_Object object;
  for (objs.Start(); !objs.Last(); objs.Next()) {
...
void
group_new_objs(Am_Object &cmd, Am_Value new_sel)
{
  Am_Object selection_widget, group, obj;
  Am_Value_List selected_objs;
  selection_widget = Am_Get_Selection_Widget_For_Command(cmd);
...
void
group_general_undo_redo(Am_Object command_obj, bool selective)
{
  ...
  if (currently_undone) { // then redo = re-delete the
    ...
    if (!selective) {
      ...
      Am_Put_Objects_Into_Group(group, grouped_objs);
...
void
do_to_top_or_bottom(Am_Object cmd, bool to_top, Am_Value new_sel = Am_No_Value)
{
  Am_Object selection_widget, group, obj, place;
  Am_Value_List selected_objs, selected_objs_places;
  selection_widget = Am_Get_Selection_Widget_For_Command(cmd);
...
void
set_default_filename(Am_Object &command_obj, Am_String &filename)
{
  Am_Object sel_widget = Am_Get_Selection_Widget_For_Command(command_obj);
  if (sel_widget.Valid())
    sel_widget.Set(Am_DEFAULT_LOAD_SAVE_FILENAME, filename, Am_OK_IF_NOT_THERE);
}
...
void
internal_do_save(Am_Object command_obj, Am_String &filename)
{
  ...
  if (!out_file.good()) {
    Am_Show_Alert_Dialog(
        Am_Value_List().Add("Cannot open C++ file for writing:").Add(filename));
    return;
  }
...
init()
{
  ...
  Am_Graphics_Set_Property_Command =
      Am_Command.Create(DSTR("Set_Property_Command"))
          ...
          .Add(Am_SLOT_FOR_THIS_COMMAND_INACTIVE,
               (int)Am_SET_PROPERTY_INACTIVE);

  Am_Graphics_Clear_Command =
      Am_Command.Create(DSTR("Delete_Objects_Command"))
          .Set(Am_LABEL, "Clear")
  ...
  Am_Graphics_Cut_Command =
      Am_Command.Create(DSTR("Cut_Objects_Command"))
          .Set(Am_LABEL, "Cut")
          .Set(Am_ACTIVE, Am_Active_If_Selection)
  ...
  Am_Graphics_Copy_Command =
      Am_Command.Create(DSTR("Copy_Objects_Command"))
          .Set(Am_LABEL, "Copy")
          .Set(Am_ACTIVE, Am_Active_If_Selection)
  ...
  Am_Graphics_Ungroup_Command =
      Am_Command.Create(DSTR("Ungroup_Objects_Command"))
          ...
          .Set(Am_SELECTIVE_REPEAT_SAME_ALLOWED,
               ungroup_selective_repeat_allowed)
  ...
  Am_Undo_Command =
      Am_Command.Create(DSTR("Undo_Command"))
          .Set(Am_LABEL, "Undo")
  ...
  Am_Redo_Command =
      Am_Command.Create(DSTR("Redo_Command"))
          .Set(Am_LABEL, "Redo")
  ...
  Am_Graphics_To_Top_Command =
      Am_Command.Create(DSTR("To_Top_Objects_Command"))
          .Set(Am_LABEL, "To Top")
          .Set(Am_ACTIVE, Am_Active_If_Selection)
  ...
  Am_Quit_No_Ask_Command = Am_Command.Create(DSTR("Quit_No_Ask_Command"))
      ...
      ;
  Am_Cycle_Value_Command =
      Am_Command.Create(DSTR("Cycle_Value_Command"))
          .Set(Am_VALUE,
               0) //access this slot to get the index of the current label
  ...
  Am_Open_Command =
      Am_Command.Create(DSTR("Open_Command"))
          .Set(Am_LABEL, "Open...")
          .Set(Am_ACTIVE, true)
          ...
          .Set(Am_SELECTIVE_REPEAT_ON_NEW_METHOD, (0L));
  Am_Save_Command =
      Am_Command.Create(DSTR("Save_Command"))
          .Set(Am_LABEL, "Save")
          .Set(Am_ACTIVE, true)
          ...
          .Set(Am_SELECTIVE_REPEAT_ON_NEW_METHOD, (0L));
  Am_Save_As_Command =
      Am_Command.Create(DSTR("Save_As_Command"))
