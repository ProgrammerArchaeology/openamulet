...
bool
inter_value_is_or_contains(Am_Object inter, Am_Object new_object)
{
  ...
  if (Am_Value_List::Test(inter_value)) { // is a list
    ...
    inter_list.Start();
    if (inter_list.Member(new_object))
      return true;
    else
      return false;
  } else { //not list
...
void
set_panel_value_from_inter_value(Am_Object inter)
{
  ...
  Am_INTER_TRACE_PRINT(Am_INTER_TRACE_SETTING,
                       "++ Panel DO method setting the Am_VALUE of "
                           << panel << " to " << value);
