...
void
create_sub_menu_window(Am_Object menu_item)
{
...
void
set_sub_window_vis(Am_Object &sub_window, bool vis, Am_Object &main_win)
{
  set_popup_win_visible(sub_window, vis, main_win);
  Am_Object for_item = sub_window.Get(Am_FOR_ITEM);
  if (for_item.Valid())
    for_item.Set(Am_INTERIM_SELECTED, vis);
}
...
Am_Object
clear_interim_sel(Am_Object obj, Am_Object menu_bar_win)
{
...
Am_Define_Method(Am_Object_Method, void, menu_bar_inter_do, (Am_Object inter))
{
...
static Am_Initializer *initializer =
    new Am_Initializer(DSTR("Am_Menu_Bar"), init, 5.208f, 114, cleanup);
