...
void
set_commands_for_sel(Am_Object &cmd, Am_Object &inter, Am_Object &widget,
                     Am_Value clicked_obj, Am_Value new_value,
                     Am_Value old_value, bool set_extras,
                     ...
                     bool extend = false)
{
  ...
  if (cmd.Valid()) {
    Am_Object impl_parent =
        cmd.Get(Am_IMPLEMENTATION_PARENT, Am_RETURN_ZERO_ON_ERROR);
    if (impl_parent.Valid() && impl_parent != widget_command &&
        impl_parent != inter_command) {
      set_commands_for_sel_values(cmd, clicked_obj, new_value, old_value,
                                  set_extras, new_label, dir, extend);
      set_commands_for_sel_values(impl_parent, clicked_obj, new_value,
                                  old_value, set_extras, new_label, dir,
                                  extend);
    }
  }
}
...
void
clear_multi_selections(const Am_Object &widget)
{
  Am_Value value;
...
Am_Define_Method(Am_Where_Method, Am_Object, Am_Default_Closest_Select_Point,
                 (Am_Object widget, Am_Object object, Am_Object ref_obj, int x,
                  int y))
{
  ...
  if (as_line) {
    ...
    long d1 = (long(x1 - x) * long(x1 - x)) + (long(y1 - y) * long(y1 - y));
    long d2 = (long(x2 - x) * long(x2 - x)) + (long(y2 - y) * long(y2 - y));
    if (d1 < d2)
      which_point = Am_ATTACH_END_1;
    else
      which_point = Am_ATTACH_END_2;
  } else {
    ...
    if (y > top + h3 + h3) { //bottom third
      ...
      case Am_ATTACH_W_val:
        which_point = Am_ATTACH_SW;
        break;
      case Am_ATTACH_E_val:
        which_point = Am_ATTACH_SE;
        break;
      case Am_ATTACH_CENTER_val:
        which_point = Am_ATTACH_S;
        break;
      default:
        Am_ERRORO("Bad Am_WHERE_ATTACH " << Am_Move_Grow_Where_Attach(xcontrol)
      ...
      switch (xcontrol) {
      case Am_ATTACH_W_val:
        which_point = Am_ATTACH_NW;
        break;
      case Am_ATTACH_E_val:
        which_point = Am_ATTACH_NE;
        break;
      case Am_ATTACH_CENTER_val:
        which_point = Am_ATTACH_N;
        break;
      default:
        Am_ERRORO("Bad Am_WHERE_ATTACH " << Am_Move_Grow_Where_Attach(xcontrol)
                                         << " for selection widget " << widget,
      ...
      switch (xcontrol) {
      case Am_ATTACH_W_val:
        which_point = Am_ATTACH_W;
        break;
      case Am_ATTACH_E_val:
        which_point = Am_ATTACH_E;
        break;
...
Am_Which_Sel_Direction
check_sel_move_direction(Am_Object &inter, Am_Input_Char ic)
{
  ...
  if (all_chars.Valid()) {
    ...
    if (test_ic == ic)
      return Am_Sel_FORWARD;
    else {
      all_chars.Next();
      test_ic = all_chars.Get();
      if (test_ic == ic)
        return Am_SEL_BACKWARD;
      else {
        all_chars.Next();
        test_ic = all_chars.Get();
        if (test_ic == ic)
...
Am_Value_List
toward_end(Am_Value_List &parts, Am_Value_List &old_value, bool to_end,
           bool extend)
{
  Am_Value_List new_value;
  Am_Object object;
  ...
  if (old_value.Valid()) {
    ...
    if (index == parts.Length() - 1) {
      if (to_end) {
        if (extend)
          return old_value; //already have end in list
        else
          return new_value.Add(object); //object is left assigned to last
      } else
        return Am_No_Value_List; //already at end, cannot go further
    }
    parts.Move_Nth(index + 1);
    if (extend) {
      new_value = old_value;
...
get_owner_start_and_convert_string(Am_Object &widget)
{
  Am_Value value;
  ...
  if (value.type == Am_STRING) {
    ...
    for (list.Start(); !list.Last(); list.Next()) {
      ...
      if (value.type == Am_STRING) {
        ...
        Am_Error("Am_START_WHEN slot has wrong type");
...
Am_Define_Formula(Am_Value, compute_drag_start_when)
{
  ...
  if (widget.Valid()) {
    ...
    return Am_Input_Char("LEFT_DRAG");
}
...
static void
set_commands_for_move(const Am_Object &widget, const Am_Object &inter,
                      const Am_Value &object_modified,
                      const Am_Value &old_value, const Am_Value &new_value,
                      int final_left, int final_top,
                      const Am_Object &dest_widget, Am_Object &final_owner)
{
  Am_Object inter_command, widget_command;
  inter_command = inter.Get_Object(Am_COMMAND);
  if (inter_command.Valid()) {
...
static void
adjust_all_objects_position(const Am_Object &widget, const Am_Object &inter,
                            Am_Value_List &list, int left_offset,
                            int top_offset, int final_left, int final_top,
                            Am_Object &new_owner, const Am_Object &dest_widget)
{
  ...
  for (list.Start(); !list.Last(); list.Next()) {
    ...
    new_values.Add(
        Am_Inter_Location(false, new_owner, new_left, new_top, w, h));
    Am_INTER_TRACE_PRINT(widget, "Selection handle widget "
                                     << widget << " moving  " << obj << " to "
                                     << new_left << "," << new_top);
...
{
  Am_Object ret =
      owner_start_where_operates_on_proc(inter, object, event_window, x, y);
...
//returns true if OK
bool
check_all_objects_moveable(Am_Object &inter)
{
  Am_Object widget = inter.Get_Owner();
...
static void
complete_drop(const Am_Object &inter, Am_Object &target)
{
                  ...
                  Am_Inter_Location data))
{
  ...
  if (list.Length() > 1) {
    ...
    set_commands_for_move(widget, inter, Am_Value(object_modified),
                          inter.Get(Am_OLD_VALUE), Am_Value(data), final_left,
                          final_top, dest_widget, final_owner);
  }
...
Am_Define_Method(Am_Where_Method, Am_Object, Am_In_Target,
...
static void
remove_object_from_selection(const Am_Object &object, const Am_Object &command)
{
...
void
update_objs_from_value(Am_Value objs_value, Am_Value new_data_value,
...
void
adjust_all_objects_position_for_undo(Am_Value_List list, int left_offset,
                                     int top_offset, Am_Value_List &new_list)
{
  Am_Object obj;
  Am_Object owner;
  new_list.Make_Empty();
...
void
get_left_and_top_from_new_value(Am_Value &new_value, Am_Object ref_obj,
  ...
  if (Am_Inter_Location::Test(new_value)) {
    ...
    for (list.Start(); !list.Last(); list.Next()) {
      ...
      if (!Am_Inter_Location::Test(item)) {
        Am_ERROR("New value for selective redo is a list containing an "
                 "non-Am_Inter_Location: "
                 << new_value);
      } else {
        loc = item;
        get_min_from_loc(ref_obj, loc, min_left, min_top);
    ...
    Am_ERROR("New value for selective redo not list or Am_Inter_Location: "
             << new_value);
  std::cout << "calculated min " << min_left << "," << min_top << std::endl
            << std::flush;
}
...
Am_Define_Method(Am_Object_Method, void, sel_move_undo, (Am_Object command_obj))
{
...
bool
check_in_line_handles(Am_Object sel_handles, Am_Object for_obj, int x, int y)
{
  int x1 = for_obj.Get(Am_X1);
  int y1 = for_obj.Get(Am_Y1);
  int x2 = for_obj.Get(Am_X2);
...
Am_Define_Method(Am_Where_Method, Am_Object, selection_handles_where_function,
                 (Am_Object /*inter*/, Am_Object object, Am_Object event_window,
                  int x, int y))
{
                  ...
                  Am_Value /* new_value */))
{
  if (!new_selection.Valid())
    return true; //ok if no new object
  if (new_selection.type == Am_OBJECT)
    return true;
  if (Am_Value_List::Test(new_selection) &&
      Am_Value_List(new_selection).Length() == 1)
    return true;
  return false;
}
...
Am_Define_String_Formula(move_or_grow_label)
{
  ...
  if (growing)
    return Am_String("Grow");
  else
    return Am_String("Move");
}
...
Am_Define_Value_List_Formula(am_operates_list)
{
  ...
  for (selection_list.Start(); !selection_list.Last(); selection_list.Next()) {
...
init()
{
  ...
  Am_Selection_Widget =
      Am_Group
          .Create(DSTR("Am_Selection_Widget"))
          //parameters
          .Add(Am_START_WHEN, Am_Input_Char("ANY_LEFT_DOWN"))
          .Add(Am_FILL_STYLE, Am_Black)
          .Add(Am_VALUE,
               Am_Value_List()) // set by the interactors or externally
          .Add(Am_ACTIVE, true) //
          .Add(Am_OPERATES_ON,
               (0L)) //fill in with group holding parts to select
          ...
          .Add(Am_GRID_X, 0)
          .Add(Am_GRID_Y, 0)
          .Add(Am_SELECT_CLOSEST_POINT_STYLE,
               (0L)) //if one point should be diff
          ...
          .Add(Am_LINE_FEEDBACK_OBJECT,
               Am_Line.Create(DSTR("Sel_Line_Feedback"))
                   .Add(Am_OPERATES_ON, Am_No_Object)
                   .Set(Am_VISIBLE, false)
                   .Set(Am_LINE_STYLE, Am_Dashed_Line))
          .Add(Am_RECT_FEEDBACK_OBJECT,
               Am_Rectangle.Create(DSTR("Sel_Rect_Feedback"))
                   .Add(Am_OPERATES_ON, Am_No_Object)
                   .Set(Am_VISIBLE, false)
                   .Set(Am_LINE_STYLE, Am_Dashed_Line)
          ...
          .Add(Am_FAKE_GROUP,
               Am_Rectangle.Create(DSTR("Fake_group")).Set(Am_VISIBLE, false))
          .Add_Part(
              Am_FEEDBACK_OBJECT,
              Am_Map.Create(DSTR("Selection_Handles"))
                  .Set_Part(Am_ITEM_PROTOTYPE, Am_One_Selection_Handle.Create(
                                                   DSTR("Sel_Handle_Proto")))
  ...
  Am_Selection_Widget.Add(Am_USER_ID,
                          Am_ONE_AT_A_TIME); //supports multiple users

  //Visual C++ can't handle really long statements
  Am_Selection_Widget.Add_Part(
      Am_INTERACTOR,
      inter = Am_One_Shot_Interactor.Create(DSTR("inter_in_sel_widget"))
                  ...
                  .Set(Am_USER_ID,
                       Am_From_Owner(Am_USER_ID)) // supports multiple users
      );

  //Visual C++ can't handle really long statements
  Am_Selection_Widget.Add_Part(
      Am_MOVE_INTERACTOR,
      inter2 = Am_Move_Grow_Interactor.Create(DSTR("move_inter_in_handle")));
  ...
  Am_Selection_Widget
      .Add_Part(
          Am_GROW_INTERACTOR,
          inter3 =
              Am_Move_Grow_Interactor.Create(DSTR("grow_inter_in_handle"))
      ...
      .Add_Part(Am_COMMAND,
                Am_Select_Command.Create(DSTR("Selection_Command"))
                    ...
                    .Set(Am_LABEL, "Select"))
      .Add_Part(Am_MOVE_GROW_COMMAND,
                Am_Command.Create(DSTR("Move_Grow_Command"))
