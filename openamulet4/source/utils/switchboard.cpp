...
//		$Id: switchboard.cpp,v 4.1.4.1 2005/11/09 18:19:27 mitosys Exp $
//
// See Also:
//		Start text here
//
...
//-------------------------------------------------------------
Switch_Board& Switch_Board::instance()
{
...
// Result			: 
//-------------------------------------------------------------
void Switch_Board::_subscribe(Callback *aCallbackObject)
{
	// insert the new Callback object into the map
	mCallbacks.insert(std::pair<const std::string,Callback*>(aCallbackObject->getSubscription(), aCallbackObject));
...
// Result			: 
//-------------------------------------------------------------
void Switch_Board::_unsubscribe(Callback *aCallbackObject)
{
	// is this a valid pointer?
	if(mCallbacks.empty() == false)
	{
		// find the callback-object in the map
		std::multimap<const std::string, Callback*>::iterator foundObject = mCallbacks.find(aCallbackObject->getSubscription());
...
//-------------------------------------------------------------
bool Switch_Board::_post(const char *aSubscription)
{
	...
	if(mCallbacks.empty() == false)
	{
		...
		std::pair<std::multimap<const std::string, Callback*>::iterator, std::multimap<const std::string, Callback*>::iterator> iteratorPair = mCallbacks.equal_range(aSubscription);

		for(std::multimap<const std::string, Callback*>::iterator it = iteratorPair.first; it != iteratorPair.second; ++it)
		{
