...
Am_Define_Pointer_Wrapper(my_int)
Am_Define_Pointer_Wrapper(float)
...
public:
  ...
  { my_int* m = Am_my_int(val).value;
    os << "PINT(" << std::hex << (Am_Ptr)val << std::dec << ")=" << m->my_int_val; }
...
void test_int (float id, int result, int expected)
{
  ++test_counter;
  ...
  if (result == expected) {
    ...
    cout << id << "       expected " << expected << ", result is " << result
	 << endl << "*** TEST DID NOT SUCCEED. ***" << endl;
    ++failed_tests;
  }
}

void test_flt (float id, float result, float expected)
{
  ++test_counter;
  if (suppress_successful) {
    if ((test_counter % 50) == 0)
...
void test_slots (float id, Am_Object obj, Am_Slot_Key keys[], int size)
{
...
void test_pts (float id, Am_Object obj, Am_Object names[], int size)
{
  Am_Object result_names[10];
  int num_results = 0;
  int i, j;
...
Am_Value gvCleft_proc (Am_Object&)
{
...
Am_Define_Formula (int, top)
{
  return self.Get (TOP);
}

Am_Define_Formula (int, top_plus_10)
{
  return 10 + (int)top (/*cc,*/ self);
}

int getLeft (Am_Object)
{
  return accessed.Get (LEFT, Am_NO_DEPENDENCY);
}

int gv1left (Am_Object)
{
  return accessed1.Get (/*cc,*/ LEFT);
}

Am_Define_Formula (int, g64)
{
  return (int)self.Get (WIDTH) + 3;
}

Am_Define_Formula (int, g90)
{
  return (int)self.Get (WIDTH) + 10;
}

Am_Define_Formula (int, g91)
{
  return (int)self.Get (HEIGHT) + 5;
}
...
Am_Define_Formula (int, ownerleft)
{
  return self.Get_Owner ().Get (LEFT);
}

Am_Define_Formula (int, ownertop)
{
  return self.Get_Owner ().Get (TOP);
}
...
Am_Define_Formula (int, parttop)
{
  ...
  if (v.type == Am_OBJECT) {
    ...
    cout << " parttop for " << self << " found shadow obj " << part
	 << " its TOP is " << i << endl<< flush;
    return i;
  }
  else {
    cout << " parttop for " << self << " found shadow val " << v
	 << endl << flush;
    return v;
  }
}
...
Am_Define_No_Self_Formula(Am_Ptr, retpint)
{
  return (Am_Ptr)&global_int;
}

Am_Define_No_Self_Formula(float, retflt)
{
  return 7.8;
}

Am_Define_No_Self_Formula (double, retdbl)
{
  return 1004.123L;
}
...
class Store_Data : public Am_Wrapper {
 ...
 public:
  ...
  Store_Data (Store_Data* proto)
  {
    datum = proto->datum;
  }
  bool operator== (Store_Data& test)
  { return datum == test.datum; }

  void Print(ostream& out) const {
    out << "STORE Print=" << datum;
  }
...
{
  Store_Data* store = (Store_Data*)Am_Object_Advanced::Get_Context()->Get_Data ();
  data_test = store->datum;
  return "hello";
}

Am_Define_No_Self_Formula(Am_Method_Wrapper*, retproc)
{
  return objproc0;
}

Am_Define_Formula (Am_Wrapper*, retobj)
{
  return self;
}
...
void change_owner (Am_Object, Am_Object, Am_Object new_owner)
{
  change_test = new_owner;
}

const char* integer_type_check (const Am_Value& value)
{
...
class Sub_Object : public Am_Object {
 public:
  Sub_Object () { data = (0L); }
  Sub_Object (Am_Wrapper* in_data)
  {
    data = (Am_Object_Data*)in_data;
  }
...
void create_object (Am_Object self)
{
...
{
  data = new Am_Foo_Data (store);
}

void Am_Foo::Set (int store)
{
...
bool test_web_create (const Am_Slot& slot)
{
  return slot.Get_Key () == LEFT;
}

void test_web_init (const Am_Slot& slot,
		    Am_Web_Init& init)
{
...
void test_web_validate (Am_Web_Events& events)
{
  Am_Object self;
  Am_Slot slot;
  events.End ();
...
bool bug_web_create (const Am_Slot&)
{
  return false;
}

void bug_web_init (const Am_Slot& slot,
		   Am_Web_Init& init)
{
  Am_Object_Advanced obj = slot.Get_Owner ();
...
static void bug_web_validate (Am_Web_Events& events)
{
  events.Start ();
...
void bug_demon (Am_Slot first_invalidated)
{
...
////  a->ps ();
  ...
  vlassoc.Add(Am_Assoc(1, "10")).Add(Am_Assoc(2, "20"))
    .Add(Am_Assoc("backwards-3", 3)) .Add(Am_Assoc("a", a));
...
//  test_int (210, strcmp (a.Get_Name (), "Name"), 0);
  ...
  a = Am_Root_Object.Create ("A")
    ...
    .Add_Part (RIGHT, b = Am_Root_Object.Create ("B")
		.Add (LEFT, ownerleft)
		.Add (HEIGHT, 20))
    .Add_Part (TOP, e = Am_Root_Object.Create ("E")
		.Add (HEIGHT, 30))
    .Add_Part (WIDTH, d = Am_Root_Object.Create ("D")
		.Add (HEIGHT, siblingheight))
  ...
  a = Am_Root_Object.Create ("")
    .Add (LEFT, 10)
    .Add (WIDTH, gvLeft.Multi_Constraint());
  b = a.Create ("")
    .Add (TOP, 50)
    .Add (HEIGHT, 100)
  ...
  a = Am_Root_Object.Create ("proto")
    .Add (LEFT, 10)
    .Add (WIDTH, 50)
    .Add_Part (SHADOW, d = Am_Root_Object.Create ("")
		.Add (LEFT, ownerleft));
  b = a.Create ("A name")
    .Set (WIDTH, 60)
    .Add_Part (e = Am_Root_Object.Create ("e"));
  c = b.Copy ();
#ifdef DEBUG
  ...
  b = Am_Root_Object.Create ("a")
    .Add (LEFT, 10)
    .Add (TOP, 15)
    .Add_Part (SHADOW, Am_Root_Object.Create ("shadow")
      .Add (WIDTH, ownerleft.Multi_Constraint()));
  a = b.Create ("b")
    .Set (LEFT, 0)
    .Set (TOP, 5);
  ...
  a = Am_Root_Object.Create ("a")
    .Add (LEFT, 0)
    .Add (TOP, gvLeft);
  ((Am_Object_Advanced&)a).Get_Demons ()
    .Set_Type_Check (1, integer_type_check);
  ...
  a = Am_Root_Object.Create ("a")
    .Add_Part (TOP, Sub_Object::Make ("sub object"));
  test_int (340, Sub_Object::Narrow (a.Get_Object (TOP)).My_Other_Method (),
	    42);

  a = Am_Root_Object.Create ("a")
    .Add (LEFT, 15)
    .Add_Part (TOP, Am_Root_Object.Create ("top")
      .Add (LEFT, ownerleft))
    .Add_Part (SHADOW, Am_Root_Object.Create ("shadow")
      .Add (LEFT, ownerleft)
      .Add_Part (SHADOW, Am_Root_Object.Create ("shadow")
  ...
  cout << "Instanced name of \"top\" is \""
       << Am_Get_Name_Of_Item(part) << "\"" << endl;
  part->Release ();
#endif
  ...
  a.Add_Part (SHADOW, Am_Root_Object.Create ("shadow")
	      .Add (TOP, 15));
  cout << "should be part" << endl;
  test_int (412, a.Get (TOP), 15);
  cout << "changing part" << endl;
  a.Set_Part (SHADOW, Am_Root_Object.Create ("shadow")
    .Add (TOP, 20));
  test_int (413, a.Get (TOP), 20);
  cout << "instance b" << endl;
  b = a.Create ("b")
    .Set_Part (SHADOW, Am_Root_Object.Create ("shadow")
      .Add (TOP, 30));
  ...
  a = Am_Root_Object.Create ("a")
    .Add (LEFT, 5)
    .Set_Inherit_Rule (LEFT, Am_LOCAL);
  b = a.Create ("b");
