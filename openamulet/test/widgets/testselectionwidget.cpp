...
void Set_LTWH_Anims_On(Am_Object& obj, Am_Slot_Key color_slot, bool wh) {
  ...
  obj
    .Set (Am_LEFT, Am_Animate_With (animlt))
    .Set (Am_TOP, Am_Animate_With (animlt))
    ;
  if (wh) {
    obj
      .Set (Am_WIDTH, Am_Animate_With (animlt))
...
void Set_XY_Anims_On(Am_Object& obj) {
  ...
  Am_Object animc = color_anim_proto.Create("line_color_anim");
  obj
    .Set (Am_X1, Am_Animate_With (animlt))
...
Am_Define_Method(Am_Create_New_Object_Method, Am_Object, create_new_object,
  ...
  if (old_object.Valid()) {
    ...
    if (v == circle_bm) proto = my_arc_proto;
    else if (v == rect_bm) proto = my_rectangle_proto;
    else if (v == line_bm) proto = my_line_proto;
    else if (v == text_bm) proto = my_text_proto;
    else std::cout << "**UNKNOWN type of create: " << v << std::flush;
                  ...
                  Am_Object event_window, Am_Input_Char ic))
{
  ...
  // start the polygon
  partial_poly = my_polygon_proto.Create ()
  ...
  bool use_new = (method_slot == Am_SELECTIVE_REPEAT_ON_NEW_METHOD);
    ...
    if (use_new)
    {
      ...
      std::cout << prompt_str << " on cmd " << cmd << " method = " << method
           << " new obj = " << current_selection << std::endl << std::flush;
...
Am_Define_Method( Am_Object_Method, void, change_setting, (Am_Object cmd) )
{
  ...
  case 'l': { // set to one rectangle object
    if (last_created_obj.Valid()) {
      ...
      v=my_selection.Peek(Am_VALUE);
      std::cout << "setting Am_VALUE of " << my_selection << " to " << v
           << " so only " << last_created_obj << std::endl << std::flush;
    }
    break;
  }
  ...
  case 'c': {
    ...
    std::cout << "Changing handles color to " << colorstr << std::endl << std::flush;
  ...
  case 'u':
    {
      ...
      if (last_command.Valid()) {
        ...
        std::cout << " undoing cmd " << last_command << " = " << s << std::endl << std::flush;
  ...
  case 'J': {
    ...
    case 1: std::cout << "Setting forward/backwards null (disabled)\n" << std::flush;
      my_selection.Set(Am_SELECTION_MOVE_CHARS, (0L));
      break;
    case 2: std::cout << "Setting forward/backwards to f,b,h,e\n" << std::flush;
      ...
      break;
    case 3: std::cout << "Setting forward/backwards to default\n" << std::flush;
  ...
  */
  ...
  case 'Z':
    ...
    if (vl.Empty()) std::cout << "List empty\n" << std::flush;
      ...
      if (v.Valid()) cmd = v;
      else obj.Add_Part(Am_INACTIVE_COMMANDS, cmd = Am_Command.Create());

      switch (cnt) {
      case 1: slot = Am_MOVE_INACTIVE; name = "MOVE"; break;
      case 2: slot = Am_GROW_INACTIVE; name = "GROW"; break;
      case 3: slot = Am_SELECT_INACTIVE; name = "SELECT"; break;
      case 4: slot = Am_CLEAR_INACTIVE; name = "CLEAR"; break;
      case 5: slot = Am_COPY_INACTIVE; name = "COPY"; break;
      case 6: slot = Am_CUT_INACTIVE; name = "CUT"; break;
      case 7: slot = Am_DUPLICATE_INACTIVE; name = "DUPLICATE"; break;
      case 8: slot = Am_TO_TOP_INACTIVE; name = "TO_TOP"; break;
      case 9: slot = Am_TO_BOTTOM_INACTIVE; name = "TO_BOTTOM"; break;
      case 10: slot = Am_GROUP_INACTIVE; name = "GROUP"; break;
      case 11: slot = Am_UNGROUP_INACTIVE; name = "UNGROUP"; break;
      }
      std::cout << "For object " << obj << " setting cmd " << cmd << " slot " <<
        name << "=" << slot << " to be " << inactive << std::endl << std::flush;
      cmd.Set(slot, inactive);
      if (inactive) inactive_which = (inactive_which  % 11) + 1;
      else active_which = (active_which  % 11) + 1;
    }
    break;
  }
  default:
...
Am_Define_Method(Am_Object_Method, void, my_do_sel, (Am_Object cmd)) {
  ...
  if (special_style != Am_No_Style) {
    ...
    std::cout << "  Attach on " << which_obj 
		 << " at " << attach << std::endl << std::flush;
  }
}
...
Am_Define_Method(Am_Object_Method, void, freehand_create,
  ...
  std::cout << "\n--++-- Created new freehand " << new_object << " at ("
       << left << "," << top << ")\n" << std::flush;
...
Am_Define_Method(Am_Object_Method, void, gesture_unrecognized,
  ...
  std::cout << "(nap = " << (double)inter.Get (Am_NONAMBIGUITY_PROB) << ", dist="
    << (double)inter.Get (Am_DIST_TO_MEAN) << ")" << std::endl;
}
...
Am_Define_Method(Am_Create_New_Object_Method, Am_Object, gesture_creator,
  ...
  if (old_object.Valid()) {
    ...
    std::cout << "\n--++-- Gesture created new object " << new_object
	 << " as a copy of " << old_object << std::endl << std::flush;
    created_objs.Add_Part (new_object);
  }
  else {
...
void load_bitmaps() {
  ...
  text_bm = Am_Text.Create("text not bitmap")
    .Set (Am_TEXT, "T")
    .Set (Am_FONT, Am_Font(Am_FONT_SERIF, true, false, false, Am_FONT_LARGE))
    .Set (Am_LINE_STYLE, Am_Black)
    ;
}
...
Am_Define_Method(Am_Get_Object_Property_Value_Method, void, get_obj_color,
  ...
  else old_value=obj.Peek(Am_FILL_STYLE);
}
...
Am_Define_No_Self_Formula(bool, should_do_animation) {
  ...
  if (anim_on) return true;  // && self.Get_Owner() == created_objs)
  else return false;
}
...
int main (int argc, char *argv[])
{
  ...
  animator_proto = Am_Animator.Create ("animator_proto")
  ...
  my_arc_proto = Am_Arc.Create("Sel_Circle_Proto")
    .Set (Am_LEFT, 0).Set(Am_TOP, 0).Set(Am_FILL_STYLE, Am_Black)
  ...
  my_line_proto = Am_Line.Create("Sel_Line_Proto")
    .Set (Am_LEFT, 0).Set(Am_TOP, 0).Set(Am_LINE_STYLE, Am_Black)
    .Add (Am_SAVE_OBJECT_METHOD, Am_Standard_Save_Object)
    .Add (Am_SLOTS_TO_SAVE, Am_Value_List()
	  .Add(Am_X1).Add(Am_Y1).Add(Am_X2).Add(Am_Y2)
	  .Add(Am_LINE_STYLE));
  my_polygon_proto = Am_Polygon.Create("Sel_Polygon_Proto")
    .Set (Am_LEFT, 0).Set(Am_TOP, 0).Set(Am_FILL_STYLE, Am_Black)
    .Set (Am_FILL_STYLE, Am_No_Style)
    .Add (Am_SAVE_OBJECT_METHOD, Am_Standard_Save_Object)
    .Add (Am_SLOTS_TO_SAVE, Am_Value_List()
	  .Add(Am_POINT_LIST).Add(Am_FILL_STYLE));
  freehand_line_proto= Am_Polygon.Create("Sel_Freehand_Proto")
    .Set (Am_LEFT, 0).Set(Am_TOP, 0).Set(Am_LINE_STYLE, Am_Black)
    .Set (Am_FILL_STYLE, Am_No_Style)
    .Add (Am_SAVE_OBJECT_METHOD, Am_Standard_Save_Object)
    .Add (Am_SLOTS_TO_SAVE, Am_Value_List()
	  .Add(Am_POINT_LIST).Add(Am_LINE_STYLE));
  my_text_proto = Am_Text.Create("Text_Proto")
    .Set (Am_LEFT, 0).Set(Am_TOP, 0).Set(Am_LINE_STYLE, Am_Black)
    .Set (Am_TEXT, "")
    .Add (Am_SAVE_OBJECT_METHOD, Am_Standard_Save_Object)
    .Add (Am_SLOTS_TO_SAVE, Am_Value_List()
	  .Add(Am_LEFT).Add(Am_TOP).Add(Am_TEXT)
	  .Add(Am_LINE_STYLE));
...
//    .Set(Am_START_WHEN, "ANY_RIGHT_DOWN")
  ...
  my_selection = Am_Selection_Widget.Create("my_selection")
    ...
    .Set(Am_GRID_Y, grid_if_should)
    ;
  my_selection.Get_Object(Am_COMMAND)
    .Set(Am_DO_METHOD, my_do_sel);
  my_selection.Get_Object(Am_MOVE_GROW_COMMAND)
 ...
 l.Add (Am_Command.Create("Arrange_Command")
...
*/			    
  ...
  Am_Object text_edit_inter = Am_Text_Edit_Interactor.Create("my_text_edit")
    .Set(Am_ACTIVE, text_is_selected)
    ;
  created_objs.Add_Part(text_create_inter)
