...
static void set_dialog_position(Am_Object &the_dialog, int x, int y) {
...
Am_Define_Formula (Am_Value, value_from_text_part)
{
  ...
  Am_Object owner = self.Get_Owner();
  if (owner.Valid ())
    value = owner.Get_Object(Am_WINDOW).Get_Object(Am_DIALOG_GROUP)
      .Get_Object(Am_TEXT_WIDGET).Get(Am_VALUE);
  else
    value = Am_Value (val);
  return value;
}
...
Am_Define_Method(Am_Object_Method, void, choice_dialog_stop, (Am_Object cmd))
{
  ...
  Am_Value return_value = widget.Get_Object(Am_DIALOG_GROUP)
    .Get_Object(Am_DIALOG_BUTTONS).Get(Am_VALUE);
...
Am_Define_Method(Am_Object_Method, void, standard_ok_method,
		 (Am_Object cmd)) {
  Am_Object widget = cmd.Get(Am_SAVED_OLD_OWNER);
  if (!widget.Valid()) Am_Error("Lost widget");
  Am_Object window = widget.Get(Am_WINDOW);
  if (!window.Valid()) Am_Error("Lost window");
...
static void init()
{
  ...
  am_empty_dialog = Am_Window.Create(DSTR("empty_dialog_window"))
    ...
    .Add_Part (Am_COMMAND, Am_Command.Create(DSTR("Command_In_Dialog"))
	       .Set(Am_VALUE, ""))
    .Add_Part (Am_DIALOG_GROUP, Am_Widget_Group.Create (DSTR("Group_In_Dialog"))
	       ...
	       .Add_Part (Am_Map.Create (DSTR("Dialog_Prompt_Text"))
			  .Set_Part (Am_ITEM_PROTOTYPE, Am_Text.Create (DSTR("Prompt_Text_Proto"))
...
//      .Set_Single_Constraint_Mode (Am_VALUE, false)
  ...
  Am_Alert_Dialog = am_empty_dialog.Create (DSTR("alert_dialog"))
    .Set (Am_TITLE, "Alert")
  ...
  Am_Text_Input_Dialog = am_empty_dialog.Create (DSTR("text_input_dialog"))
    ...
    .Get_Object (Am_DIALOG_GROUP)
      ...
      .Add_Part (Am_DIALOG_BUTTONS, Am_Button_Panel
		 .Create (DSTR("buttons_in_text_dialog"))
  ...
  Am_Choice_Dialog = am_empty_dialog.Create(DSTR("Choice_Dialog"))
  ...
  Am_Standard_OK_Command = Am_Command.Create(DSTR("Am_Standard_OK_Command"))
    ...
    .Set(Am_ACCELERATOR, return_char)
    ;
  Am_Standard_Cancel_Command = Am_Command.Create(DSTR("Am_Standard_Cancel_Command"))
  ...
  Am_About_Amulet_Command = Am_Command.Create(DSTR("Am_About_Amulet_Command"))
    .Set(Am_VALUE, Am_No_Value)
...
static Am_Initializer* initializer = new Am_Initializer(DSTR("Dialog_Widgets"), init, 5.6f, 109, cleanup);
				   ...
				   bool modal /*=false*/)
{
  ...
  Am_Screen.Add_Part (the_dialog);
  set_dialog_position(the_dialog, x, y);
  Am_Object text_widget = the_dialog.Get_Object(Am_DIALOG_GROUP)
...
Am_String Am_Show_File_Dialog_For_Open(Am_String initial_value) {
  Am_Value v =
    Am_Get_Input_From_Dialog(Am_Value_List().Add("Type filename for Read:"),
			     initial_value);
  if (v.Valid()) return v;
  else return "";
}
//*** TEMPORARY
Am_String Am_Show_File_Dialog_For_Save(Am_String initial_value) {
  Am_Value v =
    Am_Get_Input_From_Dialog(Am_Value_List().Add("Type filename for Write:"),
			     initial_value);
  if (v.Valid()) return v;
  else return "";
}
...
Am_Object Am_Get_About_Amulet_Window() {
  if (!am_about_amulet_window.Valid()) {
    ...
    Am_Object bitmap = Am_Bitmap.Create(DSTR("About Amulet GIF"))
      .Set (Am_IMAGE, picture);
    am_about_amulet_window = Am_Window.Create(DSTR("About Amulet"))
      .Set(Am_DESTROY_WINDOW_METHOD, Am_Default_Pop_Up_Window_Destroy_Method)
      ...
      .Add_Part(Am_One_Shot_Interactor.Create()
		...
		.Set(Am_START_WHEN, Am_Value_List()
