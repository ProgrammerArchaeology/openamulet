...
void Am_Execute_Command(Am_Object& command, const Am_Object& widget)
{
	...
	Am_INTER_TRACE_PRINT(command, "%%Executing Am_DO_METHOD of " << command << "=" << do_method);
...
Am_Object am_perform_on_cmd_and_impls(Am_Object & last_command,
				      Am_Slot_Key slot,
				      bool copy /* = false */,
  ...
  if (copy) {
    ...
    if (fix_name) Am_Modify_Command_Label_Name(last_command, slot);
    prev = last_command;
  }
  obj = last_command;
  ...
  while (true) {
    ...
    if (debug) {
      switch(slot) {
      case Am_UNDO_METHOD: std::cout << "%%Executing Am_UNDO_METHOD"; break;
      case Am_REDO_METHOD: std::cout << "%%Executing Am_REDO_METHOD"; break;
      case Am_SELECTIVE_UNDO_METHOD:
	std::cout << "%%Executing Am_SELECTIVE_UNDO_METHOD"; break;
      case Am_SELECTIVE_REPEAT_SAME_METHOD:
	std::cout << "%%Executing Am_SELECTIVE_REPEAT_SAME_METHOD"; break;
      case Am_SELECTIVE_REPEAT_ON_NEW_METHOD:
	std::cout << "%%Executing Am_SELECTIVE_REPEAT_ON_NEW_METHOD"; break;
      }
      std::cout << " of " << obj;
    }
    #endif
