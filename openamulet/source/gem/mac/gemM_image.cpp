...
void
Am_Drawonable_Impl::Get_Image_Size(
...
PixMapHandle
Am_Image_Array_Data::Get_Mac_PixMap(
...
Am_Image_Array_Data::load_image_from_file()
{
  ...
  if (!strcmp(&(name[l - 4]), ".gif"))
  {
    ...
    if (image) {
      ...
      transp_color = color_map[0];  // transp color has already been adjusted to pos 0 during create
      load_image_data(image->Get_Data(), width, height, depth, color_map, num_colors);
    }
  }
  else
...
void
Am_Image_Array_Data::load_image_data(
  const unsigned char* bit_data,
  ...
  int num_colors )
{
  ...
  // Make Color Table
  CTabHandle  ctab = (CTabHandle)NewHandle(sizeof(ColorTable) + num_colors * sizeof(ColorSpec));
  if( ctab )
  {
    ...
    for (int i = 0; i < num_colors; i ++)
    {
      ...
      (**ctab).ctTable[i].rgb.green = (color_map[i].green << 8) + color_map[i].green;
      (**ctab).ctTable[i].rgb.blue = (color_map[i].blue << 8) + color_map[i].blue;
    }
  ...
  if (!draw_monochrome) {
    if (!fs.Valid()) {
      ...
      if (BitMap*  bits = offScrPix1.GetBitMap()) { // assignment
        CopyBits(&((GrafPtr)mac_port)->portBits, bits, &toRect, &intermediateRect, srcCopy, (0L));
        int    transferMode = is_transparent ? transparent: srcCopy;
        CopyBits((BitMap*)*pixMap, bits, &fromRect, &intermediateRect, transferMode, (0L));
        CopyBits(bits, &((GrafPtr)mac_port)->portBits, &intermediateRect, &toRect, srcCopy, (0L));
      ...
      if (bits2) {
        if (!is_transparent){
          ...
          CopyBits(bits2, &((GrafPtr)mac_port)->portBits, &fromRect, &toRect, srcCopy, (0L));
 
          int    transferMode = is_transparent ? srcCopy:transparent;
          // transparent here makes the opaque bitmap dominate the screen
        
          CopyBits((BitMap*)*pixMap, bits2, &fromRect, &intermediateRect, transparent, (0L));
          CopyBits(bits2, &((GrafPtr)mac_port)->portBits, &intermediateRect, &toRect, srcCopy, (0L));
          ...
          if (BitMap*  bits3 = offScrPix3.GetBitMap()) { // assignment
            ...
            CopyBits(bits2, bits3, &intermediateRect, &intermediateRect, srcCopy, (0L));
            int    transferMode = is_transparent ? transparent: srcCopy;
            CopyBits((BitMap*)*pixMap, bits3, &fromRect, &intermediateRect, transferMode, (0L));
            CopyBits(bits3, &((GrafPtr)mac_port)->portBits, &intermediateRect, &toRect, srcCopy, (0L));
    ...
    // we have monochrome images
      ...
      if (bits4) {
        ...
        PaintRect (&intermediateRect);  // paints to bits2
        CopyBits(bits4, &((GrafPtr)mac_port)->portBits, &fromRect, &toRect, srcCopy, (0L));
 
        int    transferMode = is_transparent ? srcCopy:transparent;
        // transparent here makes the opaque bitmap dominate the screen
        CopyBits((BitMap*)*pixMap, &((GrafPtr)mac_port)->portBits, &fromRect, &toRect, transferMode, (0L));
