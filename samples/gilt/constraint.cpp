...
class Gilt_Constraint_Variable_Data : public Am_Wrapper{
  ...
  public :
    ...
    {
        int diff;
        if((short)var > (short)'Z') diff = 26 + (short)var - (short)'a';
        else diff = (short)var - (short)'A';
...
Am_Formula Get_Data_And_Adjust (Am_Object object, Am_Slot_Key obj_key, 
    ...
    Am_Slot_Key key = NULL, float multiplier = -0.5){
    Am_Formula formula (get_data_and_adjust_procedure, "Get_Data_And_Adjust");
    formula.Set_Data (new Gilt_Inferred_Object_Offset_Store_Data (object,
                obj_key, obj_ref_key, obj_offset, obj_multiplier, key, multiplier));
    return formula;
}    
...
Constraint_How_Set check_dependency(Am_Object &obj, Which_Constraint_Handle handle,
				    Am_Object window){
  Am_Slot_Key key;
  if(handle == CH_CENTER_X || handle == CH_RIGHT) key = Am_LEFT;
  else
  if(handle == CH_CENTER_Y || handle == CH_BOTTOM) key = Am_TOP;
  else key = get_slot_key(handle);
...
bool slot_has_dependency(Am_Object handle, Am_Object for_obj){
...
int get_ref_count(Am_Object obj, Which_Constraint_Handle wh){
...
Am_Define_Style_Formula(get_state_color){
    ...
    if(state == NOT_SELECTED) return Am_Black;
    else 
    if(state == PRIMARY) return Am_Red;
    else
    if(state == SECONDARY) return Am_Blue;
    else return Am_Style::Am_Style(1.0, 0, 1.0);
}    
...
Am_Define_Formula(bool, is_text_valid){
  if((const char *)(Am_String)self.Get(Am_TEXT)) return true;
  else return false;
}
  ...
  if(for_obj.Valid()){
    ...
    if(!v.Valid()){
        ...
        if(handle.Valid()){
          ...
          switch(wh.value){
          case CH_LEFT_val : center_x = (int)self.Get(Am_WIDTH)/2; break;
          case CH_CENTER_Y_val : center_x 
	   		       = left_off + width/2 - C_HANDLE_SIZE;
                               break;
          case CH_WIDTH_val :
          case CH_TOP_val :
          case CH_CENTER_X_val :
          case CH_BOTTOM_val : center_x = left_off + width/2; break;
          case CH_RIGHT_val : center_x = left_off + width; break;
          case CH_HEIGHT_val : center_x = left_off + width + HANDLE_SPACING; break;  
	  default : break;
        }
      }
    }
...
Am_Define_Formula(int, compute_top_rect){
  ...
  if(for_obj.Valid()){
    ...
    if(!v.Valid()){
      ...
      case CH_RIGHT_val:
      case CH_HEIGHT_val:
      case CH_LEFT_val : top = y_off + height/2; break;
      case CH_WIDTH_val : break;
      case CH_TOP_val : top = y_off; break;
      case CH_BOTTOM_val : top = y_off + height; break;
      case CH_CENTER_X_val : top = y_off + height/2 - C_HANDLE_SIZE;
                             break;
      case CH_CENTER_Y_val : top = y_off + height/2; break;			   
      default : break;
      } 
    }
  }  
...
Am_Define_Formula(int, compute_center_x_line){
  ...
  if(for_obj.Valid()){
    ...
    if(v.Valid()){
      ...
      switch(wh.value){
      case CH_Y1_val : center_x = (rsl) ? left_off + width - C_HANDLE_SIZE 
		         - (int)self.Get(Am_WIDTH)/2 : 
			   left_off + C_HANDLE_SIZE + (int)self.Get(Am_WIDTH)/2;
                         break;
      case CH_Y2_val : center_x = (!rsl) ? left_off + width - C_HANDLE_SIZE
		         - (int)self.Get(Am_WIDTH)/2 : 
			   left_off + C_HANDLE_SIZE + (int)self.Get(Am_WIDTH)/2;
                         break;
      case CH_X1_val : center_x = (rsl) ? left_off + width : left_off;
                         break;
      case CH_X2_val : center_x = (!rsl) ? left_off + width : left_off;
                         break;
      default : break;
      }
    }
...
Am_Define_Formula(int, compute_top_line){
  ...
  if(for_obj.Valid()){
    ...
    if(v.Valid()){
      ...
      switch(wh.value){
        case CH_X1_val : top = (upside_down) ? height - C_HANDLE_SIZE : 
                                               C_HANDLE_SIZE_D2 + C_HANDLE_SIZE;
                           break;
        case CH_X2_val : top = (!upside_down) ? height - C_HANDLE_SIZE : 
                                               C_HANDLE_SIZE + C_HANDLE_SIZE_D2;
                           break;
        case CH_Y1_val : top = (!upside_down) ? 0 : height; break;
        case CH_Y2_val : top = (upside_down) ? 0 : height; break;
        default : break;
      }
    }
  }
...
Am_Define_Formula(int, compute_vhline_height){
    ...
    switch(id){
        case 1 :
        case 0 : height = (v.Valid())? C_HANDLE_SIZE : HANDLE_SPACING;
                 break;
        case 3 : 
...
Am_Define_Formula(int, compute_vhline_width){
    ...
    switch(id){
        case 2 :
        case 3 : height = (v.Valid())? C_HANDLE_SIZE : HANDLE_SPACING;
                 break;
        case 0 : 
...
Am_Define_Formula(int, compute_vhline_top){
    ...
    if(!v.Valid()){
        ...
        switch(id){
            case 0 :
            case 1 : top = 0; break;
            case 2 : top = HANDLE_SPACING-1; break;
            case 3 : top = height + HANDLE_SPACING-1; break;
        }
    } else {
        int y1 = for_obj.Get(Am_Y1);
        int y2 = for_obj.Get(Am_Y2);
        ...
        bool upside_down = (y1 > y2) ? true : false;
        switch(id){
            case 0 : top = (upside_down) ? height - C_HANDLE_SIZE_D2 :
                                           C_HANDLE_SIZE_D2; break;
            case 1 : top = (!upside_down) ? height - C_HANDLE_SIZE_D2 :
                                           C_HANDLE_SIZE_D2; break;
            case 2 : top = (upside_down) ? height + C_HANDLE_SIZE_D2 :
                                           C_HANDLE_SIZE_D2; break;
            case 3 : top = (!upside_down) ? height + C_HANDLE_SIZE_D2 :
                                           C_HANDLE_SIZE_D2; break;
        }
    }
...
Am_Define_Formula(int, compute_vhline_left){
    ...
    if(!v.Valid()){
        ...
        switch(id){
            case 0 : left = left_off; break;
            case 1 :
            case 2 :
            case 3 : left = left_off+width; break;
        }
    } else {
        int x1 = for_obj.Get(Am_X1);
        int x2 = for_obj.Get(Am_X2);
        ...
        switch(id){
            case 0 : left = (rsl) ? left_off + width : left_off; break;
            case 1 : left = (!rsl) ? left_off + width : left_off; break;
            case 2 : left = (rsl) ?  left_off + width - C_HANDLE_SIZE :
                                    left_off; break;
            case 3 : left = (!rsl) ?  left_off + width - C_HANDLE_SIZE :
                                    left_off; break;
        }
    }
...
Am_Object draw_arrow_lines(bool vertical){
    ...
    if(vertical){
        main_line.Set(Am_WIDTH, 1)
                   .Set(Am_HEIGHT, Am_From_Owner(Am_HEIGHT));
    } else {
        main_line.Set(Am_WIDTH, Am_From_Owner(Am_WIDTH))    
                   .Set(Am_HEIGHT, 1);
    }
...
Am_Define_Formula(int, compute_handle_lt){
	...
	left = (x1 > x2) ? (int)for_obj.Get(Am_LEFT) 
	        - (int)self.Get_Object(X2_HANDLE).Get(Am_WIDTH)/2 :
	        (int)for_obj.Get(Am_LEFT) - (int)self.Get_Object(X1_HANDLE)
	                                             .Get(Am_WIDTH)/2;
...
Am_Define_Method(Am_Where_Method, Am_Object, compute_constraint_start_where,
  ...
  Am_Object constraint_handle_group 
            = Am_Point_In_Part(object.Get_Object(Am_FEEDBACK_OBJECT),
                               x, y, event_window);
  if(constraint_handle_group.Valid()){
    Am_Object handle = Am_Point_In_Part(constraint_handle_group, 
                                        x, y, event_window);
...
Am_Define_Method(Am_Mouse_Event_Method, void, set_primary_object,
    ...
    if(handle.Valid()) handle.Set(STATE, PRIMARY);
    ...
    feedback_obj.Set(Am_VISIBLE, true)
                .Set(Am_X1, mouse_x).Set(Am_Y1, mouse_y);
...
Am_Define_Method(Am_Mouse_Event_Method, void, look_for_secondary_object,
    ...
    if(handle_group.Valid()){                                        
        ...
        if(handle.Is_Instance_Of(handle_proto) && handle != widget.Get_Object(PRIMARY_HANDLE)){
...
Am_Value_List setup_regular_constraint(Am_Object &command, Am_Object po,
    ...
    switch(ph.value){
        ...
        case CH_BOTTOM_val :
            ...
            break;
        default : 
            p_obj_slot = get_slot_key(ph); self_slot = NULL; ;
            break;
    }                    
...
void decrease_ref_count(Am_Object obj, Which_Constraint_Handle wh){
    int index = get_count_index(wh);
...
void increase_ref_count(Am_Object obj, Which_Constraint_Handle wh){
    int index = get_count_index(wh);
    Am_Value_List ref_count = (Am_Value_List)obj.Get(REF_COUNT);
    int count = (int)ref_count.Get_Nth(index);
    ref_count.Move_Nth(index);
...
Am_Value_List depends_list_be_decreased(Am_Object primary_handle, 
...
Am_Define_Method(Am_Mouse_Event_Method, void, setup_constraint_and_clear_slots,
        ...
        if(handle.Valid() && handle.Is_Instance_Of(handle_proto) 
          && handle != primary_handle){
            ...
            Offset_Percentage_Dialog
                .Get_Object(OFFSET_INPUT).Set(Am_VALUE, 0)
                .Get_Owner()
                .Get_Object(PERCENT_INPUT).Set(Am_VALUE, percentage)
                .Get_Owner()
		    ...
		    Am_Object cmd = inter.Get_Object(Am_COMMAND);
                    constraint_info = 
                            setup_regular_constraint(cmd, primary_obj, ph, secondary_obj,
                                                     sh, offset, mult);
    ...
    if(!constraint_set){
        widget.Set(PRIMARY_HANDLE, NULL).Set(PRIMARY_OBJ, NULL)
              .Set(SECONDARY_OBJ, NULL).Set(SECONDARY_HANDLE, NULL);
        Am_Abort_Interactor(inter);              
    }              
}
...
Am_Define_Method(Am_Object_Method, void, return_constraint_info,
    ...
    if((int)cmd.Get(Am_ID) == 1){
        int offset = Offset_Percentage_Dialog.Get_Object(OFFSET_INPUT)
                        .Get(Am_VALUE);
        int percent = Offset_Percentage_Dialog.Get_Object(PERCENT_INPUT)
                         .Get(Am_VALUE);                         
        Am_Value_List vlist = Am_Value_List().Add(offset).Add(percent);
        Am_Finish_Pop_Up_Waiting(Offset_Percentage_Dialog,(Am_Value)vlist);
       
    } else Am_Finish_Pop_Up_Waiting(Offset_Percentage_Dialog, Am_No_Value);
}    
...
void Constraint_Widget_Initialize()
{
  ...
  handle_proto = Am_Group.Create()
    ...
    .Add_Part(TEXT_HANDLE, Am_Text.Create()
        .Set(Am_TEXT, compute_handle_text)
	.Set(Am_VISIBLE, is_text_valid)
        .Set(Am_LEFT, 1).Set(Am_TOP, 1)
  ...
  One_Constraint_Handle
    .Add_Part(LEFT_HANDLE, one_handle)
    .Add_Part(Am_Line.Create().Set(Am_WIDTH, 1).Set(Am_VISIBLE, rect_visible)
    ...
    .Add_Part(Am_Line.Create().Set(Am_HEIGHT, 1).Set(Am_VISIBLE, rect_visible)
              .Set(Am_LINE_STYLE, Am_Dotted_Line)
              .Set(Am_WIDTH, Am_From_Sibling(Am_ITEM, Am_WIDTH))
              .Set(Am_LEFT, Am_From_Sibling(LEFT_HANDLE, Am_WIDTH, 0, 0.5))
              .Set(Am_TOP, Am_From_Sibling(LEFT_HANDLE, Am_TOP, C_HANDLE_SIZE_D2-1)))
  ...
  One_Constraint_Handle
    .Add_Part(one_line.Set(Am_HEIGHT, Am_From_Sibling(Am_ITEM, Am_HEIGHT))
                      .Set(Am_TOP, HANDLE_SPACING)
                      .Set(Am_LEFT, Am_From_Sibling(HEIGHT_HANDLE, CENTER_X,-3)));

  one_line = draw_arrow_lines(false);                      
  One_Constraint_Handle
    .Add_Part(one_line.Set(Am_WIDTH, Am_From_Sibling(Am_ITEM, Am_WIDTH))
                      .Set(Am_TOP, C_HANDLE_SIZE_D2-3)
                      .Set(Am_LEFT, Am_From_Sibling(LEFT_HANDLE, CENTER_X)));

  one_line =  Am_Line.Create().Set(Am_LINE_STYLE, Am_Dotted_Line)
  ...
  lfeedback.Set(Am_LINE_STYLE, Am_Thin_Line)//change this to arrow line
  ...
  Constraint_Widget
    ...
    .Set_Part(Am_ITEM_PROTOTYPE, One_Constraint_Handle.Create("constraint handle proto"))
    .Get_Owner();
