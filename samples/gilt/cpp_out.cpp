...
bool is_in_list(Am_Value_List l, Am_Value item){
  l.Start();
...
void output_line_styles(bool in_header, ostream &os, Am_Value_List &line_styles){
  ...
  for(line_styles.Start(); !line_styles.Last(); line_styles.Next()){
    l = (Am_Value_List)line_styles.Get(); l.Start();
    name = (Am_String)l.Get(); l.Next();
    if(in_header){
      ...
      os << "Am_Style " << name << " = Am_Style::Am_Style(" << r << ", " << g <<", " << b << ", " <<thickness<<", ";
      os << "Am_CAP_BUTT, Am_JOIN_MITER, ";
      os << "\n                         ";
      
      switch(line_flag){
      case Am_LINE_SOLID : os << "Am_LINE_SOLID, "; break;
      case Am_LINE_ON_OFF_DASH : os << "Am_LINE_ON_OFF_DASH, "; break; 	
      }
      switch(dash_l[0]){
      case 4 : os << "Am_DEFAULT_DASH_LIST"; break;
      default : os << "{2, 2}"; break;
      }
      os << ");\n";
    }
  }
...
void output_images(bool inheader, ostream &os, Am_Value_List &image_list){
  Am_Value_List l;
  for(image_list.Start(); !image_list.Last(); image_list.Next()){
    l = image_list.Get(); l.Start();
    if(inheader) os << "extern Am_Image_Array " << l.Get()<<endl;
    else {
      os << "Am_Image_Array " << l.Get();  l.Next();
...
void linestyleout(ostream &os, char * indent, const Am_Object &obj) {
  ...
  key.Start();
  i = (int)key.Get(); key.Next(); j = (int)key.Get();
...
void sublabelout(ostream &os, char* indent, Am_Object &obj) {
  ...
  if(v.type == Am_STRING){
    ...
    } else output_object(os, s, label_obj, DO_NOT_ADD_PREFIX);
  }
  sprintf(s, "      %s", indent); 
  os << s <<")\n";
  os << indent << "    .Get_Owner()\n";
...
void fileimageout(ostream &os, char * indent, Am_Object &obj) {
  ...
  os << indent << "  .Set(Am_IMAGE, "<<convert_to_ia((char *)filename) <<")"<<endl;
} 
...
void fontout(ostream &os, char* indent, Am_Object &obj) {
  ...
  int type, size, style;
  fk.Start();
  type = fk.Get(); fk.Next(); size = fk.Get();
  fk.Next(); style = fk.Get();
...
void output_add_part(ostream &os, char* indent, Am_Object &obj, int is_list) {
  ...
  if(is_list == 1) os << indent <<".Add(";
  else if(is_list == 0) os << indent << ".Add_Part(";
...
void output_object(ostream &os, char* indent, Am_Object &obj, int is_add) {
  ...
  for (slots.Start (); !slots.Last (); slots.Next ()) {
    ...
    case Am_ACCELERATOR: slotout(os, indent, obj, "Am_ACCELERATOR", 
				  Am_ACCELERATOR); break;
    case Am_LABEL: slotout(os, indent, obj, "Am_LABEL", Am_LABEL); break;
    default: { //can't do the allocated slots with switch since not constants
      ...
      else if (slot == FILE_NAME) fileimageout(os, indent, obj);
  ...
  if(is_add != DO_NOT_ADD_PREFIX) os << indent << ")\n";
}
...
static void output_group(ostream &os, char* indent, Am_Object &obj, int mode) {
  ...
  if(mode != DO_NOT_ADD_PREFIX) os << indent << ")\n";
}
...
Am_Value_List &style_list, Am_Value_List &image_list){
  ...
  for(obj_list.Start(); !obj_list.Last(); obj_list.Next()){
    ...
    if(v.Valid()){
      ...
      i = (int)key.Get(); key.Next(); j = (int)key.Get();
      name = n2lstr[i][j];
      if(i != 8){//if i == 8, I can just use Am_Line_x instead of define new one
	s = n2l[i][j];
	key = Am_Value_List().Add(name).Add(s);
	if(!is_in_list(style_list, (Am_Value)key)) style_list.Add(key);
      }
    }
    ...
    if(v.Valid()){
      key = Am_Value_List().Add(convert_to_ia((char *)(Am_String)v)).Add((Am_String)v);
      if(!is_in_list(image_list, (Am_Value)key)) image_list.Add(key);
    }
  }
}
