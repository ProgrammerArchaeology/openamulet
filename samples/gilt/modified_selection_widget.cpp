...
void
modified_set_commands_for_sel(Am_Object &cmd, Am_Object &inter,
                              Am_Object &widget, Am_Value clicked_obj,
                              Am_Value new_value, Am_Value old_value)
{
  Am_Object inter_command, widget_command;
  inter_command = inter.Get_Object(Am_COMMAND);
  if (inter_command.Valid())
    modified_set_commands_for_sel_values(inter_command, clicked_obj, new_value,
                                         old_value);
  widget_command = widget.Get_Object(Am_COMMAND);
  if (widget_command.Valid())
    modified_set_commands_for_sel_values(widget_command, clicked_obj, new_value,
                                         old_value);
  if (cmd.Valid()) {
    ...
    if (impl_parent.Valid() && impl_parent != widget_command &&
      ...
      modified_set_commands_for_sel_values(cmd, clicked_obj, new_value,
                                           old_value);
      modified_set_commands_for_sel_values(impl_parent, clicked_obj, new_value,
                                           old_value);
    }
  }
}
...
void
modified_clear_multi_selections(const Am_Object &widget)
{
  Am_Value value;
...
void
modified_update_data_from_objs(Am_Value objs_value, Am_Value &new_data_value)
{
  Am_Object obj, owner;
  int left, top, w, h;
...
static void
modified_remove_object_from_selection(const Am_Object &object,
...
void
modified_update_objs_from_value(Am_Value objs_value, Am_Value new_data_value,
...
void
modified_adjust_all_objects_position_for_undo(Am_Value_List list,
...
void
modified_get_min_from_loc(Am_Object &ref_obj, Am_Inter_Location &loc,
...
void
modified_get_left_and_top_from_new_value(Am_Value &new_value, Am_Object ref_obj,
                                         int &min_left, int &min_top)
{
  min_left = 30000;
  min_top = 30000;
...
void
modified_toggle_object_in_list(const Am_Object &widget, Am_Object new_object,
                               Am_Value_List &list)
{
  list.Start();
...
void
modified_calculate_group_size(Am_Value_List list, int &min_left, int &min_top,
...
//fake_group
void
modified_calculate_fake_group_size_and_set(Am_Object &fake_group,
                                           Am_Value_List &list)
{
  int min_left = 29999;
  int min_top = 29999;
  ...
  Am_Object owner;
  modified_calculate_group_size(list, min_left, min_top, max_right, max_bottom,
                                owner);
...
{
  Am_Object ret =
      owner_start_where_operates_on_function(inter, object, event_window, x, y);
...
void
modified_update_new_objs_from_pos(Am_Value objs_value, int left, int top,
                                  Am_Value &new_data_value, bool growing,
                                  Am_Object command_obj)
{
  if (growing) // then must be a single object, so new_data_value will be OK
    modified_update_objs_from_value(objs_value, new_data_value, growing,
                                    command_obj);
  else {
    Am_Object obj;
    if (Am_Value_List::Test(objs_value)) {
      ...
      Am_Object owner;
      modified_calculate_group_size(objs_list, min_left, min_top, max_right,
                                    max_bottom, owner);
      modified_adjust_all_objects_position_for_undo(
          objs_list, left - min_left, top - min_top, new_data_value_list);
...
void
modified_sel_move_grow_general_undo_redo(Am_Object command_obj, bool undo,
                                         ...
                                         Am_Value new_value = Am_No_Value)
{
  ...
  if (new_sel.Valid()) {
    ...
    if (selective) {
      if (undo)
        modified_update_data_from_objs(new_sel, new_data_value);
      else
        modified_update_data_from_objs(new_sel, old_data_value);
    }
    if (undo) {
      modified_update_objs_from_value(new_sel, old_data_value, growing,
                                      command_obj);
      ...
      if (reload_data) {
        ...
        if (new_value.Valid()) {
          ...
          modified_get_left_and_top_from_new_value(new_value, ref_obj, left,
                                                   top);
        } else {
        ...
        modified_update_objs_from_value(new_sel, new_data_value, growing,
                                        command_obj);
      if (selective)
...
{
  modified_sel_move_grow_general_undo_redo(command_obj, false, true, true,
                                           new_sel, new_val);
}
...
{
  Modified_Selection_Widget =
      Am_Selection_Widget.Create()
          ...
          .Set(Am_SELECTIVE_REPEAT_SAME_METHOD,
               modified_sel_move_selective_repeat)
