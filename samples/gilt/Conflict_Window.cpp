...
Am_Define_Formula(bool, wider_than){
    ...
    if(inner_w > outter_w) return true;
    else return false;
}
...
Am_Define_Formula(bool, longer_than){
    ...
    if(inner_h > outter_h) return true;
    else return false;
}

Am_String print_value(Am_Value value){
    char buff[250] = {0};
...
Am_String enum_to_handle_string(Which_Constraint_Handle wh){
    switch(wh.value){
        case CH_RIGHT_val : return (Am_String)"RIGHT_HANDLE";
        case CH_LEFT_val : return (Am_String)"LEFT_HANDLE";
        case CH_TOP_val : return (Am_String)"TOP_HANDLE";
        case CH_BOTTOM_val : return (Am_String)"BOTTOM_HANDLE";
        case CH_CENTER_X_val : return (Am_String)"CENTER_X_HANDLE";
        case CH_CENTER_Y_val : return (Am_String)"CENTER_Y_HANDLE";
        case CH_WIDTH_val : return (Am_String)"WIDTH_HANDLE";
        case CH_HEIGHT_val : return (Am_String)"HEIGHT_HANDLE";
        case CH_X1_val : return (Am_String)"X1_HANDLE";
        case CH_X2_val : return (Am_String)"X2_HANDLE";
        case CH_Y1_val : return (Am_String)"Y1_HANDLE";
        case CH_Y2_val : return (Am_String)"Y2_HANDLE";
        default : return (Am_String)"";
    }
}

Am_Define_Method(Am_Object_Method, void, setup_new_method,
                                        (Am_Object /*cmd*/)){
...
Am_Define_Style_Formula(get_text_line_style){
  ...
  if(type == CONSTRAINT_TEXT) return Am_Blue;
  else if (type == OBJECT_TEXT) return Am_Red;
...
Am_Slot_Key get_slot_key_always(Which_Constraint_Handle wh){
  Am_Slot_Key key;

  if(wh == CH_CENTER_X || wh == CH_RIGHT) key = Am_LEFT;
  else if(wh == CH_BOTTOM || wh == CH_CENTER_Y) key = Am_TOP;
  else key = get_slot_key(wh);
  
  return key;
}


Am_Object find_next_text_obj(Am_Object text_group, Am_Value_List &list, int &left, int &top){
  Am_Object tmp;
  ...
  if(!(bool)tmp.Get(Am_VISIBLE)) tmp.Set(Am_VISIBLE, true);
  return tmp;
}  

void add_plain_text(Am_Object text_obj, char * text, int & left,
...
void new_line(int &left, int &top){
  top += TEXT_HEIGHT;
  left = 0;
}

Am_String get_handle_slot_name(Which_Constraint_Handle wh){
  char buffer[40];
...
void add_object(Am_Object text_group, Am_Object &obj, int &left, 
		int &top, Am_Value_List& list, int index){
...
void add_constraint_list(Am_Object text_group, Am_Object obj, 
		    Which_Constraint_Handle wh, int &left, 
		    int &top, Am_Value_List &list, int &index){
  Am_Object tmp;
  Am_Slot_Key key = get_slot_key_always(wh);
...
void add_constraint(Am_Object text_group, Am_Constraint * constr,
		    int &left, int &top, Am_Value_List &list, int &index){
  Am_Object tmp;
  char buffer[50];
  Am_Value_List const_list = text_group.Get(CONSTRAINT_LIST);
...
void add_slot(Am_Object text_group, Am_Slot_Key key, int &left, int &top, 
...
void add_slot_and_object(Am_Object text_group, Am_Slot_Key key, Am_Object for_obj,
		    int &left, int &top, Am_Value_List &list, int &slot_index, int &obj_index){
  char buffer[100];
...
void add_value(Am_Object text_group, Am_Object obj, Am_Slot_Key key, int &left, 
               int &top, Am_Value_List &list){
...
void add_constraint_and_value(Am_Object text_group, Am_Constraint_Iterator const_iter,
                              Am_Object obj, Am_Slot_Key key, int &left, int & top,
...
void clean_up(Am_Value_List &list){
    Am_Object tmp;
...
void clean_up_and_remove(Am_Object group, Am_Value_List &list){
    Am_Object tmp;
...
Am_Define_Method(Am_Object_Method, void, display_conflict_info,
  ...
  add_constraint_list(text_obj, working_obj, wh, left, top, g_list, const_index);
  new_line(left, top);
  ...
  sprintf(buffer, "In Slot %s of Object ",(char *)get_handle_slot_name(other_h));
  add_plain_text(text_obj, buffer, left, top, g_list);
  ...
  if(prev_obj.Valid()){
    ...
    text_obj.Get_Object(CHOICE_INTER).Get_Object(Am_COMMAND).Set(Am_VALUE, Am_No_Value);
  }    
...
Am_Value_List display_dependency_list(Am_Object &text_group, Am_Value_List &dep_info,
			     Am_Constraint *constr){
  ...
  for(dep_info.Start(); !dep_info.Last(); dep_info.Next()){
    ...
    add_slot_and_object(text_group, key, tmp, left, top, 
                        g_list, slot_index, object_index);
    const_iter = find_constraint_in(tmp, key);            
    const_iter.Start();
    if(const_iter.Last()) add_value(text_group, tmp, key, left, top, g_list);
    else{
      new_line(left, top);
      add_constraint_and_value(text_group, const_iter, tmp, key, left, top, g_list, const_index);
    }
    new_line(left, top);
  }
  return g_list;
...
Am_Define_Formula(Am_Value, compute_selected_object){
  ...
  if(ci.Valid()){
	...
	list = self.Get(OBJECT_LIST, Am_NO_DEPENDENCY);
      ...
      if(id == CONSTRAINT_TEXT) list = self.Get(CONSTRAINT_LIST, Am_NO_DEPENDENCY);
      else list = self.Get(SLOT_LIST, Am_NO_DEPENDENCY);
      if(index == -1){
        ci.Set(Am_VALUE, Am_No_Object);
        Am_Object(selected).Set(Am_SELECTED, false);
        ci.Get_Object(Am_COMMAND).Set(Am_VALUE, Am_No_Object);
      }
      else selected = list.Get_Nth(index);
    }
  }
  return selected;
}
...
Am_Define_Method(Am_Object_Method, void, show_details, (Am_Object inter)){
  ...
  if(Am_Object::Test(selected))
    ...
    dep_info = display_dependency_list(detail_group, dep_info, (Am_Constraint*)(Am_Ptr)selected);
    clean_up(dep_info);
  }
}
...
Am_Define_Method(Am_Object_Method, void, pop_detail_window, (Am_Object inter)){
  ...
  if(selected.Valid()){
    ...
    if(selected.type == Am_INT) Am_Show_Alert_Dialog("Slot selected");
    else{ 
      Am_Constraint * constr = (Am_Constraint *)(Am_Ptr)selected;
        ...
        if(Am_Formula_Advanced::Test(constr)){
          ...
          dep_list = display_dependency_list(Pop_Up_Detail_Window, dep_list, (Am_Constraint*)(Am_Ptr)selected);
          ...
          Pop_Up_Detail_Window.Set(Am_TOP, (int)window.Get(Am_TOP) + 10)
                              ...
                              .Set(Am_VISIBLE, true);
        } else Am_Error("Selected object is not recognized");
    }
  }    
}
...
Am_Object Conflict_Window_Initialize () {
  Pop_Up_Detail_Window = Am_Window.Create()
  ...
  general_text_proto = Am_Text.Create()
    ...
    .Add_Part(Am_INACTIVE_COMMANDS, Am_Command.Create()
	 .Add(Am_SELECT_INACTIVE, false));
  

  Am_Object text_group = Am_Scrolling_Group.Create()
    ...
    .Add(CONSTRAINT_LIST, Am_No_Value_List)
    .Add(SLOT_LIST, Am_No_Value_List)
    .Add_Part(CHOICE_INTER, Am_Choice_Interactor.Create("text_group_choice_inter")
	      .Set(Am_START_WHEN, "LEFT_DOWN")
	      .Set(Am_PRIORITY, 2.0)
	      .Set(Am_START_WHERE_TEST, Am_Inter_In_Text_Leaf)
	      .Set(Am_HOW_SET, Am_CHOICE_SET));

  Am_Object detail_interactor = Am_One_Shot_Interactor.Create("detail_interactor")
  ...
  Conflict_Window
    .Add_Part(Am_Border_Rectangle.Create()
	      ...
	      .Set(Am_LEFT, 3).Set(Am_TOP, 3))
    .Add_Part(MAIN_TEXT_GROUP, text_group.Create()
                                .Add_Part(detail_interactor.Copy("main_text_interactor")
		                .Set(Am_DO_METHOD, show_details)))
    .Add_Part(Am_Border_Rectangle.Create()
	      .Set(Am_FILL_STYLE, Am_Motif_Gray)
	      .Set(Am_WIDTH, Am_From_Owner(Am_WIDTH, -6))
	      .Set(Am_HEIGHT, Am_From_Sibling(DETAIL_TEXT_GROUP, Am_HEIGHT, 14))
	      .Set(Am_LEFT, 3).Set(Am_TOP, Am_From_Sibling(DETAIL_TEXT_GROUP, Am_TOP, -7)))                
    .Add_Part(DETAIL_TEXT_GROUP,text_group.Create()
	      .Set(Am_HEIGHT, 0).Set(Am_TOP, 165)
    ...
    .Add_Part(Am_Border_Rectangle.Create()
	      ...
	      .Set(Am_LEFT, 3).Set(Am_TOP, Am_From_Sibling(BUTTON_GROUP, Am_TOP, -7)))	      
    .Add_Part(BUTTON_GROUP, Am_Button_Panel.Create()
