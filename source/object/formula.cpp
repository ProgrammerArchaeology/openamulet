...
typedef enum { F_WRAPPER, F_VOID, F_INT, F_LONG, F_BOOL, F_FLOAT, F_DOUBLE,
	           F_CHAR, F_STRING, F_CONST_STRING, F_PROC, F_METHOD, F_VALUE,
			   F_CONST_VALUE
} Formula_Type;
...
#ifdef MEMORY
Dyn_Memory_Manager Formula_Constraint::memory (sizeof (Formula_Constraint), "Formula Constraint");
#endif

Am_Depends_Iterator::Am_Depends_Iterator ()
  : context(0L), current(0L)
{ ; }
...
Am_Depends_Iterator& Am_Depends_Iterator::operator=(const Am_Formula_Advanced* formula)
{
  context = formula;
  current = (0L);
  return *this;
...
unsigned short Am_Depends_Iterator::Length () const
{
  if (context)
    ...
    unsigned short count = 0;
      for (Dependency* curr = ((Formula_Constraint*)context)->depends_on; curr != (0L); curr = curr->next)
      ++count;
    return count;
  }
  else
...
bool Formula_Constraint::Is_Overridden_By (const Am_Slot& /* slot */, Am_Constraint *competitor)
{
  if (competitor == (0L)
...
Am_Formula::Am_Formula (const Am_Formula& in_formula)
...
Am_Formula& Am_Formula::operator= (const Am_Formula& in_formula)
{
  data = in_formula.data;
  data->Add_Ref();
  return *this;
...
const Am_Value Am_Formula::operator () (Am_Object& context) const
{
  Am_Value temp;
  switch (data->type) {
  case F_WRAPPER:
...
Am_Constraint* Am_Formula::Multi_Constraint (bool multi_local)
{
  Formula_Constraint *c = new Formula_Constraint (data);
  c->Set_Mode (c->Get_Mode()
	       | Am_FORMULA_MULTI_CONSTRAINT
	       | (multi_local ? Am_FORMULA_MULTI_LOCAL_CONSTRAINT : 0));
  return c;
}

void Am_Formula::Set_Data (Am_Wrapper* stored_data)
{
...
const Am_Value& Formula_Context::Get (const Am_Object_Advanced& object,
...
void Formula_Context::Set (const Am_Object_Advanced& object, Am_Slot_Key key,
			   const Am_Value& value, Am_Slot_Flags flags)
{
  if ( !(flags & Am_NO_DEPENDENCY) && (object == depender.Get_Owner ()) && (key == depender.Get_Key ()))
    {
    changed = true;
...
void Formula_Context::Note_Changed (const Am_Object_Advanced& object, Am_Slot_Key key)
{
...
void Formula_Context::Note_Unchanged (const Am_Object_Advanced& object, Am_Slot_Key key)
{
  if ((object == depender.Get_Owner ()) && (key == depender.Get_Key ()))
    changed = false;
  else
...
const Am_Value& Formula_Context::Raise_Get_Exception (const Am_Value& value,
...
Am_Wrapper* Formula_Context::Get_Data ()
{
  if (context->data)
...
void Formula_Context::Set_Data (Am_Wrapper* data)
{
  if (context->data)
