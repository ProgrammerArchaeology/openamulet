...
void call_all_commands(Am_Value value, Am_Slot_Key method_slot,
...
#ifdef DEBUG
void inc_int_value(Am_Object &obj, Am_Slot_Key slot, int inc,
		    Am_Object &command_obj) {
#else
void inc_int_value(Am_Object &obj, Am_Slot_Key slot, int inc,
		    Am_Object& /*command_obj*/) {
  ...
  Am_INTER_TRACE_PRINT(Am_INTER_TRACE_SETTING,
		       "++ Animation command " << command_obj
		       << " incrementing slot " << slot << " of " << obj
		       << " by " << inc << " to be (int)" << cur_val + inc);
  obj.Set(slot, cur_val + inc);
}

#ifdef DEBUG
void inc_float_value(Am_Object &obj, Am_Slot_Key slot, float inc,
		    Am_Object &command_obj) {
#else
void inc_float_value(Am_Object &obj, Am_Slot_Key slot, float inc,
		    Am_Object& /* command_obj */) {
  ...
  Am_INTER_TRACE_PRINT(Am_INTER_TRACE_SETTING,
		       "++ Animation command " << command_obj
		       << " incrementing slot " << slot << " of " << obj
		       << " by " << inc << " to be (float)" << cur_val + inc);
  obj.Set(slot, cur_val + inc);
}

void inc_value_slot(Am_Slot_Key slot, Am_Value inc_value, Am_Object &obj,
		    Am_Object &command_obj) {
  ...
  else Am_ERROR("Am_SMALL_INCREMENT should be of type int or float, but it is "
		<< inc_value << " in " << command_obj);
}

void inc_value_list(Am_Value_List slot_list, Am_Value_List inc_list,
  ...
  if (slot_list.Length() != inc_list.Length())
    Am_ERROR("length of list of slots not equal to length of list of "
	     " increment values in " << command_obj);
...
Am_Define_Method(Am_Object_Method, void, Am_Inc_Object_In_Owner_Do,
  ...
  if (obj.Valid()) {
	...
	if (x < 0 || right > owner_width ||
	    y < 0 || bottom > owner_height) {
	...
	if (x < -width) x = owner_width;
	else if (x > owner_width) x = -width;
	if (y < -height) y = owner_height;
	else if (y > owner_height) y = -height;
	break;
      case Am_ANIMATION_BOUNCE_val:
	if (x < 0) {
	  if (x_inc < 0) command_obj.Set(Am_X_OFFSET, -x_inc);
	  x += x_inc;
	}
	...
	if (y < 0) {
	  if (y_inc < 0) command_obj.Set(Am_Y_OFFSET, -y_inc);
	  y += y_inc;
	}
	...
	Am_ERROR("Bad Am_ANIMATE_END_ACTION " << (int)end_act.value << " in "
      ...
      Am_INTER_TRACE_PRINT(Am_INTER_TRACE_SETTING,
			   "++ Animation command " << command_obj
			   << " setting LEFT, TOP of " << obj
			   << " to be " << x << ", " << y);
      if (x_inc) obj.Set(Am_LEFT, x);
      if (y_inc) obj.Set(Am_TOP, y);
    }
  }
}
...
void interp_value_slot(Am_Slot_Key slot, Am_Value value1, Am_Value value2,
		       Am_Time elapsed_time, Am_Time time_for_animation,
		       Am_Object &obj, Am_Object &command_obj) {
  float return_val =
    Am_Clip_And_Map((float)elapsed_time.Milliseconds(),
		    0.0,
		    (float)time_for_animation.Milliseconds(),
		    (float)value1, (float)value2);
  Am_INTER_TRACE_PRINT(Am_INTER_TRACE_SETTING,
		       "++ Animation command " << command_obj
		       << " interpolating time " << elapsed_time << " out of "
		       << time_for_animation << " to get " << return_val
		       << " for slot " << slot << " of " << obj);
  if (value1.type == Am_INT && value2.type == Am_INT)
      obj.Set(slot, (int)return_val);
  else obj.Set(slot, return_val);
}

void interp_value_list(Am_Value_List slot_list,
		       Am_Value_List value1_list, Am_Value_List value2_list,
		       Am_Time elapsed_time, Am_Time time_for_animation,
		       Am_Object &obj, Am_Object &command_obj) {
  if (slot_list.Length() != value1_list.Length() ||
    ...
    Am_ERROR("length of list of slots not equal to length of list in "
	     " Am_VALUE_1 or Am_VALUE_2 slots of " << command_obj);
  ...
  for (slot_list.Start (), value1_list.Start (), value2_list.Start ();
    ...
    interp_value_slot(slot, value1, value2, elapsed_time,
...
Am_Define_Method(Am_Object_Method, void, Am_Constant_Velocity_Do,
		 (Am_Object command_obj)) {
  Am_Object obj = command_obj.Get(Am_OPERATES_ON);
  Am_INTER_TRACE_PRINT(command_obj, "Constant Velocity command " << command_obj
		       << " operating on " << obj);
  if (obj.Valid()) {
    ...
    float delta_time = (float)(((Am_Time)(command_obj.Get_Owner()
					  .Get(Am_REPEAT_DELAY)))
			       .Milliseconds());
    ...
    delta = fvalue2 > fvalue1 ? static_cast<float>(fabs(delta)) : static_cast<float>(-fabs(delta));
    ...
    if (end) {
      Am_Animation_End_Action end_act = (Am_Animation_End_Action)
	command_obj.Get(Am_ANIMATE_END_ACTION);
      Am_Object inter = command_obj.Get(Am_SAVED_OLD_OWNER);
      switch (end_act.value) {
      case Am_ANIMATION_STOP_val:
	Am_Stop_Interactor(inter);
	return;
      case Am_ANIMATION_WRAP_val:
	if (forward) new_value = value1;
	else new_value = value2;
	break;
      case Am_ANIMATION_BOUNCE_val:
	command_obj.Set(Am_ANIMATE_FORWARD, !forward);
	break;
	...
	Am_ERROR("Bad Am_ANIMATE_END_ACTION " << (int)end_act.value << " in "
      ...
      if (end_method.Valid()) end_method.Call(command_obj);
    }
    if (value1.type == Am_INT && value2.type == Am_INT)
      obj.Set(slot_key, (int)new_value);
    else obj.Set(slot_key, new_value);
...
Am_Define_Method(Am_Object_Method, void, Am_Slot_List_Setter_Do,
  ...
  if (obj.Valid()) {
	...
	break;
      case Am_ANIMATION_WRAP_val:
	if (current_index < 0) current_index = len-1;
	else if (current_index >= len) current_index = 0;
	break;
      case Am_ANIMATION_BOUNCE_val:
	if (current_index < 0) {
	  current_index = 1;
	...
	current_index = value_to_use;
      ...
      Am_INTER_TRACE_PRINT(Am_INTER_TRACE_SETTING,
			   "++ Animation list command " << command_obj
			   << " index " << current_index
...
Am_Define_Method(Am_Object_Method, void, Am_Slot_Blink_Do,
  ...
  if (obj.Valid()) {
    ...
    if (cur_value == value1) new_value = value2;
    else new_value = value1;
    Am_INTER_TRACE_PRINT(Am_INTER_TRACE_SETTING,
			 "++ Animation blink command " << command_obj
			 << " setting slot " << slot << " of " << obj
			 << " to be " << new_value);
...
static void init() {
  ...
  Am_Animation_Command = Am_Command.Create(DSTR("Animation_Command"))
  ...
  Am_Incrementer_Animation_Command = Am_Animation_Command.Create
    (DSTR("Incrementer_Animation_Command"))
  ...
  Am_Animation_Object_In_Owner_Command = Am_Animation_Command.Create
    (DSTR("Animation_Object_In_Owner_Command"))
  ...
  Am_Animator_Animation_Command = Am_Animation_Command.Create
    (DSTR("Animator_Animation_Command"))
  ...
  Am_Constant_Velocity_Command = Am_Animator_Animation_Command.Create
    (DSTR("Constant_Velocity_Command"))
  ...
  Am_Animation_Through_List_Command = Am_Animation_Command.Create
    (DSTR("Animation_Through_List_Command"))
    .Set(Am_INTERIM_DO_METHOD, Am_Slot_List_Setter_Do)
    .Add(Am_OPERATES_ON, Am_Get_Inter_Object)
    .Add(Am_SLOT_TO_ANIMATE, Am_IMAGE)
    .Add(Am_VALUES_FOR_SLOT_TO_ANIMATE, (0L)) //value_list of values for slot
  ...
  Am_Animation_Blink_Command = Am_Animation_Command.Create
    (DSTR("Animation_Blink_Command"))
  ...
  Am_Animation_Interactor = Am_Interactor.Create(DSTR("Am_Animation_Interactor"))
...
static Am_Initializer* initializer = new Am_Initializer(DSTR("Inter_Anim"), init, 4.1f);
