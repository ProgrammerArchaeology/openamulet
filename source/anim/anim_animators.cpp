...
void Am_Start_Animator(Am_Object interp, const Am_Value &value1, const Am_Value &value2)
{
...
Am_Define_Method(Am_Object_Method, void, Am_Animator_Start_Do, (Am_Object interp))
{
	// set up the animation timer
	Am_Time delay = interp.Get(Am_REPEAT_DELAY);
	Am_INTER_TRACE_PRINT(interp, "Animator Start of " << interp	<< " delay = " << delay);
...
Am_Define_Method(Am_Anim_Update_Method, void, Am_Animator_Interrupt, (Am_Object interp, const Am_Value& new_value))
{
	if((bool)interp.Get(Am_INTERRUPTIBLE))
	{
		Am_Value curr_value = interp.Get(Am_VALUE);
		Am_INTER_TRACE_PRINT(interp, "Animator Interrupt of " << interp	<< "; restarting from value "  << curr_value << " towards new value " << new_value);
	...
	{
		Am_INTER_TRACE_PRINT(interp, "Animator Interrupt of " << interp	<< "(running uninterrupted); got new value " << new_value);
		 ...
		 (Am_Object interp, const Am_Time& elapsed_time))
{
	...
	Am_INTER_TRACE_PRINT(interp, "Animator Animation of " << interp <<
		", time=" << elapsed_time <<
		", duration=" << duration <<
...
static void call_all_commands(Am_Value value, Am_Slot_Key method_slot,
		       Am_Object &inter,
		       bool skip_first)
{
	...
	while(value.Valid() && value.type == Am_OBJECT)
	{
		...
		if(Am_Object_Method::Test(value))
		{
			...
			Am_INTER_TRACE_PRINT(inter, "%%Animator " << inter
				<< " executing " << method_slot << " of "
...
Am_Define_Method(Am_Object_Method, void, Am_Animator_Do,
		 (Am_Object interp))
{
	Am_Value value_2 = interp.Get(Am_VALUE_2);
	Am_INTER_TRACE_PRINT(interp, "Animator Do of " << interp
		<< " setting value to " << value_2);
...
bool timing_web_create(const Am_Slot& slot)
{
	return slot.Get_Key() == Am_CURRENT_VELOCITY;
}

void timing_web_init( const Am_Slot& slot,
		  Am_Web_Init& init)
{
...
Am_Define_Formula(Am_Wrapper*, compute_repeat_delay)
{
	...
	return (delay_per_pixel > min_repeat_delay) ? delay_per_pixel : min_repeat_delay;
}
		  ...
		  (Am_Object interp, Am_Time t))
{
	...
	float tau = (float)
		((atan((D_0 + D_1)*fraction - D_0) + atan(D_0))
	/
		(atan(D_0) + atan(D_1)));
...
static Am_Value interpolate_1D(const Am_Value& value1,
				const Am_Value& value2, float tau)
{
	float return_val;
...
static Am_Value interpolate_ND(const Am_Value& value1,
				const Am_Value& value2, float tau)
{
...
Am_Define_Method(Am_Path_Function, Am_Value, Am_Linear_Path,
		  (Am_Object /* interp */,
		   Am_Value value1, Am_Value value2, float tau))
{
		   ...
		   Am_Value value1, Am_Value value2, float tau))
{
	...
	Am_Style(value2).Get_Values(r2, g2, b2, t2, cap, join, line_flag,
		dash_l, dash_l_length, fill_flag, poly,
		stipple);
	short dt = t2 - t1;
...
static Am_Value step_1D(const Am_Value& curr_value, const Am_Value& step_size,
...
static Am_Value step_ND(const Am_Value& curr_value,
			 const Am_Value& step_size,
			 const Am_Value& value2, bool& keep_running)
{
	...
	if(value2.Exists())
	{
		...
		{
			result.Add(step_1D(values.Get(), steps.Get(),
				value2s.Get(), keep_running));
		}
	...
	else
	{
		...
		for(values.Start(), steps.Start();
			!values.Last() && !steps.Last();
			values.Next(), steps.Next())
		{
			result.Add(step_1D(values.Get(), steps.Get(),
				Am_No_Value, keep_running));
		}
		 ...
		 (Am_Object interp, const Am_Time& elapsed_time))
{
	...
	Am_INTER_TRACE_PRINT(interp, "Stepper Animation " << interp
		<< " new value " << curr_value);
...
inline int list_position(Am_Value_List& list, const Am_Value& v)
{
	int i;
...
Am_Value add_offset(Am_Value v, Am_Value delta, Direction d, Am_Value ref,
	...
	switch(v.type)
	{
		...
		case Am_WRAPPER:
			if(Am_Value_List::Test(v))
			{
					...
					for(lv.Start(), ldelta.Start(), lref.Start();
						!lv.Last();
						lv.Next(), ldelta.Next(), lref.Next())
						result.Add(add_offset(lv.Get(), ldelta.Get(), d, lref.Get(),
							attenuation));
					return Am_Value(result);
			}
			// else fall through
...
static void next_exaggerated_phase(Am_Object interp)
{
	...
	switch(curr_state)
	{
		case Am_EXAGGERATOR_IDLE:
			...
			target = add_offset(value1, interp.Get(Am_WINDUP_AMOUNT),
				AWAYFROM, value2);
		 ...
		 (Am_Object interp))
{
	...
	if((bool)final_value)
	{ //currently invisible.  make it visible
		Am_Start_Animator(sub_animator, (Am_Wrapper*)invisible_location, (Am_Wrapper*)visible_location);

		interp.Set(Am_VALUE, true); //make visible throughout animation

		Am_INTER_TRACE_PRINT(interp,
			"  Animating to visible from " << (Am_Value)(Am_Wrapper*)invisible_location
			<< " to " << (Am_Value)(Am_Wrapper*)visible_location);
	}
	else
	{  //making invisible
		Am_Start_Animator(sub_animator, (Am_Wrapper*)visible_location, (Am_Wrapper*)invisible_location);

		interp.Set(Am_VALUE, true); //make visible throughout animation

		Am_INTER_TRACE_PRINT(interp, "  Animating invisible from " << (Am_Value)(Am_Wrapper*)visible_location
			<< " to " << (Am_Value)(Am_Wrapper*)invisible_location);
	}
...
Am_Define_Method(Am_Anim_Update_Method, void,
		 Am_Visible_Animator_Interrupt,
		 (Am_Object interp, const Am_Value& new_target))
{
	...
	if((bool)interp.Get(Am_INTERRUPTIBLE))
	{
		...
		else
		{
			...
			Am_INTER_TRACE_PRINT(interp, "Visible Interrupt changing direction, now animating to " << new_target);
			...
			Am_Start_Animator(sub_animator,
				sub_animator.Get(Am_VALUE),
				interp.Get((bool)new_target
				? Am_VISIBLE_LOCATION
...
// should really be a command object
Am_Define_Method(Am_Object_Method, void, Am_Visible_Sub_Do,
		 (Am_Object interp))
{
	Am_Animator_Do.Call(interp);
...
void add_obj_and_parts(Am_Object &orig, Am_Value_List &new_parts,
		       int &maxobjw, int &maxobjh)
{
	...
	if(method != agg_method && method != fade_method)
	{
		...
		if(wh > maxobjw) maxobjw = wh;
		wh = orig.Get(Am_HEIGHT);
		if(wh > maxobjh) maxobjh = wh;
	}
	Am_Object sub_obj;
...
Am_Define_Method(Am_Object_Method, void, fly_apart_anim_done,
		 (Am_Object cmd))
{
	...
	if(i == 0) Am_Stop_Animator(interp);
}
...
Am_Value_List break_up_objects(Am_Object &interp, Am_Object &orig, bool vis)
{
	...
	int trans_left, trans_top,
		win_w, win_h, win_center_x, win_center_y, maxwh, x, y, left, top;
		...
		new_parts.Next(), rads += rads_per_obj)
	{
		...
		Am_Translate_Coordinates(from_part, 0, 0,
			win, trans_left, trans_top);
...
Am_Define_Method(Am_Object_Method, void,
		 Am_Fly_Apart_Animator_Abort_Do,
		 (Am_Object interp))
{
...
{ Am_PL_SAME = 0, Am_PL_FROM_NOTHING,
		Am_PL_TO_NOTHING, Am_PL_ADD_POINT,
		Am_PL_REMOVE_POINT, Am_PL_VERY_DIFFERENT
};
...
int find_point_diff(Am_Point_List &old, Am_Point_List &newp)
{
	int diff = -1;
	int index = 0;
	float x1, y1, x2, y2;
	for(old.Start(), newp.Start(); !old.Last(); old.Next(), newp.Next(), index++)
	{
		old.Get(x1, y1);
		newp.Get(x2, y2);
		if(x1 != x2 || y1 != y2)
		{
			if(diff == -1)
			{ //maybe this is the point that is different
				diff = index;
				// std::cout << "different at index " << index <<std::endl <<std::flush;
				newp.Next();
			}
			else return -1;  // already found a different one to be different
		}
		//otherwise they are equal, just loop
	}
	if(diff == -1)
	{ //maybe it is the last point of newp
		if(newp.Last())
...
point_list_differences find_differences(Am_Point_List &old,
					Am_Point_List &newp,
					int &index_of_diff)
{
	//  std::cout << "checking differences of " << old << " and " << newp <<std::endl <<std::flush;
	if(old == newp) return Am_PL_SAME;
	int length_old = old.Length();
	int length_newp = newp.Length();
	if(length_old == 0) return Am_PL_FROM_NOTHING;
	if(length_newp == 0) return Am_PL_TO_NOTHING;
	if(length_newp == length_old + 1)
	{
		...
		if(index_of_diff >= 0) return Am_PL_ADD_POINT;
	}
	else if(length_newp == length_old - 1)
	{
		...
		if(index_of_diff >= 0) return Am_PL_REMOVE_POINT;
	}
...
Am_Point_List create_list_at_center(Am_Point_List &ref)
{
...
Am_Point_List add_point_to(Am_Point_List &old, int changed_point)
{
...
Am_Point_List add_points_at_start_to(Am_Point_List &old, int num_needed)
{
	Am_Point_List newl = old;
	newl.Start();
	float x1, y1;
...
void make_new_old_and_new_lists(Am_Point_List &old, Am_Point_List &newp,
				point_list_differences diff, int changed_point,
				Am_Point_List &new_old,
				Am_Point_List &new_newp)
{
	switch(diff)
	{
		case Am_PL_SAME:
...
Am_Define_Point_List_Formula(compute_point_lists)
{
	...
	if(old_value.Valid())
	{  //invalid is OK (use empty list)
		if(Am_Point_List::Test(old_value)) old = old_value;
		else Am_ERROR("Old value not a list, = " << old_value);
	}
	if(new_value.Valid())
	{  //invalid is OK (use empty list)
		if(Am_Point_List::Test(new_value)) newp = new_value;
		else Am_ERROR("New value not a list, = " << new_value);
	}
...
static Am_Value_List reverse(Am_Value_List lst)
{
	Am_Value_List rev;
...
Am_Define_Method(Am_Object_Method, void, discard_intermediate_values, (Am_Object cmd))
{
...
static void init()
{
	...
	Am_Animator = Am_Root_Object.Create(DSTR("Am_Animator"))
	// internal methods
	...
	Am_Stepping_Animator = Am_Animator.Create
		(DSTR("Am_Stepping_Animator"))
	.Set(Am_ANIMATION_METHOD, Am_Stepping_Animator_Animation)
	.Add(Am_SMALL_INCREMENT, 5)
	;

	Am_Style_Animator = Am_Animator.Create
		(DSTR("Am_Style_Animator"))
	.Set(Am_PATH_FUNCTION, Am_Style_Path)
	.Add(Am_RGB_COLOR, true)
	;

	Am_Exaggerated_Animator = Am_Animator.Create
		(DSTR("Am_Exaggerated_Animator"))
	...
	.Add_Part(Am_SUB_ANIMATOR, Am_Animator.Create
		(DSTR("Am_Exaggerated_Sub_Animator"))
	...
	.Set(Am_VALUE, Am_From_Part(Am_SUB_ANIMATOR, Am_VALUE).Multi_Constraint())
	;

	Am_Visible_Animator = Am_Animator.Create
		(DSTR("Am_Visible_Animator"))
	...
	Am_Fly_Apart_Animator = Am_Animator.Create
		(DSTR("Am_Fly_Apart_Animator"))
	...
	Am_Through_List_Animator = Am_Animator.Create
		(DSTR("Am_Through_List_Animator"))
	...
	Am_Object_In_Owner_Animator = Am_Animator.Create
		(DSTR("Am_Object_In_Owner_Animator"))
	;

	Am_Point_List_Animator = Am_Animator.Create
		(DSTR("Am_Point_List_Animator"))
	...
	Am_Animation_Wrap_Command = Am_Command.Create(DSTR("Am_Animation_Wrap_Command"))
	.Set(Am_DO_METHOD, wrap_do)
	.Set(Am_ABORT_DO_METHOD, discard_intermediate_values)
	;

	Am_Animation_Bounce_Command = Am_Command.Create(DSTR("Am_Animation_Wrap_Command"))
...
static Am_Initializer* initializer = new Am_Initializer(DSTR("Anim_Constraint"), init, 4.2f, 101, cleanup);
