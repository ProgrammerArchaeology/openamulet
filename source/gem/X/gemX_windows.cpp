...
//
unsigned long Am_GC_setmask = ( GCForeground | GCLineWidth |
				GCCapStyle   | GCJoinStyle |
				GCFunction   |
				GCLineStyle  | GCDashList  |
				GCBackground);

unsigned long Am_GC_getmask = ( GCForeground | GCLineWidth |
				GCCapStyle   | GCJoinStyle |
				GCFunction);

GC create_default_gc (Display *dpy, Window the_xlib_drawable,
 ...
 */
Window GetVRoot(Display *dpy, int scr)
{
...
void amulet_x_error (Display *display, XErrorEvent *myerr)
{
  char msg[80];
...
static Am_Drawonable* make_root_drawonable (const char* screen,
					    Display *new_display) {
  if (Main_Display != (0L)) {
  ...
  if (!XGetGeometry (new_display, the_xlib_drawable, &dummy_root,
		    &l, &t, &w, &h, &border_w, &bit_depth))
  ...
  XChangeProperty(new_display, the_xlib_drawable, XA_CUT_BUFFER0,
		  XA_STRING, 8, PropModeAppend, 0, 0);
  XChangeProperty(new_display, the_xlib_drawable, XA_CUT_BUFFER1,
		  XA_STRING, 8, PropModeAppend, 0, 0);
  XChangeProperty(new_display, the_xlib_drawable, XA_CUT_BUFFER2,
		  XA_STRING, 8, PropModeAppend, 0, 0);
  XChangeProperty(new_display, the_xlib_drawable, XA_CUT_BUFFER3,
		  XA_STRING, 8, PropModeAppend, 0, 0);
  XChangeProperty(new_display, the_xlib_drawable, XA_CUT_BUFFER4,
		  XA_STRING, 8, PropModeAppend, 0, 0);
  XChangeProperty(new_display, the_xlib_drawable, XA_CUT_BUFFER5,
		  XA_STRING, 8, PropModeAppend, 0, 0);
  XChangeProperty(new_display, the_xlib_drawable, XA_CUT_BUFFER6,
		  XA_STRING, 8, PropModeAppend, 0, 0);
  XChangeProperty(new_display, the_xlib_drawable, XA_CUT_BUFFER7,
		  XA_STRING, 8, PropModeAppend, 0, 0);
...
void install_attributes(XSetWindowAttributes *attrib,
...
void set_other_window_properties(Window the_xlib_drawable,
				 Display *dpy,
				 ...
				 bool query_user_for_size)
{
	...
	XSetWMProperties(dpy, the_xlib_drawable,
		&window_name_xtp, &icon_name_xtp, (0L), 0,
		the_sizehints, the_wmhints, (0L));
...
void Am_Drawonable_Impl::Create_X_Drawable(Am_Drawonable_Impl *parent_d,
					   ...
					   unsigned long attrib_value_mask)
{
	...
	xlib_drawable = XCreateWindow(screen->display,
		parent_d->xlib_drawable,
		left, top, width, height,
		border_width, parent_d->depth,
		InputOutput,
		CopyFromParent,
	...
	set_other_window_properties(xlib_drawable, screen->display,
		title, icon_name, iconified,
		min_width, min_height, max_width,
		max_height, ask_position, ask_size);
}

// Create a new offscreen xlib_drawable for d
void Am_Drawonable_Impl::Create_Offscreen_X_Drawable ()
{
					   ...
					   Am_Input_Event_Handlers *evh)
{
    ...
    install_attributes(&attrib, &attrib_value_mask,
		       d, screen, back_color,
		       save_under_flag, title_bar_flag,
...
XSizeHints get_xsizehints (Am_Drawonable_Impl *d) {
  long the_flags;
  XSizeHints the_sizehints;
  if (!(XGetWMNormalHints(d->screen->display, d->xlib_drawable,
			  &the_sizehints, &the_flags)))
...
{
  icon_name = new char[strlen(new_title)+1]; strcpy (icon_name, new_title);
  XTextProperty the_xtp;
...
void Am_Drawonable_Impl::Set_Position(int new_left, int new_top) {
  if (left == new_left && top == new_top) return;
...
void Am_Drawonable_Impl::Set_Size(unsigned int req_width,
  ...
  if ((width != new_width) || (height != new_height)) {
	...
	XCopyArea(screen->display, temp_drawable, xlib_drawable, screen->gc,
...
{
  if (offscreen) return; // not meaningful for offscreen pixmap
  visible = vis;
...
void Am_Drawonable_Impl::Reparent (Am_Drawonable *new_parent)
{
  ...
  XReparentWindow(screen->display, xlib_drawable,
		  ((Am_Drawonable_Impl *)new_parent)->xlib_drawable,
		  left, top);
  owner = new_parent;
  if (need_to_remap)
...
void Am_Drawonable_Impl::Map_And_Wait (bool wait)
{
...
void Am_Drawonable_Impl::Unmap_And_Wait ()
{
				      ...
				      int& dest_x_return, int& dest_y_return)
{
  ...
  if (!(XTranslateCoordinates(screen->display,
			      ...
			      &dummy_child_return))) {
   std::cerr << "** Translate_Coordinates: THIS " << this << " and src_d " << src_d
	 << " have different screens" <<std::endl;
    Am_Error ();
  }
}
...
bool Am_Drawonable_Impl::add_wm_border_offset (Window query_drawable,
			   Window expected_parent,
			   int &lb, int &tb, int &rb, int &bb,
  ...
  XQueryTree (screen->display, query_drawable, &root_return,
	      &xlib_parent, &children_return, &nchildren_return);

  // don't need list of children, so free it right away
  if (children_return) XFree((char *)children_return);
  ...
  if (expected_parent == xlib_parent) {
	...
	XGetGeometry (screen->display, query_drawable, &root_return,
		      &outer_left, &outer_top, &w_return, &h_return,
		      &bw_return, &d_return);
...
//    std::cout << "  Outer: " << outer_left << ", " << outer_top << " " <<std::endl;
    ...
    if (xlib_parent) {
      ...
      unsigned int w_return, h_return, bw_return, d_return;
      XGetGeometry (screen->display, query_drawable, &root_return,
		    &x_return, &y_return, &w_return, &h_return,
		    &bw_return, &d_return);
      lb += x_return;
      tb += y_return;
...
//           << query_drawable <<std::endl;
      ...
      return (add_wm_border_offset (xlib_parent, expected_parent,
				    lb, tb, rb, bb,
     ...
     int& outer_left, int& outer_top)
{
  ...
  if (xlib_drawable) {
   ...
   std::cout << "** Asking for border-width of a window before it has" <<std::endl;
   std::cout << "** an xlib_drawable not implemented yet.  Guessing..." <<std::endl;
...
Am_Drawonable_Impl *Get_Drawable_Backpointer (Display *dpy,
					      Window xlib_window) {
  XPointer data_return;
  if (XFindContext (dpy, xlib_window, Backpointer, &data_return))
    return (0L); //XFindContext returns non-zero error codes
  else return (Am_Drawonable_Impl *)data_return;
}

void Set_Drawable_Backpointer (Display *dpy, Window xlib_window,
			       Am_Drawonable_Impl *d) {
				...
				Am_Draw_Function df)
{
  ...
  XCopyArea(disp, src_drawable, dest_drawable, gc, s_left, s_top,
	    width, height, d_left, d_top);
...
void Am_Drawonable_Impl::Set_Cut_Buffer(const char *s)
{
  ...
  if (!s || offscreen) return;  // do nothing with a (0L) string for now.
  ...
  if (screen->cut_data != (0L)) delete[] screen->cut_data;
...
void Screen_Manager::Add (const char *name, Display *dpy,
			  Am_Drawonable *draw)
{
...
void Screen_Manager::Remove (Screen_Manager::Screen_Info* screen)
{
  if (head == screen)
...
void Screen_Manager::Remove (Am_Drawonable* draw)
{
  if (!head)
    return;
...
void Screen_Manager::Remove (const char *name)
{
  if (!head)
...
bool Screen_Manager::Get (const char *name, Display *&dpy,
			  Am_Drawonable *&draw)
{
  if (!head)
    return false;
...
Am_Drawonable* Screen_Manager::Member(Display *dpy) {
  if (!head)
    return (0L);
  Screen_Info *screen = head;
  do {
...
bool Screen_Manager::Pending (XEvent *event_return)
{
  Screen_Info *screen = head;
  do {
...
void Screen_Manager::Wait_For_Event (timeval *timeout)
{
  while (!Pending ())
    Block (timeout);
}

void Screen_Manager::Next_Event (XEvent *event_return, timeval *timeout)
{
...
void Screen_Manager::Put_Event_Back (XEvent& event)
{
...
Am_Drawonable* Am_Drawonable_Impl::Create_Drawonable_From_XWindow (
  ...
  if (!parent) {
    ...
    if (!parent) {
     ...
     std::cerr << "** Warning, Can't find parent for display " << created_display
           << " so created new one " << parent <<std::endl <<std::flush;
    }
  }
  ...
  unsigned int border_w;
  unsigned int bit_depth;
  if (!XGetGeometry (created_display, created_drawable, &dummy_root,
                     &l, &t, &w, &h, &border_w, &bit_depth))
      Am_Error ("** Can't get window geometry.\n");

  Am_Drawonable_Impl* d =
    new /* Am_External_Drawonable_Impl */
    Am_Drawonable_Impl (l, t, w, h, (char*)tit,
		     (char*)icon_tit, vis, initially_iconified, back_color,
		      save_under_flag, min_w, min_h,
		     max_w, max_h, title_bar_flag,
  ...
  set_other_window_properties(d->xlib_drawable, d->screen->display,
			      (char*)tit, (char*)icon_tit, initially_iconified,
			      min_w, min_h, max_w,
			      max_h, query_user_for_position,
			      query_user_for_size);
  return d;
}

Am_Drawonable* Am_Drawonable_Impl::Create_Offscreen_Drawonable_From_XWindow (
	Window created_drawable, Display *created_display,
        Am_External_Event_Handler* ext_ev_handler,
	Am_Style back_color) {
  Window dummy_root;
  ...
  if (!parent_draw) {
   ...
   std::cerr << "** Warning, Can't find parent for display " << created_display
	 << " so created new one " << parent_draw <<std::endl <<std::flush;
  }

  Am_Drawonable_Impl* parent = (Am_Drawonable_Impl*)parent_draw;

  Am_Drawonable_Impl* d = new /* Am_External_Drawonable_Impl */
    Am_Drawonable_Impl (0, 0, w, h,
		     "", "", false, false, back_color, false, 0, 0,
		     0, 0, false, true, parent->screen->depth, (0L));
  d->offscreen = true;
  d->owner = parent;
  d->screen = parent->screen;
  d->ext_handler = ext_ev_handler;
