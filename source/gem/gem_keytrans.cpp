...
unsigned char Am_Input_Char::As_Char () const {
  if(!any_modifier && click_count == Am_NOT_MOUSE && code <= 0xFF) {
    unsigned char ret_val = static_cast<unsigned char>(code);
    if (control)
      ret_val &= 31;
    if (meta) ret_val |= 0x80; //or in high bit
    return ret_val;
  }
  else return 0;
}

bool Am_Input_Char::helper_check_equal (Am_Input_Char i) const {
	...
	if ( ! (click_count == Am_ANY_CLICK || i.click_count == Am_ANY_CLICK ||
	    ...
	    return false;
	// now button down, up
	if ( ! (button_down == Am_ANY_DOWN_UP ||
		i.button_down == Am_ANY_DOWN_UP ||
		button_down == i.button_down))
	    return false;
	//finally, check code
	if ( code == i.code || code == Am_ANY_MOUSE || i.code ==
	     Am_ANY_MOUSE)
	    return true;
	else return false;
    } // end else
...
Am_Input_Char::Am_Input_Char(  short c, bool shf, bool ctrl, bool mt, Am_Button_Down down,
                              Am_Click_Count click, bool any_mod )
{
  if( char_map == 0 || click_count_map == 0 )
    Am_Initialize_Char_Map();

  code = c; shift = shf; control = ctrl; meta = mt;
  button_down = down; click_count = click;
...
Am_Input_Char::Am_Input_Char(const char *s, bool want_error) {
	...
	if ((Am_Click_Count)click_count != Am_NOT_MOUSE) { // then is mouse, check for
	...
	if (temp_code < 0) { // then not found
	  ...
	  if (temp_code < 0) { // then still not found
	    if (want_error) {
		   ...
		   << "' could not be parsed as an Am_Input_Char because couldn't find `" << copy_s << "'\n";
	      if (Find_Char (copy_s, '-'))
		std::cout << "   Maybe because you used a hyphen `-' instead of underscore `_'?\n";
	      Am_Error ();
	    }
    ...
    else shift = Fix_Shift_For_Code(temp_code, shift);
...
void Am_Input_Char::As_String(char *s) const
{
  int index = 0;
  if( shift )
  ...
  else code_string = char_map->Get_Key(code);
  if( code_string == 0 )
  {
...
void Am_Input_Char::As_Short_String (char *s) const
{
    int index = 0;
    if (control) {
...
std::ostream& operator<<( std::ostream& os, Am_Input_Char& i) {
    i.Print(os);
    return os;
}

// this is only used by Mac and Unix
void Am_Input_Event::Set (Am_Input_Char ic, int new_x, int new_y,
...
bool Am_Input_Char::Exit_If_Stop_Char () const {
...
// print the input_char onto the stream.
void Am_Input_Char::Print(std::ostream& os)
{
...
std::ostream& operator<< (std::ostream& os, Am_Drawonable *d) {
  if (d) os << d->Get_Title();
  else os << "(0L) Drawonable";
