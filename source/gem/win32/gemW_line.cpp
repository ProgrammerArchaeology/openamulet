...
void CALLBACK LineDDAProc(int x, int y, LPARAM data)
{
#if !defined(_MSC_VER) || (_MSC_VER >= 1010) /* MSVC 4.1 chokes on this inline asm */
...
Am_WinDDAInfo::Am_WinDDAInfo (HDC hdc, const char* dashes, int ndash, int w, BOOL onoff)
{
	m_hdc = hdc;
...
Am_WinDDAInfo ::~Am_WinDDAInfo ()
{
	delete [] m_dashes;
}

void /*GWFASTCALL*/ Am_WinDrawArrDashLine (HDC hdc, HINSTANCE hinst,
	int x1, int y1, int x2, int y2, const char* dashes, int ndash, int w, BOOL onoff)
{
...
void /*GWFASTCALL*/ Am_WinDrawArrDashPolyline (HDC hdc, HINSTANCE hinst,
	const POINT* ppt, int npt, const char* dashes, int ndash, int w, BOOL onoff)
{
	Am_WinDDAInfo info(hdc, dashes, ndash, w, onoff);
	...
	for (int i = 0; i < npt - 1; i++) {
		LineDDA(ppt[i].x, ppt[i].y, ppt[i + 1].x, ppt[i + 1].y, (LINEDDAPROC)LineDDAProc, (LPARAM)&info);
		if (iseven(info.m_curdash))
