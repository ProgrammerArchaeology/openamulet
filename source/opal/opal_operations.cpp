                     ...
                     bool above)
{
  ...
  if (object == ref_object) {
   std::cerr << "** Am_Move_Object called same object, " << object << ", as both "
      "moved object and reference object." <<std::endl;
    Am_Error ();
  }
  ...
  if (ref_object.Get_Owner () != owner) {
	...
	owner << "and the ref_object " << ref_object << " has owner " <<
        ref_object.Get_Owner () << " they must be the same." <<std::endl;
    Am_Error ();
  }
...
void Am_To_Top (Am_Object object)
{
  ...
  if (object.Is_Instance_Of (Am_Graphical_Object)) {
    ...
    if (draw) draw->Raise_Window((0L));
  }
  else {
   std::cerr << "** Am_To_Top: Attempt to raise " << object << " which is not "
      "a graphical object or window" <<std::endl;
    Am_Error ();
  }
}
...
void Am_To_Bottom (Am_Object object)
{
  ...
  if (object.Is_Instance_Of (Am_Graphical_Object)) {
    ...
    Am_Drawonable* draw = Am_Drawonable::Narrow (object.Get(Am_DRAWONABLE));
    if (draw) draw->Lower_Window((0L));
  }
  else {
   std::cerr << "** Am_To_Bottom: Attempt to lower " << object << " which is not "
      "a graphical object or window" <<std::endl;
    Am_Error ();
  }
}
...
bool Am_Translate_Coordinates(const Am_Object& src_obj, int src_x, int src_y, const Am_Object& dest_obj, int& dest_x, int& dest_y)
{
	...
	{
		if(curr_src.Is_Instance_Of(Am_Graphical_Object) || curr_src.Is_Instance_Of(Am_Window))
		{
	...
	{
		if(curr_dest.Is_Instance_Of(Am_Graphical_Object) ||	curr_dest.Is_Instance_Of(Am_Window))
		{
