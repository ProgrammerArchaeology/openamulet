...
//web functions
bool
line_x_create(const Am_Slot &slot)
{
  return slot.Get_Key() == Am_X1;
}

void
line_x_init(const Am_Slot &slot, Am_Web_Init &init)
{
...
bool
line_y_create(const Am_Slot &slot)
{
  return slot.Get_Key() == Am_Y1;
}

void
line_y_init(const Am_Slot &slot, Am_Web_Init &init)
{
  Am_Object_Advanced obj = slot.Get_Owner();
...
void
line_y_validate(Am_Web_Events &events)
{
  ...
  while (!events.Last()) {
    ...
    case Am_LINE_STYLE:
      break;
    default:
      Am_ERROR("** Bug: unexpected slot in line_y_validate: "
               << Am_Get_Slot_Name(slot.Get_Key()));
    }
    events.Next();
  }
                  ...
                  const Am_Object &ref_obj))
{
  if ((bool)in_obj.Get(Am_VISIBLE)) {
    ...
    float numer =
        (float)(x * y1 - x * y2 + y * x2 - y * x1 + y2 * x1 - y1 * x2);
    float denom = (float)(y1 * y1 - 2 * y1 * y2 + y2 * y2 + x2 * x2 -
                          2 * x2 * x1 + x1 * x1);
...
static Am_Initializer *initializer =
    new Am_Initializer(DSTR("Am_Line"), init, 2.1f, 111, cleanup);
