...
Am_Define_Value_List_Formula(map_make_components)
{
	...
	if(value.type == Am_INT)
	{
  	...
  	{  // any type of 0 is valid: no items.
		std::cerr << "** Value of Am_ITEMS slot in object, " << self << ", is not an integer or Am_Value_List." <<std::endl;
		Am_Error();
	}
...
static Am_Initializer *initializer = new Am_Initializer(DSTR("Am_Map"), init, 2.1f, 112, cleanup);
