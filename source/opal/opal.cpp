...
#endif
static Am_Initializer *inspector_init = new Am_Initializer(
...
AM_DEFINE_ENUM_SUPPORT(Am_Alignment,
                       "Am_CENTER_ALIGN Am_TOP_ALIGN "
...
Am_Drawonable *
Get_a_drawonable(Am_Object obj)
{
  ...
  if (obj.Valid()) {
    ...
    if (window.Valid()) {
      ...
      if (owner.Valid()) {
        drawonable =
            Am_Drawonable::Narrow(window.Get(Am_DRAWONABLE, Am_NO_DEPENDENCY));
        if (drawonable)
          return drawonable;
      }
    }
...
Am_Object
Am_Point_In_Obj(const Am_Object &in_obj, int x, int y, const Am_Object &ref_obj)
{
...
Am_Object
Am_Point_In_Part(const Am_Object &in_obj, int x, int y,
...
Am_Object
Am_Point_In_Leaf(const Am_Object &in_obj, int x, int y,
                 const Am_Object &ref_obj, bool want_self, bool want_groups)
{
  Am_Point_In_Or_Self_Method method;
...
void
Am_Get_String_Extents(const Am_Object &screen, const Am_Font &font,
                      const char *string, int length, int &width, int &height)
{
...
int
Am_Get_String_Width(const Am_Object &screen, const Am_Font &font,
...
void
Am_Get_Font_Properties(const Am_Object &screen, const Am_Font &font,
...
bool
Am_Test_Image_File(const Am_Object &screen, const Am_Image_Array &image)
{
  if (!screen.Valid())
    return false;
...
Am_Object
Am_GV_Screen(const Am_Object &self)
{
  Am_Value v;
  Am_Object screen;
  ...
  if (v.Valid()) {
    screen = v;
    for (; screen.Valid() && !screen.Is_Instance_Of(Am_Screen);
...
void
Am_Draw(Am_Object object, Am_Drawonable *drawonable, int x_offset, int y_offset)
{
...
void
am_generic_renew_components(Am_Object object)
{
