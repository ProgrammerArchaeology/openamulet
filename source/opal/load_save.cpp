...
void standard_save_internal (std::ostream& os,
			     Am_Load_Save_Context& context,
		  ...
		  (std::istream& is, Am_Load_Save_Context& context))
{
  ...
  if (call_proto)
    {
      Am_Object_Method proto_method = loaded_obj.Get (Am_ITEM_METHOD, Am_RETURN_ZERO_ON_ERROR);
      // TODO: Check the following test. It seems proto_method is always Valid...
      if (proto_method.Valid()) proto_method.Call (new_obj);
    }
...
void standard_save_internal (std::ostream& os, Am_Load_Save_Context& context,
			     const Am_Object& object,
			     const char * save_type_name) {
  const char* name;
