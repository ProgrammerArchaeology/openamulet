...
Am_Object bottomin, bottomout, centerX, centerY, leftin, leftout,
...
void set_ref_grey(Am_Object& win, bool grey) {
  win.Get_Object(Am_SHOW_LOC_OBJ).Set(Am_FILL_STYLE,
				      (grey ? Am_Gray_Stipple : Am_Black));
  win.Get_Object(Am_REF_OBJ).Set(Am_LINE_STYLE,
				      (grey ? Am_Gray_Stipple : Am_Line_2));
}
...
Am_Define_Method(Am_Object_Method, void, set_left_object_offset,
  ...
  case am_change_offset_left_outside:
    x = REF_OBJ_LEFT - SHOW_LOC_OBJ_SIZE - 1; break;
  case am_change_offset_left_inside:
    x = REF_OBJ_LEFT + SHOW_LOC_OBJ_OFFSET; break;
  case am_change_offset_right_inside:
    x = REF_OBJ_LEFT + REF_OBJ_SIZE - SHOW_LOC_OBJ_SIZE -
      SHOW_LOC_OBJ_OFFSET; break;
  case am_change_offset_right_outside:
    x = REF_OBJ_LEFT + REF_OBJ_SIZE + 1; break;
  default: Am_ERROR("Bad left control " << (int)cmd.Get(Am_VALUE));
  }
...
Am_Define_Method(Am_Object_Method, void, set_top_offset,
		 (Am_Object cmd)) {
  Am_Object win = cmd.Get_Object(Am_SAVED_OLD_OWNER).Get_Owner();
  am_loc_values* loc_values = (am_loc_values*)(Am_Ptr)win.Get(Am_VALUES);
  loc_values->top_control = win.Get_Object(Am_TOP_HOW).Get(Am_VALUE);
...
Am_Define_Method(Am_Object_Method, void, set_top_object_offset,
		 (Am_Object cmd)) {
  Am_Object win = cmd.Get_Object(Am_SAVED_OLD_OWNER).Get_Owner();
  am_loc_values* loc_values = (am_loc_values*)(Am_Ptr)win.Get(Am_VALUES);
  loc_values->top_control = win.Get_Object(Am_TOP_HOW).Get(Am_VALUE);
  ...
  case am_change_offset_top_outside:
    y = REF_OBJ_TOP - SHOW_LOC_OBJ_SIZE - 1; break;
  case am_change_offset_top_inside:
    y = REF_OBJ_TOP + SHOW_LOC_OBJ_OFFSET; break;
  case am_change_offset_bottom_inside:
    y = REF_OBJ_TOP + REF_OBJ_SIZE - SHOW_LOC_OBJ_SIZE -
      SHOW_LOC_OBJ_OFFSET; break;
  case am_change_offset_bottom_outside:
    y = REF_OBJ_TOP + REF_OBJ_SIZE + 1; break;
  default: Am_ERROR("Bad top control " << (int)cmd.Get(Am_VALUE));
  }
...
Am_Define_Method(Am_Object_Method, void, left_amount_widget_set,
  ...
  switch (left_control) {
  case am_change_equals: loc_values->left_equals = new_val; break;
  case am_change_offset: loc_values->left_offset = new_val; break;
  case am_ask_user_generalize: break; //shouldn't happen
  case am_ask_user_type: break; //shouldn't happen
  case am_change_offset_centered: loc_values->left_centered = new_val; break;
  case am_change_offset_left_outside:
  case am_change_offset_left_inside:
  case am_change_offset_right_inside:
  case am_change_offset_right_outside:
    loc_values->left_object_offset = new_val; break;
  default: Am_ERROR("Bad type for left control " << left_control);
  } //end switch
}
Am_Define_Method(Am_Object_Method, void, top_amount_widget_set,
		 (Am_Object cmd)) {
  Am_Object win = cmd.Get_Object(Am_SAVED_OLD_OWNER).Get_Owner();
  int new_val = cmd.Get(Am_VALUE);
  am_loc_values* loc_values = (am_loc_values*)(Am_Ptr)win.Get(Am_VALUES);
  ...
  switch (top_control) {
  case am_change_equals: loc_values->top_equals = new_val; break;
  case am_change_offset: loc_values->top_offset = new_val; break;
  case am_ask_user_generalize: break; //shouldn't happen
  case am_ask_user_type: break; //shouldn't happen
  case am_change_offset_centered: loc_values->top_centered = new_val; break;
  case am_change_offset_top_outside:
  case am_change_offset_top_inside:
  case am_change_offset_bottom_inside:
  case am_change_offset_bottom_outside:
    loc_values->top_object_offset = new_val; break;
...
Am_Define_Method(Am_Object_Method, void, width_amount_widget_set,
  ...
  case am_change_equals: loc_values->width_equals = new_val; break;
  case am_change_offset: loc_values->width_offset = new_val; break;
  case am_ask_user_generalize: break; //shouldn't happen
  case am_ask_user_type: break; //shouldn't happen
  case am_change_object_wh: loc_values->width_object_offset = new_val; break;
  case am_change_object_percent:
    loc_values->width_object_percent = new_val; break;
...
Am_Define_Method(Am_Object_Method, void, height_amount_widget_set,
  ...
  case am_change_equals: loc_values->height_equals = new_val; break;
  case am_change_offset: loc_values->height_offset = new_val; break;
  case am_ask_user_generalize: break; //shouldn't happen
  case am_ask_user_type: break; //shouldn't happen
  case am_change_object_wh: loc_values->height_object_offset = new_val; break;
  case am_change_object_percent:
    loc_values->height_object_percent = new_val; break;
...
Am_Define_Formula(bool, am_active_if_need_object) {
  ...
  if (self.Get_Sibling(Am_CHANGE_LEFT).Get(Am_VALUE).Valid()) {
    ...
    if (val >= am_change_offset_left_outside &&
	val < am_ask_user_base) return true;
  }
...
Am_Object am_locgen_Initialize () {
  ...
  am_locgen
    ...
    .Add_Part(Am_Text.Create()
      ...
      .Set(Am_FONT, Am_Font(Am_FONT_SERIF, true, false, false, Am_FONT_MEDIUM))
      .Set(Am_LINE_STYLE, Am_Black)
      .Set(Am_FILL_STYLE, Am_No_Style)
    )

    .Add_Part(Am_CHANGE_LEFT, Am_Checkbox.Create(DSTR("Am_CHANGE_LEFT"))
      .Set(Am_LEFT, 22)
      .Set(Am_TOP, 65)
      .Set(Am_FILL_STYLE, Am_Default_Color)
    ...
    .Add_Part(Am_LEFT_HOW, Am_Option_Button.Create(DSTR("Am_LEFT_HOW"))
      .Set(Am_LEFT, 90)
      .Set(Am_TOP, 65)
    ...
    .Add_Part(Am_CHANGE_TOP, Am_Checkbox.Create(DSTR("Am_CHANGE_TOP"))
      .Set(Am_LEFT, 22)
      .Set(Am_TOP, 95)
      .Set(Am_FILL_STYLE, Am_Default_Color)
      .Set_Part(Am_COMMAND, (Am_Command.Create()
    ...
    .Add_Part(Am_TOP_HOW, Am_Option_Button.Create(DSTR("Am_TOP_HOW"))
      .Set(Am_LEFT, 90)
      .Set(Am_TOP, 95)
      .Set(Am_LEAVE_ROOM_FOR_FRINGE, false)
    ...
    .Add_Part(Am_CHANGE_WIDTH, Am_Checkbox.Create(DSTR("Am_CHANGE_WIDTH"))
      .Set(Am_LEFT, 22)
      .Set(Am_TOP, 125)
      .Set(Am_FILL_STYLE, Am_Default_Color)
      .Set_Part(Am_COMMAND, (Am_Command.Create()
    ...
    .Add_Part(Am_WIDTH_HOW, Am_Option_Button.Create(DSTR("Am_WIDTH_HOW"))
      .Set(Am_LEFT, 90)
      .Set(Am_TOP, 125)
      .Set(Am_LEAVE_ROOM_FOR_FRINGE, false)
    ...
    .Add_Part(Am_CHANGE_HEIGHT, Am_Checkbox.Create(DSTR("Am_CHANGE_HEIGHT"))
      .Set(Am_LEFT, 22)
      .Set(Am_TOP, 175)
      .Set(Am_FILL_STYLE, Am_Default_Color)
      .Set_Part(Am_COMMAND, (Am_Command.Create()
    ...
    .Add_Part(Am_HEIGHT_HOW, Am_Option_Button.Create(DSTR("Am_HEIGHT_HOW"))
      .Set(Am_LEFT, 90)
    ...
    .Add_Part(Am_SHOW_LOC_OBJ, Am_Rectangle.Create()
    ...
    .Add_Part(Am_Button_Panel.Create()
      ...
      .Set(Am_ITEMS, Am_Value_List()
	   .Add(Am_Standard_OK_Command.Create()
		.Set(Am_DO_METHOD, am_locgen_ok))
	   .Add(Am_Standard_Cancel_Command.Create()
		.Set(Am_DO_METHOD, am_locgen_cancel))
        )
    )
