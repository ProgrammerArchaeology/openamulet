...
//the replaced value
bool Am_Replace_Placeholders(Am_Object & cmd, Am_Slot_Key slot,
			     bool &did_a_replace, Am_Value &orig_value,
			     Am_Value & new_value,
			     Am_Object & sel_widget) {
  ...
  if (am_sdebug)
   std::cout << "Checking placeholders for cmd " << cmd << " slot "
	 << Am_Am_Slot_Key(slot)
	 << " orig value " << orig_value <<std::endl <<std::flush;
  Am_Value val, ret_val;
  bool ok;
  did_a_replace = false;
  Am_Object ph;
  if (!orig_value.Valid()) return true; //nothing to do, and is OK
	...
	if (val.type == Am_OBJECT &&
	  ...
	  if (ret_val.Valid()) { //otherwise, leave new_list without ret_val
	    ...
	    else new_list.Add(ret_val);
	  }
	}
	else new_list.Add(val);
      }
      did_a_replace = true;
      new_value = new_list;
      if (am_sdebug)
std::cout << "Found placeholders in list and replaced with "
	     << new_value <<std::endl <<std::flush;
    }
  }
  else { //not a list
	...
	new_value = l.Get();
      ...
      did_a_replace = true;
      if (am_sdebug)
std::cout << "Found one placeholder and replaced with "
	     << new_value <<std::endl <<std::flush;
    }
  }
  return true;
...
bool Am_Replace_Create_Placeholders(Am_Object & cmd,
				    bool & replaced_create,
				    Am_Object & sel_widget,
				    Am_Value & new_start_object, 
				    Am_Value & new_loc_value) {
  replaced_create = false;
  Am_Value_List slots_to_save = cmd.Get(Am_SLOTS_TO_SAVE,
  ...
  if (slots_to_save.Valid()) {
	...
	return false;
      if (did_a_replace) {
	if (slot == Am_LOCATION_PLACEHOLDER) new_loc_value = new_value;
	else {
	  if (!start_object.Valid()) {
	    start_object = cmd.Get(Am_START_OBJECT);
	    if (!start_object.Valid())
	      Am_ERRORO("Generalizing Create command " << cmd
			<< " but no start object", cmd, Am_START_OBJECT);
...
bool get_value_from_chain (Am_Value &start_value,
			   Am_Value_List &part_chain, Am_Value &new_value) {
  if (am_sdebug)
   std::cout << "Searching using chain " << part_chain << " from " << start_value
	 <<std::endl <<std::flush;
  ...
  if (am_sdebug)
   std::cout << " part chain found value " << this_value <<std::endl <<std::flush;
...
Am_Define_Method(Am_Placeholder_Set_Value_Method, bool,
  ...
  if (am_sdebug)
   std::cout << "for placeholder " << ph << " setting value to " << v
	 << " from slot " << Am_Am_Slot_Key(slot)
	 << " of " << new_cmd <<std::endl <<std::flush;
  ph.Set(Am_VALUE, v);
  return true;
}
...
bool find_objects_for_placeholders(Am_Slot_Key slot_for_placeholders,
				   Am_Object &cmd, Am_Object &new_cmd,
				   Am_Object & sel_widget) {
  Am_Value_List all_ph = cmd.Get(slot_for_placeholders,
				 Am_RETURN_ZERO_ON_ERROR);
  Am_Object ph;
  bool ok;
...
bool Am_Handle_Placeholders_And_Call_Command(int cnt, Am_Object &cmd,
  ...
  OSTRSTREAM_COPY(oss,line,300);
  if (am_sdebug)
   std::cout << "Doing cmd: " << line << " || " << cmd <<std::endl <<std::flush;
  ...
  if (!cmd.Is_Instance_Of(Am_Object_Create_Command)) {
    ok = Am_Replace_Placeholders(cmd, Am_OBJECT_MODIFIED,
				 replaced_obj_mod, orig_obj_modified,
				 new_obj_mod, sel_widget);
    if (!ok) return false;
  }
  if (cmd.Is_Instance_Of(Am_Object_Create_Command) ||
    ...
    ok = Am_Replace_Placeholders(cmd, Am_OBJECT_MODIFIED,
				 ...
				 new_obj_mod, sel_widget);
    if (!ok) return false;
    ok = Am_Replace_Placeholders(cmd, Am_VALUE,
				 replaced_value, orig_value, new_value,
				 sel_widget);
    if (!ok) {
      return false;
    }
  ...
  if (am_sdebug)
   std::cout << "%% Going to call method on cmd " << cmd <<std::endl <<std::flush;
  ...
  if (replaced_obj_mod || replaced_value || replaced_create || selectdir) {
    ...
    if (am_sdebug)
     std::cout << "%% " << new_method << " on " << cmd << " new_obj "
	   << new_obj_mod << " val " << new_value <<std::endl <<std::flush;
  ...
  if (!find_objects_for_placeholders(Am_PLACEHOLDERS_SET_BY_THIS_COMMAND,
				     cmd, new_cmd, sel_widget)) {
    return false;
  }
  if (Am_Beep_Happened) {
...
Am_Object general_placeholder(Am_Object &new_cmd, const Am_Value& for_value,
			      const char * prompt1, const char * prompt2,
			      bool use_cmd_name,
			      Am_Object& proto, Am_Value_List& part_chain,
			      int &cnt, Am_Slot_Key slot) {
  char line[300];
  OSTRSTREAM_CONSTR (oss,line,300,std::ios::out);
  reset_ostrstream(oss); // go to the beginning
  char placeholder_char = 'A' + cnt++;
  if (prompt1) oss << prompt1;
  oss << placeholder_char;
  if (prompt2) oss << "_" << prompt2;
  if (use_cmd_name) oss << new_cmd.Get(Am_LABEL) << "_command";
  oss << std::ends;
  OSTRSTREAM_COPY(oss,line,300);
  Am_Object placeholder = proto.Create(line)
    .Set(Am_VALUE, for_value)
    .Set(Am_OLD_VALUE, for_value, Am_OK_IF_NOT_THERE) //save original value
    ...
    ;
  if (am_sdebug)
   std::cout << "created placeholder `" << placeholder << "' for cmd " << new_cmd
	 <<std::endl <<std::flush;
  if (new_cmd.Valid()) {
    ...
    new_cmd.Set(Am_PLACEHOLDERS_SET_BY_THIS_COMMAND, old_ph_list,
		Am_OK_IF_NOT_THERE);
    Am_Value_List old_slot_list = new_cmd.Get(Am_SLOTS_TO_SAVE,
					      Am_RETURN_ZERO_ON_ERROR);
...
Am_Define_Method(Am_Placeholder_Create_Method, Am_Object,
  ...
  if (part_chain.Valid()) Am_ERROR("Shouldn't be a part_chain " << part_chain
				   << " for all objects");
  Am_Object placeholder = general_placeholder(create_cmd, for_value,
	"All_Values_", "Returned_by_", true,
...
Am_Define_Method(Am_Placeholder_Create_Method, Am_Object,
		 ...
		 (Am_Object &create_cmd, Am_Value for_value, 
		  Am_Value_List& part_chain, int &cnt, Am_Slot_Key slot)) {
  const char *s;
  if (part_chain.Valid()) s = "Part_of_the_objects_";
  else s = "The_Object_";
  Am_Object placeholder = general_placeholder(create_cmd, for_value,
		s, "Created_by_", true,
...
Am_Object am_call_create_placeholder (Am_Object proto,
  ...
  Am_Placeholder_Create_Method method =
    proto.Get(Am_PLACEHOLDER_CREATE_METHOD);
...
Am_Define_Method(Am_Placeholder_Create_Method, Am_Object,
		 ...
		 (Am_Object & create_cmd, Am_Value for_value, 
		  Am_Value_List& /*part_chain*/, int &cnt,Am_Slot_Key slot)){
  Am_Object placeholder = general_placeholder(create_cmd, for_value,
		"Ask_User_To_Select_Objects_", (0L), false,
...
Am_Define_Method(Am_Placeholder_Create_Method, Am_Object,
		 ...
		 (Am_Object & create_cmd, Am_Value for_value,  
		  Am_Value_List& /* part_chain */, int &cnt,Am_Slot_Key slot)){
  Am_Object placeholder = general_placeholder(create_cmd, for_value,
	 "The_Current_Selected_Objects_", (0L), false,
...
bool pop_up_ask_location(Am_Object & sel_widget, bool growing,
			 Am_Object & ref_obj,
			 int & ask_left, int & ask_top,
  ...
  if (!inter.Valid()) {
    ...
    inter = Am_New_Points_Interactor.Create()
      ...
      .Get_Object(Am_COMMAND)
         ...
         .Get_Owner();
    ref_obj.Add_Part(Am_LOCATION_PLACEHOLDER, inter)
      .Add_Part(feedback);
  }
  ...
  if (ok.Valid()) {
    ...
    if (am_sdebug)
     std::cout << "pop up returned the value " << new_loc <<std::endl <<std::flush;
    Am_Object loc_ref_obj;
    bool as_line;
    new_loc.Translate_To(ref_obj);
    new_loc.Get_Location(as_line, loc_ref_obj, ask_left, ask_top,
		 ask_width, ask_height);
    return true;
    }
  else return false;
}

bool pop_up_ask_type_location(int old_left, int old_top ,
			      int old_width, int old_height,
			      int left_control, int top_control,
			      int width_control, int height_control,
			      int & ask_left, int & ask_top,
			      int & ask_width, int & ask_height) {
  if (am_sdebug)
...
bool get_location_from_object(Am_Object & /*ph*/, Am_Value &other_object,
			       Am_Object &/* ref_obj */,
			       int &obj_left, int &obj_top,
			       int &obj_width, int &obj_height,
			      Am_Object &sel_widget) {
  if (am_sdebug)
   std::cout << "Get location from other object " << other_object <<std::endl <<std::flush;
  ...
  // ****
  if (am_sdebug)
   std::cout << "TEMP using selected object " << obj <<std::endl <<std::flush;
...
Am_Define_Method(Am_Placeholder_Replace_Method, bool,
  ...
  if (ph.Peek(Am_PLACEHOLDER_INITIALIZED).Valid()) {
    ...
    if (am_sdebug)
     std::cout << "old_val " << left << ", " << top << ", "
	   << width << ", " << height << " ref_obj "
	   << ref_obj <<std::endl <<std::flush;

    Am_Value other_object = ph.Get(Am_CONSTANT_OBJECTS_WIDGET,
				   Am_RETURN_ZERO_ON_ERROR);
    bool ok;
    if ( other_object.Valid()) {
      ok = get_location_from_object(ph, other_object, ref_obj,
				obj_left, obj_top, obj_width, obj_height,
				    sel_widget);
      if (!ok) return false;
      if (am_sdebug)
std::cout << "Other object " << obj_left << ", " << obj_top << ", "
	     << obj_width << ", " << obj_height <<std::endl <<std::flush;
    }
	...
	height_control == am_ask_user_generalize) {
       ok = pop_up_ask_location(sel_widget, growing, ref_obj, 
				ask_left, ask_top, ask_width, ask_height,
				width, height);
       if (!ok) return false;
     }
    if (left_control == am_ask_user_type ||
	top_control == am_ask_user_type ||
	width_control == am_ask_user_type ||
	height_control == am_ask_user_type) {
       ok = pop_up_ask_type_location(left, top, width, height,
				     left_control, top_control,
    ...
    case am_change_equals:
      width = width_amount; break;
    case am_change_offset:
      width = width_amount; new_loc.Set_C_Control(Am_LOCATION_RELATIVE); break;
    case am_ask_user_generalize:
    case am_ask_user_type:
      width = ask_width; break;
    case am_change_object_wh:
      width = obj_width + width_amount; break;
    case am_change_object_percent:
      width = (int)(((float)obj_width * width_amount) / 100.0); break;
    default: Am_ERROR("Bad type for width control " << width_control);
    } //end switch

    switch (height_control) {
    case 0: break;
    case am_change_equals:
      height = height_amount; break;
    case am_change_offset:
    ...
    case am_ask_user_generalize:
    case am_ask_user_type:
      height = ask_height; break;
    case am_change_object_wh:
      height = obj_height + height_amount; break;
    case am_change_object_percent:
      height = (int)(((float)obj_height * height_amount) / 100.0); break;
    default: Am_ERROR("Bad type for height control " << height_control);
    } //end switch

    switch (left_control) {
    case 0: break;
    case am_change_equals:
      left = left_amount; break;
    case am_change_offset:
      left = left_amount; new_loc.Set_A_Control(Am_LOCATION_RELATIVE); break;
    case am_ask_user_generalize:
    case am_ask_user_type:
      left = ask_left; break;
    case am_change_offset_left_outside:
      left = obj_left - width - left_amount; break;
    case am_change_offset_left_inside:
      left = obj_left + left_amount; break;
    ...
    case am_change_offset_right_inside:
      left = obj_left + obj_width - width - left_amount; break;
    case am_change_offset_right_outside:
      left = obj_left + obj_width - 1 + left_amount; break;

    default: Am_ERROR("Bad type for left control " << left_control);
    } //end switch

    switch (top_control) {
    case 0: break;
    case am_change_equals:
      top = top_amount; break;
    case am_change_offset:
      top = top_amount; new_loc.Set_B_Control(Am_LOCATION_RELATIVE); break;
    case am_ask_user_generalize:
    case am_ask_user_type:
      top = ask_top; break;
    case am_change_offset_top_outside:
      top = obj_top - height - top_amount; break;
    case am_change_offset_top_inside:
      top = obj_top + top_amount; break;
    case am_change_offset_centered:
      top = obj_top + (int)(((float)(-height) / 2.0) +
			    ((((float)obj_height) * top_amount) / 100.0));
      break;
    case am_change_offset_bottom_inside:
      top = obj_top + obj_height - height - top_amount; break;
    case am_change_offset_bottom_outside:
      top = obj_top + obj_height - 1 + top_amount; break;
    default: Am_ERROR("Bad type for top control " << top_control);
    } //end switch

    if (am_sdebug)
     std::cout << "Computed values " << left << ", " << top << ", " << width
	   << ", " << height << " growing " << growing <<std::endl <<std::flush;
...
Am_Define_Method(Am_Placeholder_Replace_Method, bool,
  ...
  if (ph.Get(Am_PLACEHOLDER_INITIALIZED).Valid()) {
    ...
    if (ok.Valid()) {
      new_value = am_askforvalue.Get_Object(Am_CONSTANT_OBJECTS_WIDGET)
	.Get(Am_VALUES);
      if (am_sdebug)
std::cout << "for placeholder " << ph << " setting value to " << new_value
	     <<std::endl <<std::flush;
...
Am_Define_Method(Am_Placeholder_Create_Method, Am_Object,
  ...
  Am_Object placeholder = general_placeholder(create_cmd, for_value,
		"Get_Value_From_Palette_", s, false,
...
Am_Define_Method(Am_Placeholder_Replace_Method, bool,
  ...
  if (am_sdebug)
   std::cout << "Got value " << new_value << " as index " << count
	 << " from list " << values << " for " << ph <<std::endl <<std::flush;
  return true;
}
...
Am_Define_Method(Am_Placeholder_Create_Method, Am_Object,
  ...
  Am_Object placeholder = general_placeholder(create_cmd, for_value,
		"Any_Object_Of_Type_", type_name, false,
		 Am_Type_Match_Placeholder,
		 Am_No_Value_List, cnt, slot);
...
Am_Define_Method(Am_Placeholder_Replace_Method, bool,
		 type_do_match_placeholder,
		 (Am_Object & ph, Am_Value &new_value,
		  Am_Object & /* sel_widget */)) {
 Am_Object proto = ph.Get(Am_VALUES);
 
 if (new_value.type != Am_OBJECT) return false;
 Am_Object new_object = new_value;
 if (new_object.Is_Instance_Of(proto)) return true;
 else return false;
}
...
int find_index_of_part(Am_Object &obj) {
  ...
  if (!parts.Start_Member(obj))
    Am_ERRORO("part " << obj << " not in graphical parts of its owner " <<
	      owner, owner, Am_GRAPHICAL_PARTS);
  //current index
  return parts.Nth();
}
...
//the list.
bool check_obj_or_owners(Am_Object& part, Am_Object & owner,
		  Am_Value_List &part_chain, int index) {
  if (part == owner) {
    if (index >= 0) part_chain.Add(index);
    return true;
  }
  ...
  if (index >= 0) part_chain.Add(index, Am_HEAD);
  if (am_sdebug)
   std::cout << "Part " << part << " owner " << owner << " chain " << part_chain
	 <<std::endl <<std::flush;
  return true;
}

bool find_in_assoc_list_member_or_owner(Am_Object & obj, Am_Value_List &list,
					Am_Object &value_2,
					Am_Value_List &part_chain) {
  Am_Assoc as;
...
Am_Object check_assoc_2_or_part(Am_Object &obj,
				Am_Value_List &placeholder_list,
  ...
  if (am_sdebug)
   std::cout << "looking for " << obj  << " in placeholder_list "
	 ...
	 << created_objects_assoc <<std::endl <<std::flush;
  //first check if obj is already in the list
  Am_Object ph = placeholder_list.Assoc_2(obj);
  Am_Object create_cmd;
  if (ph.Valid()) return ph;
  Am_Value obj_val;
  obj_val = obj;
  Am_Value_List part_chain;
  if (find_in_assoc_list_member_or_owner(obj, placeholder_list, ph,
					 part_chain)) {
    Am_Value_List new_part_chain = ph.Get(Am_PART_CHAIN,
					  Am_RETURN_ZERO_ON_ERROR);
    if (am_sdebug)
     std::cout << "found as part of placeholder " << ph << " existing chain "
	   << new_part_chain << " new chain " << part_chain <<std::endl <<std::flush;

    if (new_part_chain.Valid()) new_part_chain.Append(part_chain);
    else new_part_chain = part_chain;
    ...
    if (am_sdebug)
     std::cout << "found as part of list created by " << create_cmd
	   << " part chain " << part_chain <<std::endl <<std::flush;
    ph = am_call_create_placeholder(Am_An_Object_From_Command_Placeholder,
				    obj_val, create_cmd, placeholder_list,
				    part_chain, cnt, slot); 
    if (am_sdebug)
...
bool check_one_obj_modified(Am_Value &obj_modified, Am_Slot_Key slot,
  ...
  Am_Value v, ret;
  Am_Value_List objs;
  if (!obj_modified.Valid()) return false;
  if (Am_Value_List::Test(obj_modified)) objs = obj_modified;
  else objs.Add(obj_modified);
  ...
  if (placeholder.Valid()) {
    ...
    if (am_sdebug)
     std::cout << "found whole group placeholder for " << obj_modified << " is "
	   << placeholder <<std::endl <<std::flush;
    ret = placeholder;
  }
  else {
    ...
    if (create_cmd.Valid()) {
      v = objs;
      if (am_sdebug)
std::cout << " checking placeholder_list " << placeholder_list << std::endl
	     <<std::flush;
      placeholder =
	am_call_create_placeholder(Am_All_Values_From_Command_Placeholder,
				   v, create_cmd, placeholder_list,
				   Am_No_Value_List, cnt, slot);
      ret = placeholder;
    }
	...
	obj = objs.Get();
	placeholder =
	  check_assoc_2_or_part(obj, placeholder_list, created_objects_assoc,
				cnt, slot);
	if (placeholder.Valid()) {
	...
	else copy_of_obj_modified.Add(obj);
      }
      if (found_one)
...
void am_check_obj_modified(Am_Object &in_cmd, Am_Value_List &placeholder_list,
			   Am_Value_List &created_objects_assoc, int &cnt) {
...
void am_check_start_obj_parameters(Am_Object &in_cmd,
				   Am_Value_List &placeholder_list,
				   Am_Value_List &created_objects_assoc,
				   int &cnt) {
  ...
  if (start_object.Valid()) {
	...
	if (am_sdebug)
	 std::cout << "checking slot " << slot << " oldval " << v
		 <<std::endl <<std::flush;
...
Am_Object get_objgen_window() {
  Am_Object w = am_get_db_from_list(am_objgen, objgen_windows,
				    num_vis_objgen_windows);
  w.Get_Object(Am_CONSTANT_OBJECTS_WIDGET)
    .Set(Am_VALUE, "");
  w.Get_Object(Am_INDEXES_OF_COMMANDS_WIDGET)
    .Set(Am_VALUE, "");
  w.Get_Object(Am_SOME_OBJECTS_WIDGET)
    .Set(Am_VALUE, "");
  w.Get_Object(Am_ALL_FROM_COMMAND_WIDGET)
    .Set(Am_VALUE, "");
 
  return w;
}
void done_with_objgen_window(Am_Object sw) {
...
bool am_find_cmd_in_sel(Am_Value_List &sel_list, Am_Object &target_cmd,
			Am_Object &found_sel) {
  Am_Object this_sel;
...
Am_Value_List sort_sel_list_cmd_order(Am_Value_List &sel_list,
  ...
  if (modify_first_only) {
    ...
    if (am_sdebug)
     std::cout << "Returning singleton line list " << new_list <<std::endl <<std::flush;
    return new_list;
  }
  ...
  if (new_list.Length() != sel_list.Length())
    Am_ERROR("sort didn't work, orig " << sel_list << " new " << new_list
	     << " all cmds " << orig_cmds);
  if (am_sdebug)
   std::cout << "Returning sorted line list " << new_list <<std::endl <<std::flush;
  return new_list;
}

void replace_value_in_chain (Am_Object & cmd, Am_Slot_Key slot,
			     Am_Value &old_val, Am_Value_List &part_chain,
			     Am_Value new_value) {
  ...
  for (part_chain.Start(); !part_chain.Last(); part_chain.Next()) {
	...
	list = this_value;
    else Am_ERROR("Element not a list " << this_value);
    this_value = list.Get_Nth(key);
  } //end loop
  ...
  if (am_sdebug)
   std::cout << "__Constructed new list " << new_list << " from old " << old_list
	 << " using partchain " << part_chain << " and new value "
	 << new_value <<std::endl <<std::flush;
...
void am_script_replace_all(Am_Value_List &sel_list,
			Am_Value &new_val,
  ...
  for (sel_list.Start(); !sel_list.Last(); sel_list.Next()) {
    ...
    if (part_chain.Valid())
      ...
      if (am_sdebug)
	std::cout << "Setting " << cmd << " slot " << slot << " old_val " << old_val
	         <<std::endl <<std::flush;
      if (Am_Value_List::Test(old_val) && !Am_Value_List::Test(new_val)) {
			new_val_list = Am_Value_List().Add(new_val);
			if (am_sdebug)
			 std::cout << "Creating list, set with " << new_val_list <<std::endl <<std::flush;
...
bool obj_or_objects(Am_Value &v) {
  ...
  if (v.type == Am_OBJECT) return true;
  else if (Am_Value_List::Test(v)) {
    Am_Value_List l = v;
    Am_Value lv;
    for (l.Start(); !l.Last(); l.Next()) {
      lv = l.Get();
      if (lv.type != Am_OBJECT) return false;
    }
    return true;
  }
  else return false;  
...
Am_Define_Method(Am_Object_Method, void, am_objgen_ok,
		 (Am_Object cmd)) {
  ...
  sel_list = sort_sel_list_cmd_order(sel_list, execute_command,
				     modify_first_only);
  ...
  case am_constant_generalize:
    old_val = objgen_window.Get_Object(Am_CONSTANT_OBJECTS_WIDGET)
      .Get(Am_VALUES);
    proto = Am_Constant_Placeholder; break;
  case am_selected_generalize:
    proto = Am_The_Selected_Objects_Placeholder; break;
  case am_ask_user_generalize: 
    proto = Am_User_Select_At_Run_Time_Placeholder; break;
  ...
  if (orig_cmd.Valid() && slot != Am_OBJECT_MODIFIED) {  // hack
    ...
    Am_Value_List old_slot_list = orig_cmd.Get(Am_SLOTS_TO_SAVE,
					       Am_RETURN_ZERO_ON_ERROR);
    old_slot_list.Append_New(slot);
    if (am_sdebug)
     std::cout << "Adding slot " << slot << " to list " << old_slot_list
	   << " for " << orig_cmd <<std::endl <<std::flush;
  ...
  Am_Value new_value;
  new_value = new_ph;
  am_script_replace_all(sel_list, new_value, script_window,
			slot, execute_command);
  done_with_objgen_window(objgen_window);
}

Am_Define_Method(Am_Object_Method, void, am_objgen_cancel,
		 (Am_Object cmd)) {
  Am_Object objgen_window = cmd.Get_Object(Am_SAVED_OLD_OWNER).Get_Owner();
  done_with_objgen_window(objgen_window);
}
...
Am_Define_Method(Am_Object_Method, void, am_load_current_command,
  ...
  if (!l.Valid()) {
   ...
   AM_POP_UP_ERROR_WINDOW("Select only one script line in the Script Window");
    ...
    Am_String s = am_gen_command_label(cmd);
    if (am_sdebug)
     std::cout << " for command " << cmd << " label is " << s <<std::endl <<std::flush;
	...
	.Set(Am_VALUE, s);
    else objgen_window.Get_Object(Am_SOME_OBJECTS_WIDGET)
	.Set(Am_VALUES, cmd, Am_OK_IF_NOT_THERE)
	.Set(Am_VALUE, s);
  }
}

bool am_get_placeholder_from(Am_Value &current_val, Am_Object &ph) {
  ph = Am_No_Object;
  Am_Value val;
...
void put_part_chain(Am_Object &objgen_window, Am_Object &ph) {
  char line[300];
  OSTRSTREAM_CONSTR (oss,line,300,std::ios::out);
  reset_ostrstream(oss); // go to the beginning
  Am_Value_List part_chain = ph.Get(Am_PART_CHAIN, Am_RETURN_ZERO_ON_ERROR);
  int index;
  if (!part_chain.Valid()) oss << "1";
  else {
    part_chain.Start();
...
void popup_generalize_script_objects(Am_Object &main_script_line_part,
...
std::cout << "setting value " << orig_value << " type "
	     << orig_value.type <<std::endl <<std::flush;
...
Am_Object get_locgen_window() {
  Am_Object w = am_get_db_from_list(am_locgen, locgen_windows,
				    num_vis_locgen_windows);
  return w;
}
void done_with_locgen_window(Am_Object sw) {
  sw.Set(Am_VISIBLE, false);
  locgen_windows.Add(sw);
  num_vis_locgen_windows--;
}

Am_Define_Method(Am_Object_Method, void, am_locgen_ok,
		 (Am_Object cmd)) {
  ...
  Am_Object new_ph = am_call_create_placeholder(Am_Inter_Location_Placeholder,
				   old_val, create_cmd, placeholder_list,
  ...
  if ((bool)locgen_window.Get_Object(Am_CONSTANT_OBJECTS_WIDGET).Get(Am_ACTIVE)) {
    //*** TEMP //should be value from that slot
    new_ph.Set(Am_CONSTANT_OBJECTS_WIDGET, new_ph, Am_OK_IF_NOT_THERE);
  }
  else new_ph.Set(Am_CONSTANT_OBJECTS_WIDGET, Am_No_Object,
		  Am_OK_IF_NOT_THERE);
...
Am_Define_Method(Am_Object_Method, void, am_locgen_cancel,
		 (Am_Object cmd)) {
  Am_Object locgen_window = cmd.Get_Object(Am_SAVED_OLD_OWNER).Get_Owner();
  done_with_locgen_window(locgen_window);
}
...
am_loc_values* create_loc_values(Am_Inter_Location &current_loc,
				 Am_Value &old_val) {
  ...
  if (as_line) Am_Error("Shouldn't be as_line");    
  ...
  if (old_val.Valid()) {
    if (Am_Inter_Location::Test(old_val)) {
      ...
      if (as_line) {
std::cout << "**old value is as line " << old_loc <<std::endl <<std::flush;
      }
      else {
	if (oref_obj != ref_obj)
	...
	ret->height_offset = d - od;
     ...
     std::cout << "**Old val not an inter location " << old_val <<std::endl <<std::flush;
    }
  }
  return ret;
}
...
bool set_location_window_from_val(Am_Object &locgen_window,
				  Am_Inter_Location &current_val,
				  Am_Value& old_val) {
  bool as_line;
  int a, b, c, d;
  Am_Object ref_obj;
...
void popup_generalize_script_location(Am_Object &main_script_line_part,
				      Am_Object &script_window,
				      Am_Value_List &sel_list) {
  Am_Value current_val = main_script_line_part.Get(Am_VALUE);
  Am_Object main_cmd = main_script_line_part.Get(Am_COMMAND);
  ...
  if (ph.Valid()) {
    ...
    else loc_values = create_loc_values(orig_loc, old_val);
    locgen_window.Set(Am_VALUES, (Am_Ptr)loc_values);
    
    int control = ph.Get(Am_LEFT_HOW);
    ok = (control ? 1 : 0);
    locgen_window.Get_Object(Am_CHANGE_LEFT).Set(Am_VALUE, ok);
    if (!control) control = am_change_equals;
...
Am_Object get_valgen_window() {
  Am_Object w = am_get_db_from_list(am_valgen, valgen_windows,
				    num_vis_valgen_windows);
  return w;
}
void done_with_valgen_window(Am_Object sw) {
  sw.Set(Am_VISIBLE, false);
...
Am_Define_Method(Am_Object_Method, void, am_valgen_ok,
		 (Am_Object cmd)) {
  ...
  switch (how_gen) {
  case am_constant_generalize:
    old_val = valgen_window.Get_Object(Am_CONSTANT_OBJECTS_WIDGET)
      .Get(Am_VALUES);
    proto = Am_Constant_Placeholder; break;
  ...
  case am_next_value_generalize:
    ...
    break;
  case am_ask_user_generalize: 
    proto = Am_Ask_User_Property_Placeholder; break;
  case am_all_values_generalize:
  ...
  Am_Object new_ph = am_call_create_placeholder (proto,
				   ...
				   part_chain, placeholder_cnt, slot);
  if (am_sdebug)
   std::cout << "ValGen: New placeholder is " << new_ph <<std::endl <<std::flush;
  execute_command.Set(Am_PLACEHOLDER_LIST, placeholder_list);
  execute_command.Set(Am_PLACEHOLDER_COUNT, placeholder_cnt);
  Am_Value new_value;
  new_value = new_ph;
...
Am_Define_Method(Am_Object_Method, void, am_valgen_cancel,
		 (Am_Object cmd)) {
  Am_Object valgen_window = cmd.Get_Object(Am_SAVED_OLD_OWNER).Get_Owner();
  done_with_valgen_window(valgen_window);
}
...
Am_Object find_palette_for_command(Am_Object &main_cmd,
				   Am_Value &current_value,
				   Am_Object &script_window) {
  Am_Value slot_key_value = main_cmd.Peek(Am_SLOT_FOR_VALUE);
  Am_Am_Slot_Key slot;
  if (!slot_key_value.Valid()) slot = 0;
  else slot = slot_key_value;
  ...
  if (am_sdebug)
   std::cout << "searching for palette for undo " << undo_handler << " from "
	 << undo_db << " from " << script_window << " slot "
	 << slot << " value " << current_value <<std::endl <<std::flush;
...
void popup_generalize_script_value(Am_Value & current_val,
  ...
  if (!list_val.Valid()) list_val.Add(current_val);
  ...
  valgen_window.Get_Object(Am_NEXT_CONSTANT_OBJECTS_WIDGET)
    ...
    .Set(Am_VALUES, list_val)
      ;
  Am_Object palette = find_palette_for_command(main_cmd, current_val,
					       script_window);
...
void Am_Pop_Up_Generalize_DB(Am_Object &script_window,
			     Am_Value_List &sel_list) {
  sel_list.Start();
  ...
  if (am_sdebug)
   std::cout << "sel " << main_script_line_part << " slot " << key <<std::endl<<std::flush;
  if (key == Am_OBJECT_MODIFIED) {
	...
	(value.type == Am_OBJECT &&
	 Am_Object(value).Is_Instance_Of(Am_Inter_Location_Placeholder))) {
      popup_generalize_script_location(main_script_line_part,
...
std::cout << "for placeholder " << obj
	     << " found old val " << old_val <<std::endl <<std::flush;
	if (old_val.type == Am_OBJECT)
	  popup_generalize_script_objects(main_script_line_part, script_window,
					sel_list);
	else popup_generalize_script_value(value, main_script_line_part,
					   script_window, sel_list);
      } 
    } 
    else // not an object
...
void Am_Scripting_Register_Palette(Am_Object undo_handler,
				   Am_Object palette,
				   Am_Value slot_or_type) {
  if (Am_Am_Slot_Key::Test(slot_or_type)) {
    palette.Set(Am_SLOT_FOR_VALUE, slot_or_type, Am_OK_IF_NOT_THERE);
  }
  Am_Value_List palette_registry = undo_handler.Get(Am_REGISTRY_FOR_PALETTES,
						    Am_RETURN_ZERO_ON_ERROR);
...
Am_Object Am_Find_Palette_For(Am_Object& undo_handler,
...
void Am_Initialize_Generalizers() {
  
   Am_A_Placeholder = Am_Root_Object.Create(DSTR("Am_A_Placeholder"))
    .Add(Am_VALUE, (0L))
    .Add(Am_ID, 0)
    .Add(Am_LABEL, (0L))
   ...
   Am_Constant_Placeholder = Am_A_Placeholder.Create
     ("Am_Constant_Placeholder")
   ...
   Am_All_Values_From_Command_Placeholder = Am_A_Placeholder.Create
   ...
   Am_An_Object_From_Command_Placeholder = Am_A_Placeholder.Create
   ...
   Am_The_Selected_Objects_Placeholder = Am_A_Placeholder.Create
     ("Am_The_Selected_Objects_Placeholder")
    ...
    ;
   Am_User_Select_At_Run_Time_Placeholder = Am_A_Placeholder.Create
     ("Am_User_Select_At_Run_Time_Placeholder")
    .Set(Am_PLACEHOLDER_REPLACE_METHOD, user_select_objs_replace_placeholder)
    .Set(Am_PLACEHOLDER_SET_VALUE_METHOD, (0L))
    .Set(Am_PLACEHOLDER_CREATE_METHOD, user_select_placeholder_create)
    ;
   Am_Inter_Location_Placeholder = Am_A_Placeholder.Create
   ...
   Am_Ask_User_Property_Placeholder = Am_A_Placeholder.Create
     ("Am_Ask_User_Property_Placeholder")
    .Set(Am_PLACEHOLDER_REPLACE_METHOD, ask_user_value_replace_placeholder)
    .Set(Am_PLACEHOLDER_SET_VALUE_METHOD, (0L))
    .Set(Am_PLACEHOLDER_CREATE_METHOD, ask_user_value_placeholder_create)
    ;
   Am_Property_From_Palette_Placeholder = Am_A_Placeholder.Create
     ("Am_Property_From_Palette_Placeholder")
   ...
   Am_Cycle_Values_Placeholder = Am_A_Placeholder.Create
     ("Am_Cycle_Values_Placeholder")
    .Set(Am_PLACEHOLDER_REPLACE_METHOD,
	 cycle_values_replace_placeholder)
    .Set(Am_PLACEHOLDER_SET_VALUE_METHOD, (0L))
   ...
   Am_Type_Match_Placeholder = Am_A_Placeholder.Create
     ("Am_Type_Match_Placeholder")
    .Set(Am_PLACEHOLDER_REPLACE_METHOD, value_replace_placeholder)
    .Set(Am_PLACEHOLDER_SET_VALUE_METHOD, (0L))
   ...
   Am_Any_Value_Placeholder = Am_A_Placeholder.Create
     ("Am_Any_Value_Placeholder")
    .Set(Am_PLACEHOLDER_REPLACE_METHOD, value_replace_placeholder)
    .Set(Am_PLACEHOLDER_SET_VALUE_METHOD, (0L))
