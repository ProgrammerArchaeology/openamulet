...
void
calc_position(Am_Object inter, Am_Object obj, const Am_Object &ref_obj, int x,
              int y, int &out_x, int &out_y)
{
  ...
  default:
    Am_ERRORO("Bad Am_WHERE_ATTACH " << attach << " for move on inter "
                                     << inter,
  ...
  Am_INTER_TRACE_PRINT(inter, "    calc_position attach="
                                  << attach << " x,y=(" << x << "," << y
...
void
calc_line_position(Am_Object inter, const Am_Object &ref_obj, int x, int y,
                   int &out_x1, int &out_y1, int &out_x2, int &out_y2)
{
  ...
  default:
    std::cerr << "** Amulet Error: Bad Am_WHERE_ATTACH " << attach
              << " for move on inter " << inter;
    Am_Error();
    break;
  }
  Am_INTER_TRACE_PRINT(inter, "    calc_line_position attach="
                                  << attach << " x,y=(" << x << "," << y
...
static void
calc_size(Am_Object &inter, const Am_Object &obj, const Am_Object &ref_obj,
          int x, int y, int &out_x, int &out_y, int &out_width, int &out_height)
{
  Am_Move_Grow_Where_Attach attach = inter.Get(Am_WHERE_ATTACH);
  ...
  case Am_ATTACH_SW_val:
  case Am_ATTACH_W_val:
    if (rightp1 - x <= min_width)
      out_x = rightp1 - min_width;
    else
      out_x = x;
  ...
  case Am_ATTACH_E_val:
    ...
    break;
  default:
    Am_ERRORO("Bad Am_WHERE_ATTACH " << attach << " for grow on inter "
                                     << inter,
              inter, Am_WHERE_ATTACH);
    break;
  }
  ...
  case Am_ATTACH_NE_val:
  case Am_ATTACH_N_val:
    if (bottomp1 - y <= min_height)
      out_y = bottomp1 - min_height;
    else
      out_y = y;
  ...
  Am_INTER_TRACE_PRINT(inter, "    calc_size attach="
                                  << attach << " x,y=(" << x << "," << y
                                  << ") out x,y,width,height=(" << out_x << ","
                                  << out_y << "," << out_width << ","
                                  << out_height << ")");
}
...
// have changed
static void
calc_line_end_move(Am_Object &inter, const Am_Object &ref_obj, int x, int y,
                   int &out_x1, int &out_y1, int &out_x2, int &out_y2)
{
  Am_Move_Grow_Where_Attach attach = inter.Get(Am_WHERE_ATTACH);
  ...
  if (min_length) { // time for expensive math
    ...
    case Am_ATTACH_END_2_val:
      ...
      break;
    default:
      Am_ERRORO("Bad Am_WHERE_ATTACH "
                    << attach << " for move line end on inter " << inter,
                inter, Am_WHERE_ATTACH);
      break;
    }
  ...
  Am_INTER_TRACE_PRINT(inter, "    calc_move_line_end attach="
                                  << attach << " x,y=(" << x << "," << y
                                  << ") out x1,y1,x2,y2=(" << out_x1 << ","
                                  << out_y1 << "," << out_x2 << "," << out_y2
                                  << ")");
}

static Am_Object
translate_if_needed(const Am_Object &inter, const Am_Object &obj, int &x,
                    int &y, const Am_Object &ref_obj)
{
  Am_Object owner = obj.Get_Owner();
  if (ref_obj != owner) {
    Am_INTER_TRACE_PRINT(inter, "Translating coordinates from "
                                    << ref_obj << " to " << owner);
...
//   sets all four
static void
calc_all(Am_Object &inter, Am_Object obj, int x, int y,
...
void
set_line_initial_where_hit_attach(Am_Object inter, int first_x, int first_y,
                                  int x1, int y1, int x2, int y2, bool growing)
{
  if (growing) {
    ...
    if (d1 < d2)
      attach = Am_ATTACH_END_1;
    else
      attach = Am_ATTACH_END_2;
...
void
set_not_line_initial_where_hit_attach(Am_Object inter, int first_x, int first_y,
...
// returns orig_points
Am_Inter_Location
set_initial_values(Am_Object inter, Am_Object obj, int first_x, int first_y,
                   Am_Object ref_obj)
{
  ...
  if (Am_Type_Class(inter.Get_Slot_Type(Am_OLD_VALUE)) == Am_WRAPPER) {
    ...
    allocated_new_orig = true;

  bool growing = inter.Get(Am_GROWING);
  bool as_line = inter.Get(Am_AS_LINE);
  bool where_hit_attach = inter.Get(Am_WHERE_ATTACH) == Am_ATTACH_WHERE_HIT;
  Am_Object owner = obj.Get_Owner();
  if (where_hit_attach) // convert firstX and Y to be same coords as x1,y1
    Am_Translate_Coordinates(ref_obj, first_x, first_y, owner, first_x,
                             first_y);
  ...
  if (as_line) {
    ...
    if (where_hit_attach)
      set_line_initial_where_hit_attach(inter, first_x, first_y, x1, y1, x2, y2,
                                        growing);
  } else { // not a line
    int left = obj.Get(Am_LEFT);
    int top = obj.Get(Am_TOP);
    ...
    if (where_hit_attach)
      set_not_line_initial_where_hit_attach(inter, first_x, first_y, left, top,
                                            width, height, growing);
  }
  ...
  if (allocated_new_orig)
    inter.Set(Am_OLD_VALUE, orig_points, Am_OK_IF_NOT_THERE);
  else
    inter.Note_Changed(Am_OLD_VALUE);
...
static Am_Inter_Location
move_grow_interim_val(Am_Object &inter, Am_Object &command_obj,
                      Am_Object &object_to_move, int x, int y,
                  ...
                  Am_Input_Event *ev))
{
  ...
  Am_Inter_Location data =
      set_initial_values(inter, object, x, y, event_window);
                  ...
                  Am_Object &event_window, Am_Input_Event *ev))
{
  ...
  Am_Inter_Location data = move_grow_interim_val(
      inter, command_obj, object, x, y, event_window, ev->input_char);
}
...
void
Am_Move_Grow_Register_For_Undo(Am_Object inter)
{
  Am_Object command_obj = inter.Get_Object(Am_COMMAND);
...
void
Am_Report_Set_Vis(Am_Object inter, Am_Object obj, bool value)
{
  std::cout << "++Object " << inter << " setting Am_VISIBLE of " << obj
            << " to ";
  if (value)
    std::cout << "true\n" << std::flush;
  else
...
void
Am_Modify_Object_Pos(Am_Object &obj, const Am_Inter_Location &data,
                     bool growing)
{
...
void
Am_Set_Feedback_As_Object_Line(Am_Object feedback, Am_Object object_modified)
{
  int x1, y1, x2, y2;
...
Am_Object
Am_Check_And_Fix_Feedback_Group(Am_Object &feedback, const Am_Object &inter)
{
  ...
  if (Am_Value_List::Test(v))
    ...
    Am_ERRORO("Am_MULTI_FEEDBACK_OWNERS slot of "
                  << inter << " must be list of objects but is " << v,
  ...
  for (fb_owner_list.Start(), owner_list.Start(); !fb_owner_list.Last();
       fb_owner_list.Next(), owner_list.Next()) {
  ...
  if (fb_owner.Valid()) {
    Am_INTER_TRACE_PRINT(Am_INTER_TRACE_SETTING,
                         "Inter " << inter << " moving feedback object "
                                  << feedback << " to new owner " << fb_owner
                                  << " because mouse moved from prev owner "
                                  << feedback.Get_Owner() << " to "
                                  << fb_owner);
  } else {
    Am_INTER_TRACE_PRINT(
        inter, "Inter " << inter << " moved object over window " << event_window
...
Am_Object
Am_Find_Destination_Group(const Am_Object &obj, const Am_Object &inter)
{
  // if we are moving a top-level window, don't need to do anything
...
Am_Object
Am_Find_Window_At_Cursor(Am_Object some_window)
{
...
Am_Object
Am_Check_And_Fix_Point_In_Feedback_Window(Am_Object &inter, Am_Object &feedback)
{
  ...
  if (inter_window == feedback) {
    ...
    if (new_window.Valid()) {
      ...
      int y = inter.Get(Am_INTERIM_Y);
      int newx, newy;
      Am_INTER_TRACE_PRINT(inter, "Translating coordinates of "
                                      << inter << " from " << inter_window
                                      << " to " << new_window);
...
bool
Am_Check_Final_Point_In_Multi(Am_Object inter)
{
  Am_Value v;
  Am_Value_List owner_list;
  v = inter.Peek(Am_MULTI_OWNERS);
  if (v.Valid()) {
    if (Am_Value_List::Test(v))
      ...
      return true; //MULTI_OWNER = true, so just return true
  } else
    return true; //not a multi-window
...
//test for a frequent bug:
    ...
    bool growing = inter.Get(Am_GROWING);
    bool as_line =
        (bool)inter.Get(Am_AS_LINE) && (bool)feedback.Get(Am_AS_LINE);
                  ...
                  Am_Inter_Location data))
{
  ...
  if (obj.Valid()) { //don't switch windows if growing
    if (!growing)
      Am_Check_And_Fix_Feedback_Group(obj, inter);
...
bool
Am_Check_And_Fix_Owner_For_Object(Am_Object &obj, Am_Inter_Location &data)
{
  Am_Object owner = obj.Get_Owner();
  Am_Object new_owner = data.Get_Ref_Obj();
  if (owner != new_owner) {
    Am_INTER_TRACE_PRINT(Am_INTER_TRACE_SETTING,
                         "Moving object " << obj << " to new owner "
                                          << new_owner << " from " << owner);
    obj.Remove_From_Owner(); //make the object no longer be a part of its old
                             //place
...
//command obj is a copy of the original, so can modify it
Am_Define_Method(Am_Object_Method, void, Am_Move_Grow_Command_Repeat_Same,
                 (Am_Object command_obj))
{
  //assume object has moved, so no longer where the command object put it, so
  //put it there again, and save current place in case undone.
                  ...
                  Am_Value new_value))
{
  ...
  if (new_selection.Valid()) {
    if (new_selection.type == Am_OBJECT)
      ...
      Am_ERRORO("Am_Move_Grow_Command_Repeat_New called with non-object "
                    << new_selection,
                command_obj, 0);
  }
...
init()
{
  ...
  Am_Move_Grow_Interactor =
      Am_Interactor.Create(DSTR("Am_Move_Grow_Interactor"))
          ...
          .Add_Part(Am_IMPLEMENTATION_COMMAND,
                    Am_Move_Grow_Internal_Command.Create(
                        DSTR("Move_Grow_Internal_Command")));
...
static Am_Initializer *initializer =
    new Am_Initializer(DSTR("Inter_Move_Grow"), init, 3.3f);
