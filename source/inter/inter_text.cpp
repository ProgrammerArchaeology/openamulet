...
void
move_text_cursor(Am_Object text, int x, int y)
{
...
Am_Object
text_set_impl_command(Am_Object inter, Am_Object object_modified,
...
Am_Text_Abort_Or_Stop_Code
am_check_text_legal(Am_Object &inter, Am_Object &text)
{
                  ...
                  Am_Object &event_window, Am_Input_Event *ev))
{
  ...
  Am_Object command_obj = inter.Get(Am_COMMAND);
  Am_Inter_Call_Both_Method(inter, command_obj, Am_INTERIM_DO_METHOD, ev->x,
                            ev->y, event_window, ev->input_char, object,
                            Am_No_Location);
}
                  ...
                  Am_Object &event_window, Am_Input_Event *ev))
{
  ...
  case Am_TEXT_STOP_ANYWAY: {
    ...
    Am_Call_Final_Do_And_Register(inter, command_obj, ev->x, ev->y,
                                  ...
                                  Am_No_Location, text_set_impl_command);
...
Am_Define_Method(Am_Object_Method, void, Am_Text_Abort_Do, (Am_Object inter))
{
  Am_Object text = inter.Get(Am_OBJECT_MODIFIED);
  if (text.Valid()) {
...
{
  Am_Object ret =
      Am_Inter_In_Text_Object_Or_Part.Call(inter, object, event_window, x, y);
  if (ret.Valid())
    return Am_No_Object;
  else {
...
Am_Define_Method(Am_Where_Method, Am_Object, Am_Inter_Not_In_Text_Leaf,
                 (Am_Object inter, Am_Object object, Am_Object event_window,
                  int x, int y))
{
  Am_Object ret = Am_Inter_In_Text_Leaf.Call(inter, object, event_window, x, y);
  if (ret.Valid())
    return Am_No_Object;
  else {
    if (Am_Point_In_All_Owners(object, x, y, event_window))
...
Am_Define_Method(Am_Selective_Repeat_New_Method, void,
                 ...
                 (Am_Object command_obj, Am_Value /* new_selection */,
                  Am_Value new_value))
{
  am_new_points_general_undo_redo(command_obj, false, true, true, Am_No_Object,
                                  new_value);
}
...
static Am_Initializer *initializer =
    new Am_Initializer(DSTR("Inter_Text"), init, 3.4f);
