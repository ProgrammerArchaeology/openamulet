...
#ifdef DEBUG
void
am_report_set_sel_value(bool selected_slot, Am_Object inter_or_cmd,
                        Am_Object obj, bool value)
{
  std::cout << "++ Object " << inter_or_cmd << " setting ";
  if (selected_slot)
    std::cout << "Am_SELECTED";
  else
    std::cout << "Am_INTERIM_SELECTED";
  std::cout << " of " << obj << " to ";
  if (value)
    std::cout << "true\n" << std::flush;
  else
...
void
report_set_value(Am_Object inter_or_cmd, Am_Slot_Key slot, Am_Object obj,
                 Am_Value value)
{
...
Am_Define_Method(Am_Object_Method, void, Am_Choice_Start_Do, (Am_Object inter))
{
...
static Am_Initializer *initializer =
    new Am_Initializer(DSTR("Inter_Choice"), init, 3.3f);
