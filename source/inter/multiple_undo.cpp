...
Am_Define_Method(Am_Handler_Selective_Undo_Method, Am_Object,
  ...
  bool should_register = !command_obj.Peek(Am_IMPLEMENTATION_PARENT).Valid();
  Am_Object new_top =
    am_perform_on_cmd_and_impls(command_obj, Am_SELECTIVE_REPEAT_SAME_METHOD,
				true, Am_No_Value, Am_No_Value,
				should_register);
  if (should_register)
    do_multiple_register(undo_handler, new_top, false);
  return new_top;
...
Am_Define_Method(Am_Handler_Selective_Repeat_New_Method, Am_Object,
  ...
  bool should_register = !command_obj.Peek(Am_IMPLEMENTATION_PARENT).Valid();
  Am_Object new_top =
    am_perform_on_cmd_and_impls(command_obj, Am_SELECTIVE_REPEAT_ON_NEW_METHOD,
			true, new_selection, new_value, should_register);
  if (should_register)
    do_multiple_register(undo_handler, new_top, false);
  return new_top;
}
...
static void init()
{
  Am_Multiple_Undo_Object = Am_Undo_Handler.Create(DSTR("Am_Multiple_Undo_Object"))
...
static Am_Initializer* initializer = new Am_Initializer(DSTR("Am_Undo_Handler"), init, 3.14f);
