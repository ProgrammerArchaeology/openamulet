...
void
Am_Priority_List::Delete(Am_Object obj)
{
  this->Start();
  if (this->Member(obj))
    this->Delete();
...
void
Am_Priority_List::Start()
{ // Make first element be current.
  current = head;
}

void
Am_Priority_List::End()
{ // Make last element be current.
  current = tail;
}

void
Am_Priority_List::Prev()
{ // Make previous element be current.
  if (current)
...
void
Am_Priority_List::Next()
{ // Make next element be current.
  if (current)
...
std::ostream &
operator<<(std::ostream &os, Am_Priority_List &list)
{
  list.Print(os);
  return os;
}
