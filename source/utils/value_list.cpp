...
Am_Value_List_Data::~Am_Value_List_Data() { Make_Empty(); }
...
void
Am_Value_List_Data::Set(const Am_Value &value, Am_List_Item *current)
{
  if (!current)
...
void
Am_Value_List_Data::Make_Unique(Am_Value_List_Data *&data,
                                Am_List_Item *&current)
{
  if (!Is_Unique()) {
...
std::ostream &
operator<<(std::ostream &os, const Am_Value_List &list)
{
  list.Print(os);
  return os;
}
...
bool
Am_Value_List::Test(const Am_Wrapper *in_data)
{
  if (in_data)
    return in_data->ID() == Am_Value_List_Data::Am_Value_List_Data_ID();
  else
    return false;
}
bool
Am_Value_List::Test(const Am_Value &in_value)
{
...
Am_Value_Type
Am_Value_List::Type_ID()
{
  return Am_Value_List_Data::Am_Value_List_Data_ID();
}

bool
Am_Value_List::operator==(const Am_Value_List &test_value) const
{
...
void
Am_Value_List::Move_Nth(int index)
{
  if (!data)
...
Make_Member(Am_Ptr) Make_Member(int) Make_Member(long) Make_Member(bool)
    Make_Member(float) Make_Member(double) Make_Member(char)
        Make_Member(const char *) Make_Member(const Am_String &)
            Make_Member(Am_Generic_Procedure *) Make_Member(Am_Method_Wrapper *)
                Make_Member(const Am_Value &)

#undef Make_Member

#define Make_Add(type)                                                         \
  Am_Value_List &Am_Value_List::Add(type value, Am_Add_Position position,      \
                                    bool unique)                               \
  {                                                                            \
    if (data) {                                                                \
      if (unique)                                                              \
    ...
    return *this;                                                              \
                    ...
                    Make_Add(Am_Wrapper *) Make_Add(Am_Ptr) Make_Add(int)
                        Make_Add(long) Make_Add(bool) Make_Add(float) Make_Add(
                            double) Make_Add(char) Make_Add(const char *)
...
bool
Am_Value_List::Start_Member(Am_Wrapper *value)
{
  Start();
  return Member(value);
}
...
Am_Value_List &
Am_Value_List::Add(const Am_Value &value, Am_Add_Position position, bool unique)
{
  if (data) {
    if (unique)
      data->Make_Unique(data, item);
...
#define Make_Insert(type)                                                      \
  void Am_Value_List::Insert(type value, Am_Insert_Position position,          \
                             bool unique)                                      \
  {                                                                            \
    if (data) {                                                                \
      if (unique)                                                              \
...
Make_Insert(Am_Wrapper *) Make_Insert(Am_Ptr) Make_Insert(int) Make_Insert(long)
    Make_Insert(bool) Make_Insert(float) Make_Insert(double) Make_Insert(char)
        Make_Insert(const char *) Make_Insert(const Am_String &)
            Make_Insert(Am_Generic_Procedure *) Make_Insert(Am_Method_Wrapper *)
...
Am_Value_List &
Am_Value_List::Append_New(Am_Value other_value, bool unique)
{
  if (data) {
    if (unique)
      data->Make_Unique(data, item);
...
void
Am_Value_List::Set_Difference(Am_Value_List other_list, bool unique)
{
  if (data) {
    if (unique)
      data->Make_Unique();
  } else
    Am_Error("Value Lists is not initialize");
  for (other_list.Start(); !other_list.Last(); other_list.Next()) {
...
Am_Value
Am_Value_List::From_String(const char *string)
{
  if (data)
...
void
Am_Value_List::Print(std::ostream &out) const
{
  if (data)
    data->Print(out);
  else
    out << "(Am_Value_List)(0L)";
}
...
void
Am_Assoc_Data::Print(std::ostream &out) const
{
  out << To_String();
  return;
}
...
std::ostream &
operator<<(std::ostream &os, const Am_Assoc &as)
{
  as.Print(os);
  return os;
}
...
Am_Value
Am_Value_List::Assoc_2(const Am_Value &value_1)
{
  if (!data)
    return Am_No_Value;
  Am_List_Item *this_item = data->head;
...
Am_Value
Am_Value_List::Assoc_2(Am_Wrapper *value_1)
{
  Am_Value v = value_1;
  return Assoc_2(v);
}
...
Am_Value
Am_Value_List::Assoc_1(Am_Wrapper *value_2)
{
  Am_Value v = value_2;
  return Assoc_1(v);
}
