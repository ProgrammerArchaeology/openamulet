...
//
// Constructors
Am_Filename::Am_Filename(const char* in_fname)
  : filename(in_fname)
{ }

Am_Filename::Am_Filename(const std::string in_fname)
  : filename(in_fname)
{ }
...
// Conversion
const char* Am_Filename::c_str() const
{
  return filename.c_str();
}

std::string Am_Filename::name() const
{
  return filename;
}

// Operators
bool Am_Filename::operator==(Am_Filename f)
{
  return (filename == f.filename);
}

bool Am_Filename::operator==(const Am_Filename f) const
{
  return (filename == f.filename);
}

// Output method
void Am_Filename::Print(std::ostream& out) const
{
  out << filename;
}

// Query methods
bool Am_Filename::Is_Directory() const
{
#ifdef _WIN32
  WIN32_FIND_DATA *mEntryInformation = new WIN32_FIND_DATA;
  HANDLE mDirectoryIterator = FindFirstFile(filename.c_str(), mEntryInformation);  
  if (mDirectoryIterator != INVALID_HANDLE_VALUE)
    {
      FindClose(mDirectoryIterator);
    }
  bool res = ((mEntryInformation->dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) == FILE_ATTRIBUTE_DIRECTORY);
...
bool Am_Filename::Is_Relative() const
{
  return ( (filename.size() == 0) ? false
	   : (filename[0] != '/') ); // UNIX-specific ? TODO: Check on Win32
}

bool Am_Filename::Exists() const
{
#ifdef _WIN32
  bool res = true;
...
static const char envvar_separator = ':';
#endif
Am_Data_Locator::Am_Data_Locator(const Am_Filename first_dir, const char* envvarname)
{
...
bool Am_Data_Locator::Find_File (const Am_Filename f, Am_Filename& found) const
{
...
void Am_Data_Locator::Add_Search_Directory(const Am_Filename dir)
{
  if (dir.Is_Directory())
    {
      dir_list_t::iterator it = std::find_if(search_list.begin(),
					     search_list.end(),
					     search_pred(dir));
	...
	++((*it).second);
	  ...
	  std::cout << "Am_Data_Locator: search path " << dir << " is not a directory!" << std::endl;
  return;
}

void Am_Data_Locator::Remove_Search_Directory(const Am_Filename dir)
{
  dir_list_t::iterator it = std::find_if(search_list.begin(),
					 search_list.end(),
					 search_pred(dir));
  if (it != search_list.end())
	...
	search_list.erase(it); // and even deletes it if we reach 0
	  ...
	  std::cout << "Am_Data_Locator: removing search path " << dir << " absent from search list" << std::endl;
  return;
}
...
void Am_Data_Locator::Print(std::ostream& out) const
{
