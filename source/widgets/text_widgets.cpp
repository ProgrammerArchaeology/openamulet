...
static void
draw_text_input(int left, int top, int width, int height, const char *string,
                Am_Object obj, Am_Object string_obj, bool /*active*/,
                bool key_selected, Am_Font label_font, Am_Widget_Look look,
                ...
                bool mask = false)
{
  ...
  if (look == Am_MOTIF_LOOK) {
    if (key_selected)
      draw->Draw_Rectangle(Am_Key_Border_Line, Am_No_Style, left, top, width,
                           height, mask ? Am_DRAW_MASK_COPY : Am_DRAW_COPY);
  ...
  if (string) {
    ...
    draw->Draw_Text(Am_Black, string, strlen(string), label_font, left, str_top,
  ...
  case Am_MOTIF_LOOK_val:
    if (mask)
      ...
      Am_Draw_Motif_Box(box_left, top, box_width, height, true, color_rec,
                        draw);
    break;
  ...
  case Am_MACINTOSH_LOOK_val:
    draw->Draw_Rectangle(Am_Black, Am_White, r.left, r.top, r.width, r.height,
                         mask ? Am_DRAW_MASK_COPY : Am_DRAW_COPY);
    break;
                  ...
                  int y_offset))
{
  ...
  draw_text_input(left, top, width, height, string, obj, string_obj, active,
                  key_selected, label_font, look, rec, drawonable);
}
                  ...
                  int y_offset))
{
  ...
  // finally ready to draw it
  draw_text_input(left, top, width, height, string, obj, string_obj, active,
                  key_selected, label_font, look, rec, drawonable, true);
}
                 ...
                 (Am_Object inter_command))
{
  ...
  if (widget_command.Valid()) {
    if (!widget_command.Is_Instance_Of(Am_Command))
      Am_ERRORO("Am_COMMAND slot of widget "
                    << widget
    ...
    std::cout << "**No widget for " << inter_command << " in " << widget
              << std::endl;
}
...
Am_Define_Formula(int, text_widget_box_left)
{
  ...
  if (string.Valid()) {
    ...
    draw->Get_String_Extents(label_font, string, strlen(string), width, ascent,
                             descent, a, b);
  } else if (obj.Valid())
...
bool
icheck_max_min(Am_Value &result, long mini, long maxi, bool want_db,
               Am_Object &widget)
{
...
bool
fcheck_max_min(Am_Value &result, float minf, float maxf, bool want_db,
               Am_Object &widget)
{
...
Am_Object
get_next_from(Am_Value_List &all_text_widgets, bool forward)
{
  if (forward) {
...
Am_Object
find_next_active_widget(Am_Object &next_widget, Am_Value_List &all_text_widgets)
{
...
bool
check_value_legal(Am_Object &text, Am_Object &inter)
{
...
init()
{
  ...
  Am_Text_Input_Widget =
      Am_Widget_Aggregate.Create(DSTR("Text_Input_Widget"))
          ...
          .Add_Part(
              Am_INTERACTOR,
              inter =
                  Am_Text_Edit_Interactor.Create(DSTR("inter_in_text_input"))
  ...
  Am_Password_Input_Widget.Add_Part(
      Am_TEXT_WIDGET_TEXT_OBJ,
  ...
  Am_Tab_To_Next_Widget_Command =
      Am_Command.Create(DSTR("Tab_To_Next_Widget_Cmd"))
  ...
  Am_Tab_To_Next_Widget_Interactor =
      Am_One_Shot_Interactor.Create(DSTR("Tab_To_Next_Widget_Interactor"))
          ...
          .Set(Am_PRIORITY,
               Am_INTER_PRIORITY_DIFF + 5) //must be higher than running
...
static Am_Initializer *initializer =
    new Am_Initializer(DSTR("Text_Widgets"), init, 5.3f, 123, cleanup);
