...
bool
in_inside_scroll_group(Am_Object in_obj, int x, int y)
{
                  ...
                  const Am_Object &ref_obj, bool want_self, bool want_groups))
{
  ...
  if (in_inside_scroll_group(in_obj, x1, y1)) {
    int offx, offy;
    scroll_group_translate_coordinates_proc(in_obj, Am_No_Object, 0, 0, offx,
                                            offy);
...
Am_Define_Formula(int, scroll_clip_left)
{
  ...
  if ((bool)self.Get(Am_V_SCROLL_BAR) &&
      (bool)self.Get(Am_V_SCROLL_BAR_ON_LEFT)) {
    Am_Object v_scroller = self.Get_Object(Am_V_SCROLLER);
    Am_Widget_Look look = v_scroller.Get(Am_WIDGET_LOOK);
    int extra_width = (look == Am_MOTIF_LOOK) ? 4 : 0;
    int scroll_width =
        (int)v_scroller.Get(Am_WIDTH) - extra_width + SCROLL_MARGIN;
    ret += scroll_width;
  }
  return ret;
}
...
Am_Define_Formula(int, scroll_clip_top)
{
  ...
  if ((bool)self.Get(Am_H_SCROLL_BAR) &&
      (bool)self.Get(Am_H_SCROLL_BAR_ON_TOP)) {
    Am_Object h_scroller = self.Get_Object(Am_H_SCROLLER);
    Am_Widget_Look look = h_scroller.Get(Am_WIDGET_LOOK);
    int extra_width = (look == Am_MOTIF_LOOK) ? 4 : 0;
    int scroll_height =
        (int)h_scroller.Get(Am_HEIGHT) - extra_width + SCROLL_MARGIN;
    ret += scroll_height;
  }
  return ret;
}
...
Am_Define_Formula(int, scroll_clip_height)
{
  ...
  if ((bool)self.Get(Am_H_SCROLL_BAR)) {
    ...
    Am_Widget_Look look = h_scroller.Get(Am_WIDGET_LOOK);
    int extra_width = (look == Am_MOTIF_LOOK) ? 4 : 0;
    return group_height - (int)h_scroller.Get(Am_HEIGHT) + extra_width -
           SCROLL_MARGIN;
  } else
    return group_height;
}
...
Am_Define_Formula(int, v_scroll_height)
{
  ...
  int scroll_border = (look == Am_MOTIF_LOOK) ? 2 : 0;
  int ret =
      (int)group.Get(Am_CLIP_HEIGHT) + 2 * scroll_border + 2 * group_border;
  return ret;
}
                  ...
                  int y_offset))
{
  ...
  if (mywidth && myheight) {
    ...
    if (Am_Type_Class(value.type) == Am_WRAPPER)
      ...
      background_style = self.Get(Am_FILL_STYLE);
...
void
scroll_group_inter_abort_do(Am_Object command_obj, Am_Slot_Key slot)
{
...
Am_Define_Method(Am_Object_Method, void, scroll_group_v_do, (Am_Object command))
{
...
Am_Define_Method(Am_Object_Method, void, scroll_group_h_do, (Am_Object command))
{
...
{
  Am_Object group = self.Get_Owner();
  if ((bool)group.Get(Am_VISIBLE) && (bool)group.Get(Am_H_SCROLL_BAR))
    return true;
  else
    return false;
}
...
init()
{
  ...
  Am_Scrolling_Group =
      Am_Widget_Group.Create(DSTR("Scrolling_Group"))
          ...
          .Add_Part(Am_V_SCROLLER,
                    v_scroll =
                        Am_Vertical_Scroll_Bar.Create(DSTR("V_Scroll_In_Group"))
                            ...
                            .Set(Am_SMALL_INCREMENT,
                                 Am_From_Owner(Am_V_SMALL_INCREMENT))
                            .Set(Am_LARGE_INCREMENT,
                                 Am_From_Owner(Am_V_LARGE_INCREMENT))
                            .Set(Am_PERCENT_VISIBLE, v_scroll_percent_visible));
  // Visual C++ can't handle long expressions
  Am_Scrolling_Group.Add_Part(
      Am_H_SCROLLER,
      h_scroll =
  ...
  v_scroll.Get_Object(Am_INTERACTOR)
  ...
  h_scroll.Get_Object(Am_COMMAND)
      ...
      .Set(Am_SELECTIVE_REPEAT_ON_NEW_METHOD, (0L))
      .Set(Am_IMPLEMENTATION_CHILD, Am_DONT_UNDO_CHILDREN);
  h_scroll.Get_Object(Am_INTERACTOR)
      .Get_Object(Am_COMMAND)
