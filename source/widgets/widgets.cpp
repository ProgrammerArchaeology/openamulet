...
////////////////////////////////////////////////////////////////////////
#if 1
  void Am_Widget_Look::Print(std::ostream& out) const {				 \
    Am_Enum_Print_Helper(out, Am_Widget_Look_ID, (long)value);      \
  }                                                              \
  void Am_Widget_Look::Println() const {					 \
...
Am_ID_Tag widget_look_id =
      Am_Get_Unique_ID_Tag ("Am_Widget_Look", Am_ENUM_TYPE);


Am_Widget_Look_Enum_Support Am_Widget_Look_Support ("Am_MOTIF_LOOK "
     "Am_WINDOWS_LOOK Am_MACINTOSH_LOOK", widget_look_id);

std::ostream& operator<< (std::ostream& os, const Am_Widget_Look& item) {
  item.Print(os);
  return os;
}
...
{
        DERR("** Tried to set a " "Am_Widget_Look" " enum with a non "
                 "Am_Widget_Look" " wrapper: " << in_value <<std::endl);
	Am_Error();
}
...
Am_Define_Style_Formula (Am_Default_Motif_Fill_Style)
{
  ...
  if (win.Valid()) {
    ...
    if (color) return Am_Default_Color;
    else return Am_White;
  }
  else return Am_Default_Color;
}
...
bool make_old_bright_enough(const Am_Style &old, float &y, float &i, float &q){
...
Am_Style make_style_from_yiq (float y, float i, float q, bool adjust) {
  ...
  if (adjust) {
    if (red != 1.0f) red = fmin(1.0f, red + static_cast<float>(FULL_SAT_ADJUST));
    else if (green != 1.0f) green = fmin(1.0f, green + static_cast<float>(FULL_SAT_ADJUST));
    else if (blue != 1.0f) blue = fmin(1.0f, blue + static_cast<float>(FULL_SAT_ADJUST));
...
void make_converted_styles(const Am_Style &old,
			   float adjustment1, Am_Style &style1,
			   float adjustment2, Am_Style &style2,
			   float adjustment3, Am_Style &style3,
			   float adjustment4, Am_Style &style4,
...
void print_color(std::ostream& os, const char* s, const Am_Style& style) {
  float red,green,blue;
  style.Get_Values(red, green, blue);
  os << s << "=(" << red << "," << green << "," << blue << ")";
}
void Computed_Colors_Record_Data::Print (std::ostream& os) const {
  os << "[" << (void*)this << "]";
    ...
    Computed_Colors_Record_Data (const Am_Style& foreground)
{
  ...
  if (foreground == Am_No_Style) {
    ...
    make_converted_styles(foreground,
			  Y_SHADOW_VALUE, shadow_style,
			  Y_BACKGROUND_VALUE, background_style,
			  1.0f, foreground_style,
			  Y_HIGHLIGHT2_VALUE, highlight2_style,
			  Y_HIGHLIGHT_VALUE, highlight_style);
  ...
  if (Am_Input_Char::Test (accel)) {
    ...
    if (old_cmd.Valid()) {
     std::cerr << "** Amulet Error, In window " << window
      ...
      if (widget.Valid())std::cerr << " in widget " << widget;
     std::cerr << " conflicts with accelerator on existing command " << old_cmd;
...
static void check_create_inter_and_list(Am_Object &window, Am_Object &inter,
...
Am_Object Am_Check_Accelerator_Char_For_Window(Am_Input_Char accel,
					       Am_Object window) {
  if (window.Valid()) {
    Am_Value value;
...
void Am_Share_Accelerators(Am_Object &source_window, Am_Object &new_window) {
  Am_Object inter;
  Am_Value_List accel_list;
...
void widgets_set_slot_names () {
  ...
  Am_Register_Slot_Key (Am_SELECTIONS_HANDLES_USE_LT_RB, "~SELECTIONS_HANDLES_USE_LT_RB~");
  ...
  Am_Register_Slot_Key (Am_SELECT_CLOSEST_POINT_OBJ,
			"SELECT_CLOSEST_POINT_OBJ");
...
Am_Define_Method(Am_Object_Method, void, am_cycle_widget_look,
  ...
  switch(look.value) {
  case Am_MOTIF_LOOK_val: look = Am_WINDOWS_LOOK; break;
  case Am_WINDOWS_LOOK_val: look = Am_MACINTOSH_LOOK; break;
  case Am_MACINTOSH_LOOK_val: look = Am_MOTIF_LOOK; break;
  default: Am_Error ("Unknown Look parameter"); break;
  }
...
void Am_Widgets_Initialize()
{
  ...
  Am_Accelerator_Inter = Am_One_Shot_Interactor.Create(DSTR("Accelerator"))
...
void Am_Pop_Up_Error_Window(const char *error_string) {
  Am_Beep();
  Am_Show_Alert_Dialog(error_string);
}


static Am_Initializer* widgets_basics_init = new Am_Initializer(DSTR("Widgets_Basics"), Am_Widgets_Initialize, 5.0f, 124, Am_Cleanup_Widgets);
