...
public:
  ...
  {
    return (
        static_cast<Am_Value_List>(listbox.Get(Am_GRAPHICAL_PARTS)).Length());
  };
  ...
  operator Am_Object &() { return (listbox); }
protected:
private:
...
Am_Listbox<OWNER>::Am_Listbox(OWNER aOwner) : owner(aOwner)
{
  // init the new object
  initialize();
}
...
template <class OWNER>
void
Am_Listbox<OWNER>::deleteAllItems()
{
  // Get all the items
  Am_Value_List itemList =
      static_cast<Am_Value_List>(listbox.Get(Am_GRAPHICAL_PARTS));
...
template <class OWNER>
void
Am_Listbox<OWNER>::deleteFirstItem()
{
  // Get all the items
  Am_Value_List itemList =
      static_cast<Am_Value_List>(listbox.Get(Am_GRAPHICAL_PARTS));
...
template <class OWNER>
void
Am_Listbox<OWNER>::deleteLastItem()
{
  // Get all the items
  Am_Value_List itemList =
      static_cast<Am_Value_List>(listbox.Get(Am_GRAPHICAL_PARTS));
...
template <class OWNER>
void
Am_Listbox<OWNER>::initialize()
{
  // listbox interactor command
  listbox_interactor_command =
      Am_Command.Create().Set(Am_DO_METHOD, listbox_method);
  ...
  listbox =
      Am_Scrolling_Group.Create("listbox")
          ...
          .Set(Am_VISIBLE,
               Am_From_Owner(
                   Am_VISIBLE)) // inherits the visible flag from the owner
  ...
  listitem =
      Am_Group.Create("listitem_group")
          ...
          .Add_Part(Am_Rectangle.Create("listitem_rectangle")
                        ...
                        .Set(Am_LINE_STYLE, selected_owner_color)
                        .Set(Am_FILL_STYLE, selected_owner_color)
                        .Add_Part(listbox_interactor))
          .Add_Part(Am_LISTITEM,
                    Am_Text.Create("listitem_text").Set(Am_TEXT, ""));
}

//: inserts an item at the beginning into the listbox
template <class OWNER>
void
Am_Listbox<OWNER>::insertItemFront(std::string const &item)
{
...
template <class OWNER>
void
Am_Listbox<OWNER>::insertItemBack(std::string const &item)
{
  // create the listitem to be added
  Am_Object tmpItem = listitem.Create();
...
template <class OWNER>
void
Am_Listbox<OWNER>::selectMethod()
{
  if (static_cast<bool>(listbox.Get(Am_MULTIPLE_SELECTION)) == true) {
    Am_Show_Alert_Dialog(
        "Listbox multiple select-function not implemented yet!");
    return;
  } else {
    ...
    for (list.Start(); list.Last() == false; list.Next()) {
      ...
      if (static_cast<bool>(
              static_cast<Am_Object>(list.Get()).Get(Am_SELECTED)) == true) {
