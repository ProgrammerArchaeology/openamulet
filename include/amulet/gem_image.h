...
public:
  Am_Image_Info()
      : width(0), height(0), depth(0), num_colors(0), transparent(false),
        raw_transparent_color_index(0)
  {
  }
  Am_Image_Info(int w, int h, int d, int n, bool t, int tx)
      : width(w), height(h), depth(d), num_colors(n), transparent(t),
        raw_transparent_color_index(tx)
  {
  }
