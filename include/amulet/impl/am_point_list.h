...
public:
  ...
  _OA_DL_MEMBERIMPORT void Get_Extents(int &min_x, int &min_y, int &max_x,
                                       int &max_y) const;

  // translates all points to a new origin.
  void Translate(int offset_x, int offset_y, bool unique = true);

  // scales all points.  Scaling is relative to (origin_x, origin_y).
  void Scale(float scale_x, float scale_y, int origin_x = 0, int origin_y = 0,
             bool unique = true);
