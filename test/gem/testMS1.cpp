...
void
redraw(Am_Drawonable *drawon)
{
  int width, height;
  drawon->Get_Size(width, height);
  drawon->Clear_Area(0, 0, width, height);
  if (selected) {
    drawon->Draw_Rectangle((0L), Red, width / 4, height / 4, width / 2,
                           height / 2);
    drawon->Draw_Rectangle((0L), Stipple, 0, 0, width / 4, height / 4);
  } else {
    drawon->Draw_Rectangle((0L), Green, width / 4, height / 4, width / 2,
                           height / 2);
    drawon->Draw_Rectangle((0L), Stipple, 0, 0, width / 4, height / 4);
  }
  drawon->Flush_Output();
...
main(int argc, char **argv)
{
  ...
  if (argc == 2) {
    Am_Drawonable *root2 = Am_Drawonable::Get_Root_Drawonable(argv[1]);
