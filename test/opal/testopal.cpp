...
Am_Define_Style_Formula (rect_fill)
{
  if ((bool)self.Get (Am_SELECTED) ||
      (bool)self.Get (Am_INTERIM_SELECTED))
    return Am_Black;
  else
    return Am_White;
}
...
Am_Define_No_Self_Formula (int, at_sw_x)
{
  return sw.Get(Am_LEFT);
}
Am_Define_No_Self_Formula (int, at_sw_y)
{
  return sw.Get(Am_TOP);
}
...
Am_Define_Method(Am_Object_Method, void, change_setting, (Am_Object self))
{
  ...
  if (c == "q") {
    ...
    if (modeb) {
      ...
      modeb = true;
      second = window.Create ("new window")
        .Set (Am_LEFT, 375);
    ...
    if (in_obj) cout << "   in object " << in_obj << endl << flush;
    else cout << " NOT in an object\n" << flush;
  }
  else if (c == "Y") {
    int x1 = l1.Get(Am_X1);
    int y1 = l1.Get(Am_Y1);
...
Am_Define_No_Self_Formula (int, parrot_left)
{
  return sw.Get (Am_LEFT);
}

Am_Define_No_Self_Formula (int, parrot_top)
{
  return sw.Get (Am_TOP);
}

Am_Define_No_Self_Formula (int, parrot_width)
{
  return sw.Get (Am_WIDTH);
}

Am_Define_No_Self_Formula (int, parrot_height)
{
  return sw.Get (Am_HEIGHT);
}
...
int main (int argc, char **argv)
{
  ...
  Am_Screen
    .Add_Part (Am_Window.Create ("filter window")
  ...
  if (argc > 1) {
    ...
    if (screen2)
      screen2
        .Add_Part (Am_Window.Create ("other screen")
          .Set (Am_WIDTH, 200)
          .Set (Am_HEIGHT, 100)
  ...
  Am_Object foo_group = Am_Group.Create ("foo")
    .Add_Part (PART1, Am_Rectangle.Create ("bar"));
  Am_Object noo_group = foo_group.Create ("noo")
    .Set_Part (PART1, Am_Rectangle.Create ("bart"));
