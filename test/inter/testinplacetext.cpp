...
Am_Slot_Key Am_INPLACE_TEXT_INFO = Am_Register_Slot_Name("~INPLACE_TEXT_INFO~");

Am_Slot_Key Am_INPLACE_TEXT_NAV = Am_Register_Slot_Name("~INPLACE_TEXT_NAV~");

Am_Slot_Key Am_AFFECTING_SLOTS = Am_Register_Slot_Name("~AFFECTING_SLOTS~");
...
Am_Slot_Key Am_TEXT_X_OFFSET = Am_Register_Slot_Name("~TEXT_X_OFFSET~");

Am_Slot_Key Am_TEXT_Y_OFFSET = Am_Register_Slot_Name("~TEXT_Y_OFFSET~");
...
public:
  ...
  static Am_Text_Info *Narrow(void *value) { return (Am_Text_Info *)value; }
};
...
static Am_Object
test_inplace_text(Am_Object &inter, const Am_Object &object,
                  const Am_Object &event_window, int x, int y)
{
  Am_Value info_value;
...
group_like(Am_Object object)
{
  return (object.Is_Instance_Of(Am_Group) || object.Is_Instance_Of(Am_Map) ||
          object.Is_Instance_Of(Am_Window) || object.Is_Instance_Of(Am_Screen));
}
                 ...
                 (Am_Object command))
{
  ...
  Am_Start_Interactor(text_inter, feedback, Am_Default_Start_Char, event_window,
                      x, y, false);
}
...
Am_Define_Formula(int, inplace_feedback_resetter)
{
  ...
  text_info->Get_Info(offset_x, offset_y, string, style, font, changing_slots);
  self.Set(Am_TEXT_X_OFFSET, offset_x)
  ...
  Am_Object object;
  Am_Slot_Key key;
  for (changing_slots.Start(); !changing_slots.Last(); changing_slots.Next()) {
...
static void
Am_Inplace_Move_Cursor_Right(Am_Object text)
{
...
static void
Am_Inplace_Move_Cursor_Up(Am_Object text)
{
  Am_Text_Navigate *text_nav =
      Am_Text_Navigate::Narrow((Am_Ptr)text.Get(Am_INPLACE_TEXT_NAV));
  if (text_nav) {
...
static void
Am_Inplace_Move_Next(Am_Object text)
{
  Am_Text_Navigate *text_nav =
      Am_Text_Navigate::Narrow((Am_Ptr)text.Get(Am_INPLACE_TEXT_NAV));
  if (text_nav) {
...
static void
Am_Inplace_Add_Item(Am_Object text)
{
  Am_Text_Navigate *text_nav =
      Am_Text_Navigate::Narrow((Am_Ptr)text.Get(Am_INPLACE_TEXT_NAV));
  if (text_nav) {
...
init_proto()
{
  Am_Inplace_Text_Edit_Interactor =
      Am_One_Shot_Interactor.Create("Inplace Interactor")
  ...
  Am_Edit_Translation_Table table = Am_Edit_Translation_Table::Default_Table();
  ...
  Am_Inplace_Text_Feedback =
      Am_Text.Create("Inplace Feedback Object")
          ...
          .Add_Part(Am_INTERACTOR,
                    Am_Text_Edit_Interactor.Create()
...
static void
Am_Motif_Button_Text_Info(Am_Object self, int width, int height,
                          const char *string, Am_Font font,
                          Am_Button_Type but_type, int /*box_height*/,
                          int box_width, bool box_on_left, int align,
                          int offset_left, int &text_left, int &text_top,
                          int &text_width, int &text_height)
{
  ...
  draw->Get_String_Extents(font, string, strlen(string), text_width, ascent,
                           descent, a, b);
...
static bool
inside_button_text(const Am_Object &self, const Am_Object &ref_object,
                   int ref_x, int ref_y)
{
  ...
  if (look == Am_MOTIF_LOOK)
    Am_Motif_Button_Text_Info(self, width, height, string, font, Am_PUSH_BUTTON,
                              0, 0, false, align, 0, text_left, text_top,
  ...
  return (x >= text_left) && (x <= text_left + text_width) && (y >= text_top) &&
...
Am_Define_Method(Am_Inplace_Info_Method, Am_Text_Info *, button_inplace_text,
                 (const Am_Object &self, const Am_Object &ref_object, int ref_x,
                  int ref_y))
{
...
#define Add Am_Instrumented(Add)
    ...
    {
      Am_Object new_item =
          Am_Command.Create("button panel item").Set(Am_LABEL, text);
...
Am_Define_Method(Am_Inplace_Info_Method, Am_Text_Info *, menu_bar_inplace_text,
                 (const Am_Object & /*self*/, const Am_Object & /*ref_object*/,
                  int /*ref_x*/, int /*ref_y*/))
{
  return (0L);
...
mod_widgets()
{
  Am_Button.Add(Am_INPLACE_TEXT_INFO, button_inplace_text);
  Am_Button_Panel.Add(Am_INPLACE_TEXT_INFO, button_panel_inplace_text);
  Am_Menu_Bar.Set(Am_INPLACE_TEXT_INFO, menu_bar_inplace_text);
}
              ...
              Am_Group.Create()
                  ...
                  .Add_Part(Am_Button_Panel.Create()
                                ...
                                .Set(Am_ITEMS, Am_Value_List()
                                                   ...
                                                   .Add(Am_Command.Create().Set(
                                                       Am_LABEL, "Item 3"))))
                  .Add_Part(Am_Inplace_Text_Edit_Interactor.Create().Set(
                      Am_FEEDBACK_OBJECT, feedback_text)))
