...
void print_summary()
{
	...
	{
		std::cout << i << ".  " << all_times[i].time_each << " msec per object for " << all_times[i].name << std::endl;
	}
	std::cout << std::flush;
}
...
{
	// get the drawonable
	draw_for_win = reinterpret_cast<Am_Drawonable*>(static_cast<Am_Ptr>(win.Get(Am_DRAWONABLE)));

	// get the dispatch function
	draw_for_win->Get_Input_Dispatch_Functions(evh);

	// create an input-event by hand, hope ev.timestamp doesn't matter
	ev.input_char 	= Am_Input_Char(Am_LEFT_MOUSE, false, false, false, Am_BUTTON_DOWN, Am_SINGLE_CLICK);
...
void time_object_creation()
{
	...
	Am_Update_All();
	end_timing("set added MY_LEFT slot repeatedly in same root object", NUM_OBJ_SET);
	...
	end_timing("get NEW_SLOT as int from all objects and sum them", NUM_OBJ_CREATE);
...
void time_data_structures()
{
	...
	start_timing();
	for(const char* value = mapIteratorInt2Str(); value; value = mapIteratorInt2Str())
	{
		++counter;
		int key = mapIteratorInt2Str.Key();
	}
	end_timing("iterating through all objects in Int2Str map", NUM_OBJ_DATA_STRUCT);
	...
	start_timing();
	for(const char* value = oaMapIteratorInt2Str(); value; value = oaMapIteratorInt2Str())
	{
		++counter;
		int key = oaMapIteratorInt2Str.Key();
	}
	end_timing("iterating through all objects in OA Int2Str map", NUM_OBJ_DATA_STRUCT);
