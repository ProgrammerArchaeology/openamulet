...
#define m_height m_rc.bottom
  ...
  Am_WinNotifyInfo(Am_WinNotifyFunc func, Am_WinDrawonable *drw, int left,
...
public:
  ...
  static void CALLBACK TimeProc(UINT uID, UINT uMsg, DWORD dwUser, DWORD dw1,
                                DWORD dw2);
...
public:
  ...
  virtual Am_Drawonable *Create_Offscreen(int width = 0, int height = 0,
                                          Am_Style back_color = Am_No_Style);
  ...
  virtual void
  Raise_Window(/*const*/ Am_Drawonable *target_d); // const;  //to top
  virtual void
  Lower_Window(/*const*/ Am_Drawonable *target_d); // const;  //to bottom
  ...
  virtual void Fill_Area(Am_Style fill, int c_left, int c_top, int c_width,
                         int c_height); // not in Am_Drawonable
  ...
  virtual int Get_String_Width(const Am_Font &font, const char *s,
                               int len); //const;
  ...
  virtual void Get_String_Extents(const Am_Font &font, const char *s, int len,
                                  int &swidth, int &ascent, int &descent,
                                  int &left_bearing,
  ...
  virtual void Get_Font_Properties(const Am_Font &font, int &max_char_width,
  ...
  virtual void Get_Polygon_Bounding_Box(const Am_Point_List &pl,
                                        ...
                                        int &out_top, int &width, int &height);
  ...
  Am_WinRegion *Get_Clip() const { return m_clip; }; // not in Am_Drawonable
  ...
  virtual void Draw_3_Lines(const Am_Style &ls, const Am_Style &fs, int x1,
                            ...
                            int y4, Am_Draw_Function f = Am_DRAW_COPY);
  ...
  virtual void Draw_Text(const Am_Style &ls, const char *s, int str_len,
                         ...
                         Am_Draw_Function f = Am_DRAW_COPY,
                         const Am_Style &fs = Am_No_Style, bool invert = false);
  ...
  virtual void
  Get_Input_Dispatch_Functions(Am_Input_Event_Handlers *&evh) //const
  {
    evh = m_evh;
  };
  /*    
...
public:
  ...
  public:
    ...
    void DeselectAll();
    void Set_DC_Transparent_Stipple(const Am_Style &ls, const Am_Style &fs,
                                    int emulate = 0);
    void Adjust_DC_Transparent_Stipple(const Am_Style &fs);
    void Set_DC(const Am_Style &ls, const Am_Style &fs, Am_Draw_Function f,
                int emulate = 0);
    void Set_DC_Text(const Am_Style &ls, const Am_Style &fs, Am_Draw_Function f,
                     bool invert);
...
//----------------------------------------
  ...
  static LRESULT CALLBACK __export MainWndProc(HWND hWnd, UINT message,
  ...
  HWND WndGet() const { return (m_hwnd); };

  void WndAttach(HWND hwnd);
  void WndClose();
  bool WndCreate(bool fQueryPos = false, bool fQuerySize = false,
                 bool fSaveBits = false);
  void WndDestroy();
  HWND WndDetach();
  ...
  virtual void WndOnInput(short sType, Am_Button_Down down,
  ...
  BOOL UsesBufferring() const { return m_usebuff; }
  void SetUsesBufferring(BOOL usebuff);
  BOOL IsBuffered() const { return m_isbuff; }
  void SetBuffered(BOOL isbuff);
  HBITMAP BufferBitmap() const { return m_hbmp; }
  BOOL HasBuffer() const { return BufferBitmap() != 0; }
  void CreateBuffer();
...
protected:
  void poly_draw_it(Am_Style ls, Am_Style fs, Am_Draw_Function f, POINT *pld,
                    int num_points);
