...
public:
  ...
  // internal functions
  void Create_X_Drawable (Am_Drawonable_Impl *parent_d,
  ...
  Am_Drawonable* Create_Offscreen (int width = 1, int height = 1, Am_Style background_color = Am_No_Style);
  ...
  void Clear_Clip ()
    { screen->clip_region->Clear(); Set_Clip(screen->clip_region); }
  void Set_Clip (int l, int t, unsigned int w, unsigned int h)
    { screen->clip_region->Set(l, t, w, h); Set_Clip(screen->clip_region); }
  void Push_Clip (Am_Region *the_region)
    { screen->clip_region->Push(the_region); Set_Clip (screen->clip_region); }
  void Push_Clip (int l, int t, unsigned int w, unsigned int h)
    { screen->clip_region->Push(l, t, w, h); Set_Clip(screen->clip_region); }
  void Pop_Clip ()
    { screen->clip_region->Pop(); Set_Clip(screen->clip_region); }
  bool In_Clip (int x, int y)
    { return screen->clip_region->In(x, y); }
 ...
 void Pop_Clip ();
  ...
  void adjust_gcvalues_stipple (XGCValues& values,
				unsigned long& mask,
...
{
 public:
  Font_Index (Display *disp, XFontStruct *in_xfont) : dpy(disp), xfont(in_xfont), next(0L)
    { }
  Display *dpy;
  XFontStruct *xfont;
  Font_Index* next;
...
{
 public:
  Cursor_Item (Display *disp, Cursor in_cursor) : display(disp), cursor(in_cursor), next(0L)
    { }
  Display *display;
  Cursor cursor;
  Cursor_Item* next;
...
class Am_Image_Array_Data : public Am_Wrapper
{
 ...
 public:
  ...
  inline int Get_Depth()
    { return ( (image_) ? image_->Get_Depth() : 0); }
...
protected:
  Pixmap Make_Pixmap_From_Generic_Image (const Am_Drawonable_Impl*, XColor*&, int&);
