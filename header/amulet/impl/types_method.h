...
_OA_DL_IMPORT std::ostream& operator<< (std::ostream& os, const Type_name& method) { \
  ...
  void Type_name::TypeError(const Am_Value& value) {                         \
        DERR("** Tried to assign a " #Type_name " method with a non "     \
                 #Type_name " wrapper: " << value << std::endl);                   \
	Am_Error();						             \
      }				   		
...
_OA_DL_IMPORT extern std::ostream& operator<< (std::ostream& os, const Type_name& method);
