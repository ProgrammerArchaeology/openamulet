...
class _OA_DL_CLASSIMPORT Am_Object
{
  AM_WRAPPER_DECL(Am_Object)
	...
	public:
		// CREATORS
			...
			// if given.
			_OA_DL_MEMBERIMPORT Am_Object Create(const char *new_name = static_cast<const char*>(0)) const;
			...
			_OA_DL_MEMBERIMPORT Am_Object Copy(const char *new_name = static_cast<const char*>(0)) const;
			...
			_OA_DL_MEMBERIMPORT Am_Object Copy_Value_Only(const char *new_name = (0L)) const;
		...
		// ACCESSORS
			...
			_OA_DL_MEMBERIMPORT const Am_Value& Get(Am_Slot_Key key, Am_Slot_Flags flags = 0) const;
			...
			_OA_DL_MEMBERIMPORT const Am_Value& Peek(Am_Slot_Key key, Am_Slot_Flags flags = 0) const
			{
			...
			_OA_DL_MEMBERIMPORT Am_Object Get_Object(Am_Slot_Key key, Am_Slot_Flags flags = 0) const;

			//:Like Get_Object but doesn't generate an error of the slot doesn't exists
			_OA_DL_MEMBERIMPORT Am_Object Peek_Object(Am_Slot_Key key, Am_Slot_Flags flags = 0) const
			{
			...
			_OA_DL_MEMBERIMPORT Am_Object Get_Sibling(Am_Slot_Key key, Am_Slot_Flags flags = 0) const
			{
				return(Get_Owner(flags).Get_Object(key, flags)); 
			}

			//:Like Get_Sibling but doesn't generate an error of the slot-key doesn't exists
			_OA_DL_MEMBERIMPORT Am_Object Peek_Sibling(Am_Slot_Key key, Am_Slot_Flags flags = 0) const
			{
			...
			_OA_DL_MEMBERIMPORT Am_Value_Type Get_Slot_Type(Am_Slot_Key key, Am_Slot_Flags flags = 0) const
			{
  ...
  Am_Object& Add_Part (Am_Object new_part, bool inherit = true, long aPositio = 0);

  //slot must not already exist (unless Am_OK_IF_THERE flag).
  //slot must never be a regular slot
  Am_Object& Add_Part (Am_Slot_Key key, Am_Object new_part, Am_Slot_Flags set_flags = 0);
  
  //Slot must exist (unless Am_OK_IF_THERE), and must already be a part slot
  Am_Object& Set_Part (Am_Slot_Key key, Am_Object new_part, Am_Slot_Flags set_flags = 0);
...
_OA_DL_IMPORT extern std::ostream& operator<< (std::ostream& os, const Am_Object& object);
