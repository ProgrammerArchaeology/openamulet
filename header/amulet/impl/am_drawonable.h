...
class _OA_DL_CLASSIMPORT Am_Drawonable
{
	...
	virtual Am_Drawonable 	*Create_Offscreen(int width = 0, int height = 0, Am_Style background_color = Am_No_Style) = 0;
	virtual void 			Reparent(Am_Drawonable *new_parent) = 0;

	virtual bool Inquire_Window_Borders(int& left_border, int& top_border, int& right_border, int& bottom_border,  int& outer_left, int& outer_top) = 0;
	...
	virtual void Set_Max_Size(unsigned int new_width, unsigned int new_height) = 0;
	virtual void Set_Min_Size(unsigned int new_width, unsigned int new_height) = 0;
	...
	const char* Get_Icon_Title() const
	{ return icon_name;
	}
	...
	virtual void Get_Window_Mask(bool& want_enter_leave_events,	bool& want_move_events,	bool& want_multi_window) = 0;
