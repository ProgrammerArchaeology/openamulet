...
public:
    ...
    // polygon bounding box calculation
     void Get_Polygon_Bounding_Box (const Am_Point_List& pl, 
     				const Am_Style& ls,
  ...
  void Draw_Lines (const Am_Style& ls, const Am_Style& fs, const Am_Point_Array& pts,
		     Am_Draw_Function f = Am_DRAW_COPY);
...
extern void Set_Drawable_Backpointer (WindowRef a_mac_win, Am_Drawonable_Impl* d);
...
protected:
	...
	void			load_image_data(const unsigned char* bit_data, unsigned short width,
						unsigned short height, int depth, Am_RGB_Value* color_map,
						int num_colors);
...
public:
  ...
  void Destroy ();
  void Clear ();
  void Set (int left, int top, unsigned int width,
	    unsigned int height);
  void Push (Am_Region *region);
  void Push (int left, int top, unsigned int width,
	     unsigned int height);
  void Pop ();
  void Union (int left, int top, unsigned int width,
	      unsigned int height);
  void Intersect (int left, int top, unsigned int width,
		  unsigned int height);
  bool In (int x, int y);
  bool In (int left, int top, unsigned int width,
	   unsigned int height, bool& total);
